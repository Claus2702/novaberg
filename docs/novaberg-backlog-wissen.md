...
> **Die Zählung über sieben Spalten ist vollständig und zugleich zu eng — gemessen am 12.09.2026.** Sie erfasst **persistierte** Vektorspalten, und das ist ihr Kriterium. Ein Verstoß derselben Klasse sitzt aber an einem Vergleich, der seinen Vektor **im Turn erzeugt und nie ablegt**: Der Charakter-Filter des Lückenpfades bettet den ganzen `kern_hash` als **einen** Vektor ein — ein vielgestaltiges Profil von 684 bis 4720 Zeichen — und hält einen Turn von rund hundert Zeichen dagegen. `[gemessen]` Zerlegt man den Kern in Sätze, steigt der Median jedes Paares um **+0,031 bis +0,097** und das Maximum von 0,428 auf 0,555; die Schwelle des Filters liegt bei 0,30, also mitten in diesem Abstand. **Eine Spaltenzählung kann ihn nicht finden**, weil es keine Spalte gibt — dieselbe Klasse, die keine Liste findet. Zahlen in `novaberg-convention-embedding.md` §5, Wirkung in `novaberg-kalibrierung_k.md` §3.3a.
...
| PLANNER-TIMELINE-INTENT-MISS | **Gegenstück seit dem 16.09.2026:** `PLANNER-ZEITWORT-UEBERSTIMMT-DIENSTWAHL` beschreibt die andere Richtung derselben Naht (**behoben am 17.09.2026**, Scheibe 12 D1 — dieser Eintrag bleibt offen: Er hängt an der Erkennung eines Auftrags durch den Router, nicht an der Wahl des Dienstes) — hier wird ein Timeline-Auftrag nicht erkannt, dort wird jede Zeitangabe zur Timeline geschickt. Beide fallen mit Scheibe 12 C und D, wo die Wahl über das Objekt-Merkmal gerechnet wird (`novaberg-thinking-lage_k.md`). · Der Planner erkennt explizite Timeline-Aufträge ("Merk dir bitte den 17. Oktober als Annas Geburtstag") nicht zuverlässig als Timeline-Intent und dispatcht den TimelineAgent nicht. Folge: `magnete_aufloesen` legt einen `erinnerungs_anker` an, statt einen echten `geburtstag`-Eintrag zu sehen. Aufgedeckt im P3-V7-Clipboard-Test (Chat 88): Test-Turn mit expliziter Timeline-Absicht erzeugte nur einen `erinnerungs_anker` für den 17.10.2026, keinen `geburtstag`-Eintrag. Konsequenz: Clipboard-Pattern strukturell vorbereitet, aber im Live-Betrieb selten getriggert. Tiefere Betrachtung deutet auf eine grundsätzliche Architektur-Frage hin (Agenten-Aktivierungs-Modi, Push vs. Pull), die nach P9 in einem eigenen Konzept-Doku adressiert werden soll. **Neu bewertet 14.09.2026:** Die Zustellung scheitert nicht im Planner, sondern davor — der Router setzt `management_action` nicht (`[gemessen 13.09.2026]` unter `gemma4-a4b-gpu` 0 von 40 Zustellungen an vier Aussagen mit Termin, unter `gemma4-gpu` 9 von 9). Seit dem 14.09.2026 verlangt der Timeline-Aushang ausdrücklich einen Auftrag; ein *„Merk dir …"* ist einer und wird zugestellt (12 von 12 Aufträgen im Labor). **Das Clipboard erreicht die Nutzerhälfte strukturell nie** — der HumanGraph schreibt seinen KZG-Kern, bevor der TimelineAgent im CharacterGraph läuft (Fundliste 13.09.2026). Die Architektur-Frage ist als Scheibe 12 des Lage-Konzepts entworfen (`novaberg-thinking-lage_k.md` §4). | [WIS] ⬜ Prio mittel — nach P9 strukturell adressieren |
