...
**Stand:** 19. September 2026, 13:08 UTC (§6.8: eine Ablehnung als Nicht-Auftrag ist gekennzeichnet, `Korrektur.kein_auftrag`). Davor 18. September 2026 (§7/§8.4: **der eigene Dienst schreibt sein Audit selbst**, entschieden vom Eigentümer; der Aufrufer schreibt nur, wenn der Dienst schweigt). Davor 17. September 2026, 10:58 UTC (§3.4: **die Zustellung folgt der Nähe** — der Planner fragt die Dienste in ihrer Reihenfolge, nach einer Ablehnung den nächsten, Scheibe 12 D1). Davor 16. September 2026, 20:58 UTC (§3.4: **entschieden — je Zettel für sich, ohne Abstand**; im Grenzfall stehen beide Dienste am Objekt, und die Ablehnung des einen hält den anderen nicht auf). Davor 16. September 2026, 20:18 UTC (§3.4: **die Nähe wird gerechnet, im Schatten** — der Router urteilt je akutem Objekt mit zwei Schwellen und protokolliert, ohne zuzustellen). Davor 16. September 2026, 19:43 UTC (§3.4: **das Objekt-Merkmal ist eine Angabe der Anmeldung** — deklariert von Timeline und Notizen, beim Start eingebettet, noch ohne Leser am Empfang). Davor 14. September 2026 (§3.4: die absehbare Erweiterung um das Objekt-Merkmal der Anmeldung, entschieden, nicht gebaut). Davor 17. August 2026 (v0.1, Erstfassung)
...
| **Objekt-Merkmal** | Welches Objekt der Lage bedienst du? | **zulässig, nicht Pflicht** — wird **eingebettet, nie gelesen** (seit 16.09.2026) |
...
>
> **Nachtrag 16.09.2026 — die Angabe steht, ihr Leser nicht.** `BaseAgent.objekt_merkmal`, Vorgabe leer; Timeline und Notizen deklarieren einen geeichten Wortlaut, der Server bettet ihn beim Start ein (`agents/object_feature.py`). **Drei Regeln folgen aus der Bauart:** Das Merkmal steht nicht am Brett — der Empfang liest es nicht als Text, es wird gerechnet. Es ist nicht Pflicht — nicht jeder Dienst am Empfang bedient ein Objekt, und der Handshake meldet sein Fehlen nicht. Und **sein Wortlaut ist geeicht, nicht formuliert**: Die Schwellen der Nähe gelten für genau diese Zeichen und dieses Modell. ~~Die Nähe selbst und die Zustellung sind nicht gebaut~~ → **die Nähe ist seit demselben Abend gebaut, im Schatten** (`agents/object_nearness.py`): Sie wird je akutem Objekt gegen **jedes Merkmal für sich** gerechnet, und das Urteil geht ins Pipeline-Log, nicht an den Empfang. Die Zustellung ist nicht gebaut; §3.6 gilt unverändert. ~~**Offen und nicht entschieden: ob der Abstand ein Verhältnis zwischen Zetteln ist.**~~ → **Entschieden am 16.09.2026: ohne Abstand.** Das Objekt steht an jedem Dienst, dessen Untergrenze es erreicht — an einem, an mehreren oder an keinem; im Grenzfall werden beide gefragt, und `abgelehnt` des einen beendet die Zustellung an den anderen nicht. Gemessen über die Reihe: nie nur beim falschen Dienst (0 von 60), 16 von 60 an beiden. Damit gilt §3.6 auch für die gerechnete Nähe: **kein Urteil über einen Zettel hängt davon ab, welcher andere angemeldet ist.** **Gebaut am 17.09.2026 (D1):** Der Planner fragt die nahen Dienste nach ihrer Nähe, nach `abgelehnt` den nächsten; ein Dienst ohne Merkmal, auf den der Router zeigt, wird von der Nähe nicht verdrängt. Eine Weitergabe zählt der Quotenabgleich heute als Zustellung und Ablehnung (Fundliste 17.09.2026). Die Frage, wie sie stand: Diese Angabe verlangt oben, ein Objekt stehe am Zettel eines Dienstes *ohne Verhältnis zu anderen Zetteln*; die zweite Schwelle der Eichung — der Abstand zwischen größter und zweitgrößter Nähe — vergleicht zwei Merkmale miteinander. Gemessen trägt sie (über die Reihe mit Abstand 33 zugestellt und 32 richtig, ohne ihn 37 und 34), aber eine Messung entscheidet nicht, welches Soll gilt. Im Schatten wirkt sie nicht; **vor Teil D ist es zu entscheiden.**
...
**Eine Ablehnung als Nicht-Auftrag ist gekennzeichnet** (`Korrektur.kein_auftrag`, seit 19.09.2026). Lehnt die Klassifikation ab, weil die Äußerung kein Auftrag an diesen Dienst war, sagt die Ablehnung nicht *„das stimmt so nicht"*, sondern *„das war nicht an mich"*. Dann hat niemand etwas verlangt: Der Auftraggeber bildet keinen Aufgabenblock daraus, das Kurzzeitgedächtnis führt sie nicht als abgelehnten Auftrag, und ein Angebot bleibt möglich (`novaberg-thinking-lage_t.md` §4 Scheibe 12, Teil E2, Nachtrag). Merkmal: der Beleg `Klassifikation: …` — sechs Dienste setzen es. Die Zeile des Agent-Dispatch im `pipeline_log` trägt es als `kein_auftrag`, damit eine Messung die beiden Ablehnungen trennen kann.
...
> **Entschieden am 18.09.2026, vom Eigentümer:** *„ich würde sagen, dass er am Besten seine Funktion auditieren kann, oder? Es macht sinn."* — **Der eigene Dienst schreibt sein Audit selbst**, weil nur er weiß, was er getan hat: welche Einträge, welche Zahlen, warum er leer lief. Die Zeile *Audit* in der Tabelle von §7 gilt damit unverändert. **Die Antwort auf die Frage oben** — *wer schreibt, wenn der Dienst schweigt?* — ist auch beim eigenen Dienst der Aufrufer, aber nur dann: Der Pixie-Dispatch schreibt `fehler` für eine Ausnahme, die aus dem Dienst entkommt, und für einen Auftrag an einen Dienst, den es nicht gibt; sonst nichts.
>
> Anlass war ein Rahmen im Dispatch, der vom 18.09.2026 (`ac86792`) bis zur Entscheidung `gestartet`/`erledigt` für jeden Dienst ohne eigenes Audit schrieb, gesteuert über eine Selbstauskunft `writes_own_audit`. Die Auskunft war zweimal falsch gesetzt — zwei Dienste belegten nur Teilschritte und verloren damit den ganzen Lauf. Umgebaut in `80d4b37`: fünf Dienste belegen ihren Lauf selbst, die Selbstauskunft ist entfallen, ein Zeuge verlangt von jedem Dienst, den Pixie routet, ein eigenes `gestartet` (`tests/test_background_audit.py`). **Bauform:** Der Dienst schreibt über `BaseAgent._audit`; bei den fünf nachgerüsteten Diensten ist `invoke` eine schmale Hülle, die `gestartet` schreibt, den bisherigen Lauf als `_run_once` ausführt und danach `erledigt` oder `fehler` belegt. **Für Zeugen folgt daraus:** Wer `invoke` eines Hintergrunddienstes ruft, ersetzt `agents.base.write_audit` durch eine Attrappe — sonst schreibt die Suite echte Zeilen ins produktive `hintergrund_log` (am 18.09.2026 zweimal geschehen: 42 und 38 Zeilen, beide gelöscht).
