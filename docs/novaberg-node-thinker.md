...
**Lösung — ThinkingNormalizer.** Code in `tools/thinking_normalizer.py`. Basisklasse `ThinkingNormalizer` (No-Op: `content` gilt immer als brauchbar) plus erbende Klasse `ThinkSplitNormalizer` (behandelt den Split). Auswahl über die Factory `get_thinking_normalizer()` — das modell-spezifische Verhalten ist hinter der Factory gekapselt; der Thinker selbst bleibt modell-agnostisch. (~~Connector-Factory~~: Der Schlüssel war nie der Connector, siehe den Absatz darunter.)

**Auswahl — Match aufs aufgelöste Modell, nicht den Connector-Namen.** `get_thinking_normalizer()` matcht jetzt per Substring gegen das aufgelöste GPU-Modell (`OLLAMA_MODEL`, z. B. `gemma4-gpu`), nicht mehr gegen den Connector-Namen. Grund: Der live aktive `qwen36`-Connector fährt im CharacterGraph `gemma4-gpu` und zeigt den Split — hieß aber nicht „gemma4", sodass der alte Connector-Name-Match ihn fälschlich auf den No-Op-`ThinkingNormalizer` fallen ließ (Bug NORMALIZER-CONNECTOR-NOOP, `novaberg-bugs.md`). Der Match aufs Modell ist die ehrliche Bedingung: Der Split hängt am Modell, nicht am Profilnamen.

**Und er war bis zum 05.09.2026 nur zur Hälfte ehrlich.** Verglichen wurde `OLLAMA_MODEL` — das **konfigurierte** GPU-Modell des Connectors. Welches Modell tatsächlich antwortet, entscheidet `MODEL_WORKER_BACKENDS["chat"]`; solange dort ein Ollama-Backend stand, waren beide dasselbe. Mit dem vierten Rückhalt (`openrouter`) wäre der Unterschied wirksam geworden: `OLLAMA_MODEL` sagt unverändert `gemma4-gpu`, der Split-Aufräumer hätte also weiter gegriffen, obwohl das antwortende Modell den Split gar nicht erzeugt. Die Factory liest seither `config.antwortendes_chat_modell()` — dieselbe Quelle wie die Prompt-Modellebene. Zeugen: `tests/test_thinking_normalizer_modellwahl.py` (4), Gegenprobe 2 rot bei vorhergesagter 1.
...
> ⚠ **Live gefeuert am 14.09.2026, 18:52–18:57 UTC, unter `gemma4-a4b-gpu` — die Kette lief, das Erlebnis nicht.** Erster Durchlauf: Nachfass-Iteration 1/2 und 2/2 mit leerem `content` bei gefülltem `thinking` → Geste und Self-Trigger (18:54:10). Wiederholung: *„Unsicherheits-Retry erkannt"* (18:55:20), wieder zweimal leerer `content` → *„Nachfass-Limit erreicht im Unsicherheits-Retry"*, keine weitere Selbstauslösung — **die Härtung hielt**. **Die zweite Antwort beginnt mit dem vollständigen Text der ersten** und hängt einen neuen Absatz an, unter derselben `turn_id`; im Client stehen zwei Blöcke mit demselben Anfang. Die erwartete *„zweite Nachricht mit Klärung"* trat nicht ein. Offen, woher die Wiederholung kommt (der Retry-Lauf trägt die erste Antwort im Verlauf); Fundliste 14.09.2026. Das Gesprächsmodell driftete in beiden Läufen in allen vier Nachfass-Iterationen ins `thinking`-Feld.
