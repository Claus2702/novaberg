...
#### SEARX1 — SearXNG Engines timen aus ⚠️
**Entdeckt:** Chat 48, Naan-Rezept-Frage
**Symptom:** `curl` gegen SearXNG liefert `"number_of_results": 0`, alle Engines (aol, brave, duck...) im Timeout. Nova generiert Antworten aus dem Trainings-Wissen statt zu recherchieren ("laut Rezept" → halluzinierte Quellenangabe).
**Ursache:** Unklar — entweder Netzwerk/DNS-Problem im Docker-Netzwerk oder Engines haben ihre APIs geändert.
**Prüfung:** `docker exec -it searxng curl -s "https://www.google.com" -o /dev/null -w "%{http_code}" --max-time 5` prüft, ob der Container überhaupt ins Internet kommt.
**Prio:** Mittel — Nova kann aktuell nicht recherchieren, Antworten auf Wissensfragen sind potentiell halluziniert.
