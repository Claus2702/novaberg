# Novaberg — EI: Plutchik-Emotionsmodell
...
|-----------|--------|-------------|------------|
| **Gewichtungs-Decay** | Ist die Emotion noch *da*? | Arousal-basierter logarithmischer Decay (novaberg-pixie-decay.md) | ✅ Seit Chat 16 |
...
→ EI-Konzept (Gesamtrahmen): novaberg-ei.md
→ Perzeption (erkennt Emotion + Arousal): novaberg-node-perception.md
→ Enricher (berechnet Verlauf + Normalisierung): novaberg-node-enricher.md
→ Arousal-basierter Decay: novaberg-pixie-decay.md, novaberg-ei-plutchik_l.md
→ Charakter-Profile (nutzt Emotions-Daten): novaberg-ei-character-profiles.md
→ Emotions-Vektoren (nutzt Gruppenlogik): novaberg-node-perception.md
→ Responder (nutzt Verlauf + Vektor): novaberg-node-responder.md