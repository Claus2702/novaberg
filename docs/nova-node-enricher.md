...
**Pfad:** novaberg/docs/nova-node-enricher.md
...
> **Lesson gelernt (Chat 7):** Shadow-Delivery-Turns verunreinigten den Responder-Kontext. Lösung: Shadow-Turns werden komplett gefiltert. → nova-pixie_l_kontamination.md

> **Lesson gelernt (Chat 30):** Die frühe Destillation (`kern` statt `inhalt`, Metadaten als String-Tags) zerstörte emotionale Information. Der Responder sah sachliche Zusammenfassungen statt Originaltext und antwortete therapeutisch. Lösung: Vollständiges Durchreichen. → nova-graph_l_datentransport.md
...
→ Details: nova-node-perception.md, Abschnitt EI-Plausibilitäts-Gate
...
→ Konzept: nova-graph.md — Graph-Konzept`
→ Architektur: nova-graph.md — Graph-Architektur`
→ Emotionale Intelligenz: nova-ei.md — EI-Konzept`, `nova-node-perception.md — Perzeption & Emotions-Vektoren`
→ Plutchik-Emotionsmodell: nova-ei-plutchik.md
→ Lesson Timing-Bug: nova-node-perception.md (Turn-0-Fix)
→ Lesson Session-Kontamination: nova-pixie_l_kontamination.md
→ Profil-Pipeline (CAT + Destillation): nova-ei-character-profiles.md
