...
**Befund (2026-07-31).** `gv_detail["laenge"]` heißt wie eine Antwortlänge und ist die **Vektorlänge**: die Zahl der Antizipationsschritte aus `_vektor_laenge_berechnen`, hart auf 3 gedeckelt (Cognitive Load Theory). Wer beim Bauen einer Umfangsregel darauf aufsetzt, rechnet auf einem Wert, der für etwas anderes erhoben wurde — dieselbe Fehlerklasse wie eine Schwelle aus einer anderen Größe. **Und der Deckel war bis zum 12.09.2026 nicht 3:** In den drei Modi mit Zuschlag −0,3 lag er bei **2** — beste Summe exakt 2,5, `round` zur geraden Zahl —, also hätte eine Umfangsregel dort drei Stufen statt vier gehabt, ohne dass es irgendwo stand. **Seit dem 12.09.2026 behoben** (`GV-LAENGE-RUNDUNG-ZUR-GERADEN`, im Archiv). Der Hinweis bleibt: Wer auf dieser Zahl eine Umfangsregel baut, rechnet weiterhin auf einem Wert, der für das Vorausdenken erhoben wurde.
...
**Zustand:** **zur Haelfte gebaut am 30.08.2026** — der Testlauf existiert. `client/tests/` mit 17 Zeugen (**Nachtrag 13.09.2026: 30** — die Zuordnung an einen anderen Absender und die Ausfallmeldung mit Kennungen, Gegenproben 3/3, 1/1, 1/1; der offene Rest unten gilt unveraendert), Aufruf `python3 -m unittest discover -s tests -t .` aus `novaberg/client`; Gegenprobe 3/2/2 rot. **Die Zuordnungspruefung ist gedeckt** (alle drei Ausgaenge, die Menge offener Fragen, der Fall *Antwort ohne Kennung* getrennt von *passend*), dazu die Verdrahtung *registriert → Toolbar-Button*, an der das Kontext-Panel am 28.08. haengengeblieben war. **Was die Diagnose von 2026-08-01 uebersah:** Der Client ist im Behaelter nicht importierbar, **auf dem Host aber schon** — GTK 3.56.3 ist dort vorhanden, und `ui.stream_handler` laedt ohne Display und ohne laufenden Client. **Offener Rest:** Der Lauf haengt an einem Menschen, der ihn aufruft; er ist nicht Teil der Server-Suite und faellt deshalb still aus, wenn niemand daran denkt. Vom Client sind 2 von 24 Dateien bezeugt. Voriger Zustand: offen — nachgesehen am 28.08.2026, ein Testlauf fuer den Client ist nicht entstanden. **Datenpunkt 28.08.2026, abends:** zwei Panel-Aenderungen (Gespraechskontext-Tab neu, Ziele-Panel ohne inaktive Karten) mit `py_compile` als einziger Pruefung; und das Kontext-Panel hing seit dem Morgen registriert, aber ohne Toolbar-Eintrag — ein Zeuge ueber `_TOOLBAR_PANELS` gegen die Registry haette es am Tag des Baus gefunden.
...
**Zustand:** ✅ **umgesetzt am 28.08.2026, abends.** Beleg: `graph/nodes/sachlage.py::question_target`, `ei/haltungssprache.py::_rueckfragenzeile(gegenstand)`, Verfasser reicht durch; 10 Zeugen, Suite 2488 gruen, Gegenprobe 1/2/3/1 rot; Labor `labor/2026-08-28_scheibe3_zwei_arme.py` (zwei Arme ueber die zehn Aeusserungen der Scheibe 1: Rueckfrage trifft den Gegenstand 4/4 gegen 1/4, Fragehaeufigkeit 10/10 in beiden Armen); Betrieb 21:04 UTC (`Rueckfrage-Gegenstand:` im Log, Novas Schlussfrage traf die zweite offene Eigenschaft). **Rest, als Rest benannt:** die Betriebsmessung der Formfrage (2,2 Fragen je Turn) steht aus — Konzept §5; ob eine vom Nutzer gestellte Frage als Deckung gelesen werden soll, ist offen (Fundliste 28.08.). **Nachtrag 29.08. abends — der eigene Zug:** `question_target_origin` liefert die Herkunft des Gegenstands (`blase` | `eigener_zug`), die Rueckfrage-Zeile fuehrt, wenn das staerkste Kurzziel nicht zur Blase gehoert; 10 Zeugen (`tests/test_question_origin.py`), Gegenprobe 2/2/1, Labor 5/5 statt 0/5 als Person As Schritt; dazu spricht der `[SACHLAGE]`-Block in den Namen seines Lesers (Person A/B beim Verfasser — 5 Zeugen `tests/test_sachlage_block_reader.py`, Suite 2586).
...
## Feature: SACHLAGE-SCHEIBE-7-PLAUSIBILITAET — Behauptungen des Nutzers gegen Weltwissen (29.08.2026)

**Kategorie:** [ANT] ANTWORTPFAD

**Zustand:** ✅ **umgesetzt am 29.08.2026, morgens** — entworfen, gebaut und gemessen in einem Zug. Beleg: `graph/nodes/sachlage_plausibility.py` (`has_acute_object`, `assess_plausibility`, `apply_plausibility`), 13 Zeugen (`tests/test_sachlage_plausibility.py`), Suite 2533 gruen, Gegenprobe 1/1/1/1 rot; Labor `labor/2026-08-29_sachlage_plausibilitaet_messung.py` (zweite Prompt-Fassung: Fehlalarme 0/12, 15 von 18 nicht-plausiblen gemeldet, Stufe im Mittel eine zu hoch; »schneller als Licht« 0/3). Konzept `novaberg-thinking-lage_k.md` §4 Scheibe 7, Pipeline-Konzept §11 (Schritt 5 in der Konversationsfassung), Frames §6. **Rest, als Rest benannt:** die Stufeneichung (der Call haelt Ungewoehnliches fuer Konflikt und Konflikt fuer Unmoeglich — ob das im Verfasser stoert, zeigt der Betrieb); die als Scherz gelesene Unmoeglichkeit; Schritt 4 (Cross-Frame) bleibt Konzept, weil er Slots aus Fakten oder Lager braucht; der Reaktionszeitpunkt ist sofort (Frames §5.5 A), eine aufgeschobene Reaktion waere eine Pixie-Sache.

Die Sachlage deckte jede Behauptung des Nutzers als Eigenschaft, ohne zu fragen, ob die Welt sie hergibt; der Sykophanz-Sprint (`graph/einwand.py`) urteilt nur ueber Einwaende gegen Novas Werte. Jetzt prueft ein eigener kleiner Call — nur bei akutem Objekt, nach dem Aufloeser — die Aeusserung gegen Weltwissen in den vier Stufen aus Frames §6.2; nur die drei ueber `plausibel` stehen im Artefakt (`plausibilitaet` je akutem Objekt), der `[SACHLAGE]`-Block nennt sie dem Verfasser als *Zweifel (Stufe): Behauptung — Grund*, der Kontext-Tab zeigt sie. **Die Form der Reaktion bleibt bei Haltung und Vehikel.** Preis: 1,4 s je Turn mit akutem Objekt.

**Zusammenhang:** SACHLAGE-SCHEIBE-6-AUFLOESER (laeuft davor) · SACHLAGE-SCHEIBE-3-FRAGE-GEGENSTAND · Sykophanz-Eindaemmung B1/B4 (der Einwand, nicht die Behauptung) · Pipeline-Konzept §4.5 (Zielbild), §4.4 und §12.1 (offen).


## Feature: SACHLAGE-SCHEIBE-8-TRAEGER — der Wissenstraeger: Antwortstoff statt Rueckfrage, mit Websuche (29.08.2026)

**Kategorie:** [ANT] ANTWORTPFAD

**Zustand:** ✅ **umgesetzt am 29.08.2026, vormittags** — Anlass, Entwurf, Bau und Messung in einem Zug. Beleg: `traeger` im Sachlage-Call und in `_validate_artifact`, `question_target`/`answer_targets`, `graph/nodes/sachlage_research.py`, 15 Zeugen (`tests/test_sachlage_traeger.py`), Gegenprobe 2/1/2/1 rot; Labor `labor/2026-08-29_sachlage_traeger_messung.py` (Alltag 43 nutzer / 5 welt / 0 nachschlagen, Wissenschaft 3 / 24 / 15, 0 ohne Traeger); Konzept `novaberg-thinking-lage_k.md` §4 Scheibe 8, Pipeline-Konzept §11 (Schritt 6 als Lueckenbehandlung in der Konversationsfassung, Schritt 9 mit einer Websuche begonnen). **Rest, als Rest benannt:** *Zutaten* und *Equipment* beim Kuchen als `welt`, *Relevanz fuer kognitive Prozesse* als `nutzer` — die Grenze ist weich und wird im Betrieb sichtbar; eine Suche je Turn ohne Nachlesen der Seite; keine Kritikalitaet der Luecke (Pipeline §4.6); ob der Verfasser den Antwortstoff auch nutzt statt zu fragen, misst der Betrieb ueber die naechsten Turns. **Betriebsmessung 29.08. abends, 20 Turns:** 31 offene Eigenschaften 0 `nutzer` / 17 `welt` / 14 `nachschlagen`, Antwortstoff in 17 von 20 Bloecken, kein Rueckfrage-Gegenstand auf `nutzer`; die Websuche traf 1 von 8 Mal — die Wikipedia-API sperrte, weil der Thinker 23-mal suchte (Fundliste). **Abhilfe 29.08. spaet:** der Thinker sucht einmal je Turn, und die Sachlage-Treffer sind seine erste Suche (`thinker.py::prior_research`, `novaberg-node-thinker.md` §4.4) — die Sperre teilt sich nicht mehr.

Turn 07:04 UTC: *„Das muss ganz schoen knallen bei einem Kollaps"* — die Sachlage las richtig (der Nutzer will eine Einschaetzung *erhalten*), Scheibe 3 machte die offene Eigenschaft »Energieentladung« zum Rueckfrage-Gegenstand, Nova fragte den Nutzer, wie das Knallen aussieht. Scheibe 3 war an Alltagsframes entworfen, wo nur der Nutzer die Luecke kennt. Jetzt traegt jede offene Eigenschaft ihren Wissenstraeger; nur `nutzer` ist Fragestoff, `welt` und `nachschlagen` sind Antwortstoff fuer den Verfasser, und fuer die erste `nachschlagen`-Eigenschaft laeuft eine Websuche. Ohne Traeger (alte Artefakte) gilt `nutzer`. Fundliste-Zeile vom 29.08. (»Der Rueckfrage-Gegenstand kennt die Rollen nicht«) damit umgesetzt; der zweite Teil der Zeile (der Verfasser-Stoff traegt nicht, wer den Satz gesagt hat) bleibt offen.

**Zusammenhang:** SACHLAGE-SCHEIBE-3-FRAGE-GEGENSTAND (liest jetzt den Traeger) · SACHLAGE-SCHEIBE-6-AUFLOESER (nimmt gedeckte aus `offen`, bevor der Traeger zaehlt) · Pipeline §4.6 (Lueckenbehandlung), §4.9 (Werkzeug-Aufruf) · `thinker` (dieselbe Suche, dort mit Nachlesen).


## Feature: SACHLAGE-SCHEIBE-9-SPRECHER — der Sprecher: der Gedanke des Nutzers bleibt seiner (29.08.2026)

**Kategorie:** [ANT] ANTWORTPFAD

**Zustand:** ✅ **umgesetzt am 29.08.2026, mittags** — Anlass, Entwurf, Bau und Messung in einem Zug. Beleg: `sprecher` im Sachlage-Call und in `_validate_artifact` (`_normalize_speakers`), `carry_speakers` in `_derive`, `speaker_lines` im `[SACHLAGE]`-Block, `prompts/default/verfasser.fremder_reiz.txt` neu gefasst (fuehrt statt zu verweisen), Kontext-Tab `↳nutzer`/`↳nova`; 15 Zeugen (`tests/test_sachlage_sprecher.py`), Suite 2571 gruen, Gegenprobe 1/2/2/2 und 1/1 rot wie vorhergesagt; Labor `labor/2026-08-29_sachlage_sprecher_zwei_arme.py` (zwei Arme 5 × 2: Zuschreibung an Person B 0/10 → 10/10, Echo 3/10 → 0/10; frische Artefakte 5 × `nutzer`; Fortfuehrungsfall nach Anheben der Ausgabetoken 3/3 mit `nova` am Rekordwert); Konzept `novaberg-thinking-lage_k.md` §4 Scheibe 9, Verfasser-Konzept §2.2/§2.2a, Pipeline-Konzept §11, Frames Stand, Rechenkette S14a. **Rest, als Rest benannt:** Die Ausgabetoken des Sachlage-Calls mussten von 768 auf 1024 (`config.py`) — eine fortgeschriebene Blase mit zwei Objekten und allen Feldern brauchte 706–709 Token, der dritte Laborlauf und der erste Betriebsturn brachen bei 768 ab; die Artefaktform waechst mit jeder Scheibe, und die naechste misst den Bedarf vorher. Der `[GEDAECHTNIS]`-Block kennt den Sprecher seiner Eintraege weiter nicht (`_format_kzg` verwirft `beobachter`; Fundliste 29.08.). Ob der Responder den Anschluss (*»Ja, das muss …«*) auch bei anderen Haltungen haelt, misst der Betrieb. **Die zweite Kontrolle** fand am latenten Objekt der Live-Blase 3 von 7 Sprecher geraten (2 vertauscht) — seither nur an akuten Objekten und ohne Zwang zum Raten; ein Sprecher ausserhalb des 6-Turn-Fensters bleibt unerkannt, und die `quellen` einer Gedaechtnis-Deckung gehen am latenten Objekt verloren (Scheibe 6, Fundliste). **Betriebsmessung 29.08. abends, 20 Turns:** 63 gedeckte Eigenschaften 31 `nutzer` / 11 `nova` / 21 ohne, Sprecherzeile in 18 von 20 Bloecken, der Stoff nennt den Nutzer an 7 von 8 Behauptungsturns.
