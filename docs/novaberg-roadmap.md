...
|---|---|---|
| 2026-09 | **novaberg-roadmap.md** ← diese Datei | 102 |
...
**Der Fund** (Fundliste 29.08., aus der Erkundung zu Scheibe 9): `memory_context.py::_format_kzg` verwarf `meta['beobachter']`, und die LZG-Resonanz zitierte jede Erinnerung woertlich ohne Sprecher — ein Nutzersatz las sich als Novas eigene Erinnerung. Der Bestand am selben Tag: `lzg_knoten.beobachter` **3029 `assistant` / 219 `user`**, KZG-Stichprobe 300: 276 / 24. Die Ursache lag nicht in `anker_retrieval` (das las die Spalte), sondern im Spreading: `_knoten_details_laden` lud sie nicht, und Anker wie Nachbarn verloren sie beim Umbau ins Erinnerungs-Dict.

**Der Bau:** `speaker_label(beobachter)` — `user` → *Nutzer*, `assistant` → *Nova*, alles andere → *unbekannt* mit Warnung (ein Eintrag ohne Sprecher ist ein Defekt seiner Quelle). Die KZG-Zeile lautet jetzt `[KZG] {themen} (Salienz: …, Sprecher: …): {inhalt}`; jede Resonanz-Erinnerung traegt hinter dem Zitat `Sprecher: …`; `_knoten_details_laden` liest `beobachter`, `spreading_lesen` reicht es von Anker und Nachbar bis ins Ergebnis. **Die zweite Kontrolle** (*wer liest die Erinnerungen noch?*) fand den zweiten Leser derselben Dicts, `gespraechsvektor.py::_resonanz_kontext_laden` — dort steht der Sprecher jetzt in der Klammer (*»direkt zum Thema; Sprecher: Nutzer; Themen: …«*). Die Worte sind die des Gespraechsvektors; die Anrede des Blocks (*»dir«*, *»Du fuehlst«*) bleibt Bestand gegen F-PROMPT-2, die Umstellung auf die Namen des Lesers ist die offene Entscheidung. 8 Zeugen (`tests/test_memory_context_speaker.py`: Kanon, KZG-Zeile, Resonanz, Spreading mit ersetzten Lesern, Detail-Lader lesend gegen den Bestand) + 2 im GV-Zeugen; Suite 2608 → **2618 gruen, 0 uebersprungen**; Gegenprobe **2 / 2 / 1 / 1** und GV **2** rot wie vorhergesagt; Linter 1227, Wand sauber. **Berichtigt am Fund:** Die Session-Zusammenfassung benennt die Sprecher seit dem 24.08. (`sprecher_bezeichnen`).

**Betrieb 22:50–22:52 UTC** (ein Wissenschaftsturn ueber die Alterung eines Magnetars, vor dem GV-Nachtrag): In den Prompt-Ausgaben des Turns **30 KZG-Zeilen mit Sprecher (18 Nutzer / 12 Nova)** und **9 Resonanz-Sprecherzeilen (6 / 3)**, 0 *unbekannt*, 0 Kanon-Warnungen; eine Erinnerung im Block: *»Der Nutzer fragt nach der Bestaendigkeit eines Magnetar-Magnetfeldes.« — Sprecher: Nutzer*. Nebenbei derselbe Turn fuer die Suchdisziplin: Der Thinker versuchte eine zweite Suche und wurde **am Budget abgewiesen** (*»Suchbudget des Turns verbraucht«*), Maschine 1 ×. **Messnotiz:** Die erste Zaehlung ueber ein Zeitfenster per `grep` fand 0 × *Nova* — Prompt-Ausgaben im Log tragen keinen Zeitstempel und fallen aus jedem Zeitfenster; gezaehlt wird im Zeilenfenster des Turns.

## 29.08.2026, 22:44 UTC — Die Suchdisziplin des Thinkers: eine Suche je Turn, die Sachlage-Treffer zuerst

**Der Fund aus der Kettenmessung (19:48 UTC):** Der Thinker rief die Suchmaschine in 20 Turns **23-mal** — bis zu dreimal je Turn, deutsch und englisch, mit Umformulierungen nach leeren Treffern — und lud jeden ersten Treffer nach; die Wikipedia-API, seit dem Vormittag die einzige antwortende Engine, sperrte zweimal fuer 180 s, und die eine Suche je Turn der Sachlage-Recherche (Scheibe 8) traf 1 von 8 Mal. Wer die Suche teilt, teilt die Sperre — und ein Anbieter mit Schluessel zaehlt Anfragen, nicht Treffer.

**Der Bau:** Das Suchbudget des Turns lebt im Per-Turn-Cache (`ThinkerToolCache(web_search_budget=…)`, `web_search_allowed()` / `web_search_spent()`; `config.py::THINKER_WEBSUCHE_MAX_JE_TURN = 1`); ein leerer oder gescheiterter Aufruf zaehlt, `web_fetch` nicht. Ohne Budget bekommt das Modell eine Fuehrung statt einer Suche (F-PROMPT-1), und `thinker.rules.txt` fuehrt vorab: *»Eine Websuche je Pruefung: Formuliere sie so, dass sie die Kernbehauptung trifft. Weitere Seiten aus der Trefferliste holst du mit web_fetch(url).«* **Und die Sachlage-Recherche ist diese Suche, wenn sie Treffer hat:** `thinker.py::prior_research` liest `state["sachlage"]` (das erste akute Objekt mit `recherche`-Treffern), `think()` reicht sie an `create_tools(…, prior_hits)`, die erste `web_search` wird daraus bedient — Kopfzeile *»in diesem Turn bereits nachgeschlagen zu … — …«*, Auto-Fetch des ersten Treffers wie sonst — und verbraucht das Budget. Dieselbe Suche, zwei Leser. Jede Weiche steht im Log, bevor sie wirkt (F-LOG-3). 22 Zeugen (`tests/test_thinker_search_budget.py`: Budget, Maschine einmal je Turn, Sachlage-Treffer ohne Maschine, `prior_research`, Verdrahtung in `think()`), Suite 2586 → **2608 gruen, 0 uebersprungen**, Gegenprobe **4 / 1 / 1** rot wie vorhergesagt, Linter 1227 (unveraendert gegen HEAD), Wand sauber.

**Betrieb, drei Wissenschaftsturns 22:38–22:44 UTC** (`labor/2026-08-29_thinker_suchbudget_betriebszeuge.py`): Turn 1 (Entfernung des naechsten Magnetars — der Aufloeser deckte die Eigenschaft aus dem Gedaechtnis, keine Sachlage-Suche): **1** Maschinenaufruf, Budget danach 0. Turn 2 (Feldstaerke gegen die Erde, `welt`): **1** Maschinenaufruf. Turn 3 (Anzahl bekannter Magnetare, `nachschlagen`): Sachlage-Recherche **3 von 3** Treffer, der Thinker **aus der Sachlage bedient, 0** Maschinenaufrufe, Auto-Fetch `de.wikipedia.org/wiki/Magnetar`, Antwort korrigiert. Kein Turn mit mehr als einem Aufruf. **Nebenbefund** (Fundliste): In Turn 1 suchte der Thinker englisch (*»nearest known magnetar distance«*) gegen die deutschsprachige Wikipedia-Volltextsuche und lud eine Liste von Doppelsternen nach — die eine Suche muss die Sprache der Engine treffen. **Bestand nebenbei gezaehlt** (fuer den `[GEDAECHTNIS]`-Sprecher, naechster Auftrag): `lzg_knoten.beobachter` 3029 `assistant` / 219 `user`; KZG-Hashes 3536, Stichprobe 300: 276 / 24.

## 29.08.2026, 21:40 UTC — Der eigene Zug: die Rueckfrage sagt, wessen Ziel sie nennt

**Der Fund aus der Kettenmessung, berichtigt:** Der Rueckfrage-Gegenstand kommt im Wissensgespraech aus dem **staerksten** aktivierten Kurzziel — das ist die Gravitation und gewollt; nach einem Themenwechsel zieht zwei Turns lang das alte, weil das neue noch kein Kurzziel hat (Scheibe 2). Falsch war die Form: Die Zeile gab Novas Zug als Frage an den Nutzer zu seiner Sache aus, und der Verfasser verbog das Nutzerthema (*»Magnetar-Modell als Analogie fuer neuronale Felder«*). **Der Bau:** `question_target_origin` liefert Gegenstand und Herkunft (`blase` | `eigener_zug`), `question_target` bleibt wortgleich; die Rueckfrage-Zeile fuehrt bei eigenem Zug (*»das ist Novas eigenes Ziel, nicht die Sache von Person B — sie nennt es als das, was sie gerade zieht, und laesst ihm die Wahl, mitzugehen«*, F-PROMPT-1); der Verfasser reicht die Herkunft durch (`_question_target`). Die Gravitation bleibt unangetastet, die Haltung bleibt der Regler. 10 Zeugen, Suite 2571 → **2581 gruen**, Gegenprobe 2/2/1 rot (vorhergesagt 1/2/1), Linter unveraendert, Wand sauber. **Labor** (Blase Gamma-Oszillationen + Kurzziel Magnetar, 5 × 2, T = 0): Arm A verbog in **5/5** das Nutzerthema, Arm B nannte in **5/5** den Magnetar als Novas naechsten Schritt mit Wahl fuer den Nutzer. **Betrieb:** Herkunft erkannt (*»(Novas eigener Zug)«* im Log); die Haltung setzte in dem Turn keine Rueckfrage — die Fuehrung greift nur, wenn sie fragt. **Dazu (21:55 UTC) die Namen des Lesers:** Das Modell ist der Schauspieler, der Charakter der Auftrag — nirgends wird es als der Charakter angesprochen. Der `[SACHLAGE]`-Block der Scheiben 1–9 trug *»der Nutzer«*, *»Nova«* und *»beantworte es aus deinem Wissen«* in den Verfasser-Prompt; jetzt spricht er in den Namen seines Lesers (`sachlage_block(…, leser=verfasser|gv)`): Person A und Person B beim Verfasser, Nova und der Nutzer beim Gespraechsvektor; Bruecke und Rueckfrage-Zeile ebenso. 5 Zeugen, Suite **2586 gruen**, Gegenprobe 1/2. Bestand, der dieselbe Regel verletzt (Fundliste): die Aufzeichnungs-Bloecke (*»das habe ich nachgelesen«*, *»woran du dich erinnerst«*) und der `[GEDAECHTNIS]`-Block (*»Du fuehlst dazu«*, *»Sie ist dir eingefallen«*).

## 29.08.2026, 19:48 UTC — Die Betriebsmessung der Lage-Kette: 20 Turns

**Die Anordnung** (`labor/2026-08-29_kette_betriebsmessung.py`, 18:59–19:47 UTC): 20 Wissenschaftsturns ueber `/chat`, zwei Themenwechsel, Fragen und Behauptungen (drei unhaltbare), Fertig-Signal an der Turnzaehlung, Waermewaechter; je Turn Logfenster und Redis-Blase. **Die Kette:** Sachlage 19 × fortgeschrieben, `eval_count` median 444, max 912 (kein `length` bei 1024); Latenz bis zur Antwort median **99 s** (49–185); Aufloeser **16 Deckungen** aus dem Gedaechtnis; Plausibilitaet **3 Befunde an genau den drei unhaltbaren Behauptungen, 0 Fehlalarme**; Wiederaufnahme in 16 von 20 Turns. **Traeger:** 31 offene Eigenschaften, 0 `nutzer` / 17 `welt` / 14 `nachschlagen`; Antwortstoff in 17 von 20 Bloecken; der Rueckfrage-Gegenstand war nie eine Frage an den Nutzer, 17 × das staerkste aktivierte Kurzziel — die Gravitation, gewollt; in den ersten zwei Turns nach einem Themenwechsel ist das das alte, weil das neue noch kein Kurzziel hat (*»wie es mit Magnetar weitergeht«* bei den Gamma-Oszillationen). Der Fund ist die Form: Novas Zug steht als Frage an den Nutzer zu seiner Sache (Fundliste). **Sprecher:** 63 gedeckte Eigenschaften, 31 `nutzer` / 11 `nova` / 21 ohne; Sprecherzeile in 18 von 20 Bloecken; der Stoff nennt den Nutzer an **7 von 8 Behauptungsturns** (an Fragen gibt es nichts aufzunehmen). **Websuche:** 8 Suchen, 1 relevanter Treffer — die Wikipedia-API sperrte zweimal fuer 180 s, weil der **Thinker 23-mal suchte** (bis zu drei je Turn) und die Sachlage-Suche im selben Budget steht (Fundliste). **Form:** Frage am Ende in 12 von 19 Antworten, 1,21 Fragezeichen je Antwort. Nebenbefund am Zeugen: die Session-Liste ist gekappt, ein Fertig-Signal auf *N+1* reisst die Frist (Turn 13).
