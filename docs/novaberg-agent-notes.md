...
~~**Warum der Anlass bleibt, wahrscheinlich:** Dieselbe Vorprüfung trägt ältere Regeln, die Bedarfsaussagen als Auftrag lesen — *»Wir brauchen auch Erdbeeren«* und *»Ach, und Butter!«* als `add_content`, *»X brauchen wir nicht«* und *»brauch ich nicht mehr«* als Entfernen (`classify_notizen.task.txt`, 3 Stellen). Nicht gemessen, welche davon S2 trägt.~~ → **Gemessen und behoben am 19.09.2026** (`labor/2026-09-18_notizen_vorpruefung/`, `labor/2026-09-19_notizen_liste/`): Von den drei älteren Regeln trug **Regel A** (*»Wir brauchen auch Erdbeeren«* = `add_content`) es allein — das Modell las den Satzbau, nicht die laufende Liste. Sie allein zu streichen kippte die Ergänzung einer laufenden Liste; ersetzt durch eine **Ausnahme in der Vorprüfung** (*„Hat der User im Verlauf gerade etwas auf eine Liste setzen lassen, ist eine Bedarfsaussage eine Ergaenzung dieser Liste → add_content"*): Bedarfsaussagen ohne Liste 15 → 6 von 27 geschrieben, Listen 21 von 21. Seit `968c56b` ist eine Ablehnung der Klassifikation als Nicht-Auftrag gekennzeichnet (`Korrektur.kein_auftrag`); im Betrieb 19.09.2026: keine Notiz auf 3 von 3 Bedarfsaussagen, Nova bot an (`labor/2026-09-19_angebot_bedarf/`). **Offen:** 6 von 27 Bedarfsaussagen, die Telefonnummer als Aussage (Fundliste).
