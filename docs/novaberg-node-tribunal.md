...
**Stand:** 18. September 2026, 21:14 UTC (**gebaut:** eine Behauptung, die beide Korrekturrunden übersteht, bekommt einen Korrektursatz). Davor 16. September 2026, 16:51 UTC (**die zweite Entscheidung, nicht gebaut:** Eine Behauptung, die beide Korrekturrunden übersteht, wird nicht aus der Antwort entfernt — angehängt wird ein Korrektursatz, der Ausgang wird am Turn vermerkt, ein später gefundener Fall am Gedächtnisknoten nachgetragen — *Was nach drei Läufen offen bleibt*). Davor 15. September 2026, 21:26 UTC (**entschieden, nicht gebaut:** Die Speicherbehauptung hält dieser Riegel vor der Korrektur gegen den Bestand, und `abgeschlossen` folgt der Verifikation des Dienstes — *Was nach drei Läufen offen bleibt*). Davor 15. September 2026 (**die Speicherbehauptung** — eine dritte gerechnete Prüfung in der Auswertung: Meldet die Antwort eine Speicherung, ohne dass ein Dienst dieses Turns `abgeschlossen` zurückgab, geht sie in die Korrekturrunde; `evaluate` schreibt dazu je Durchlauf einen dauerhaften Eintrag). Davor 23. August 2026 (die drei System-Prompts haben einen Override ueber dem Default — dreistufige Prompt-Segregation). Davor: 22. August 2026 — die Zeitangabe wird gerechnet, nicht beurteilt, und seit heute in zwei Formen: Wochentag gegen Datum, Bestätigung gegen das Gemeldete
...
**Was nach drei Läufen offen bleibt:** Zwei der 24 Behauptungen überstanden in Lauf 3 beide Runden — das Modell hielt den beanstandeten Satz fest — und gingen mit der Antwort hinaus, gemeldet, nicht verhindert. Eine Wirkung, die nicht vom Modell abhängt, ist nicht gebaut. ~~Offen sind zwei Entscheidungen: eine solche Behauptung aus der Antwort zu entfernen, und eine wahre — die Wiederholung einer früheren Schreibung — vor der Korrektur gegen den Bestand zu halten~~ → **Die zweite ist am 15.09.2026 entschieden, nicht gebaut:** Eine Behauptung ohne abgeschlossenen Dienst hält dieser Riegel vor der Korrektur selbst gegen den Bestand — nicht das Thinker-Modell —, und ein Dienst, dessen eigene Verifikation der Schreibung fehlschlägt, meldet nicht `abgeschlossen`. ~~**Offen bleibt die erste:** eine solche Behauptung aus der Antwort zu entfernen~~ → **auch sie ist entschieden, am 16.09.2026, und nicht gebaut: Der Satz wird nicht entfernt, sondern ein Korrektursatz angehängt** — entfernen setzt voraus, dass die Behauptung als falsch belegt ist, und das leistet erst der Bestandsabgleich. Dazu wird der Ausgang am Turn vermerkt, und ein später gefundener Fall wird am Gedächtnisknoten nachgetragen (`novaberg-thinking-lage_k.md`, Scheibe 12, *Offen für den Bau*). Die Läufe korrigieren einzelne Antworten mit schmalem Lagebild und neutralen Voten; der Betrieb ist damit nicht gemessen.
...
## Der Korrektursatz — gebaut am 18.09.2026

Übersteht eine Speicherbehauptung beide Korrekturrunden, hängt die Auswertung an die Antwort: *»(Korrektur: Dafür wurde kein Eintrag angelegt — gespeichert ist davon nichts.)«* (`utils/storage_claims.py::append_correction`, `CORRECTION_NOTE`). Genau einmal, nur nach der letzten Runde, über den Helfer der Antwortspur (`antwort_setzen`); der behauptende Satz bleibt stehen. Der Satz selbst ist für die Erkennung keine Speicherbehauptung. Offen: der Bestandsabgleich vor der Korrektur und der Nachtrag am Gedächtnisknoten.
