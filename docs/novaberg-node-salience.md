...
**Stand:** 9. September 2026, 18:45 UTC (**der HumanGraph-Boost fuellt auf statt zu addieren** — `neu = basis + term x (1 - basis)`; die Bewertung des Modells wird verschoben statt ausgeloescht, gerechnet 5 statt 307 exakte Einsen. `GRAVITATIONSTERM-OHNE-OBERGRENZE` behoben). Davor 1. September 2026, 16:50 UTC (`zielsog_roh` als Eingang, §6: Novas eigener Zielsog zieht die Salienz ihrer Aeusserung, statt im `max()` zu konkurrieren). Davor 23. August 2026 (`salienz.rules` und `salienz_segment.rules` haben einen Override; er lud bis zum 23.08.2026 nicht, weil nach Connector statt nach Modell geschluesselt wurde). Davor: 27. Juli 2026, Chat 112 (Salienz-Formel, Rollen-Switch am System-Prompt)
...
| `gravitationsterm` | float | Im HumanGraph weiterhin Salienz-Boost. Für `character`/`agent` seit Chat 112 **einer der Antriebe des Eigen-Pfads**, kein Zuschlag mehr — sonst zählte er zweimal. ~~**⚠ Ohne Obergrenze, gemessen am 09.09.2026:**~~ → **normiert am selben Tag.** Er war die Summe über alle aktivierten Ziele und erreichte **5,9955**; im Boost trugen **277 von 498** Zeilen einen Term ≥ 1,0 (die Salienz stand danach immer auf 1,0, während die Bewertung des Modells im Mittel bei 0,327 lag), im Eigen-Pfad gewann er das `max()` in 214 von 721 Fällen. **Seither liefert `gravitationsterm_berechnen` einen Wert in [0, 1]** (`sin^0.5`, Cap 6,0 aus der gemessenen Spanne), und der HumanGraph-Boost **füllt auf statt zu addieren**: `neu = basis + term × (1 − basis)`. `GRAVITATIONSTERM-OHNE-OBERGRENZE` |
