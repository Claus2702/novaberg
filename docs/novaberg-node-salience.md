...
| `gravitationsterm` | float | Im HumanGraph weiterhin Salienz-Boost. Für `character`/`agent` seit Chat 112 **einer der Antriebe des Eigen-Pfads**, kein Zuschlag mehr — sonst zählte er zweimal. **⚠ Ohne Obergrenze, gemessen am 09.09.2026:** Er ist die Summe über alle aktivierten Ziele und erreicht **5,995**. In beiden Verwendungen bricht er die Skala — im Boost tragen **277 von 498** Zeilen einen Term ≥ 1,0 (die Salienz ist danach immer 1,0, während die Bewertung des Modells im Mittel bei 0,327 liegt), im Eigen-Pfad gewinnt er das `max()` in 214 von 426 Fällen. `GRAVITATIONSTERM-OHNE-OBERGRENZE` |
