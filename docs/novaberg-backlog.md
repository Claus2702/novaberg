...
| REFAC-SCHEMA-MIGRIEREN-FAILMODE | `schema_migrieren()` verschluckt Fehler mit `logger.warning(...)` und macht weiter. Verletzt „fail loud" (Handbuch §1). In P1 wird die `pipeline_log`-Migration als bewusste Abweichung mit `raise` gebaut — Bestand zieht im Refactor nach. | ⬜ Prio mittel |
