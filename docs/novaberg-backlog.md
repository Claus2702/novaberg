...
|---|---|
| `GLIEDERUNG-NUR-MARKDOWN` | **`DATEIEN_INDEX_ENDUNGEN` lässt `.md`, `.txt`, `.rst`, `.org` und `.adoc` zu; `_ERKENNER` kennt allein Markdown.** In reStructuredText steht die Überschrift über einer Unterstreichung, in Org-Mode beginnt sie mit einem Stern, in AsciiDoc mit einem Gleichheitszeichen — keine davon trägt ein Rautenzeichen. Seit dem 20.08.2026 wirft eine solche Datei `FormatOhneErkennerError` statt still eine leere Karte zu liefern; **der Riegel ersetzt die Erkenner nicht**, er macht ihr Fehlen sichtbar. Heute unbetreten: alle 174 indizierten Dateien sind Markdown. **Zu bauen sind drei zeilenbasierte Erkenner** — die Syntax ist in wenigen Zeilen zu fassen, und für keines der drei Formate lohnt eine Abhängigkeit. `.txt` bleibt bewusst ohne Erkenner: Dort ist die leere Karte die richtige Aussage, und das muss als Registrierung dastehen statt als Lücke. |
