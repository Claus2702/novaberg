...
**Stand:** 19. September 2026, 13:12 UTC (das Angebot trägt die Dienste seiner Sache nach Nähe, `e451226`). Davor 18. September 2026, 21:14 UTC (der Dispatcher hält Angebote fest, vermerkt ein Nein am Objekt und den Ausgang am Turn). Davor 9. September 2026, 20:43 UTC (**der Turn-Abschluss haengt nicht mehr an den `pending_writes`, und der Knoten belegt seinen Lauf.** Bis heute stand in Schritt 1 ein `return state`, das `_session_turn_schreiben` und `_turn_roh_schreiben` uebersprang — zwei Schritte, die mit den Writes nichts zu tun haben; **15 von 227 Turns mit erzeugter Antwort** verloren so ihre `turn_roh`-Zeile (6,6 %, 14 davon nach einem gescheiterten Salienz-JSON). Dazu eine Eingangszeile mit den vier Groessen, an denen die Schreibpfade entscheiden — der Knoten schrieb zuvor **ausschliesslich bei Erfolg** und war damit an genau der Stelle unbeobachtbar, deren Ausbleiben er protokollieren soll. `TURN-ROH-FEHLT-BEI-ERZEUGTER-ANTWORT`. Davor 5. September 2026, 10:20 UTC (**die Verwendungszeile sagt jetzt, warum nichts verstaerkt wurde.** Der erste Betriebsbeleg war ein Nullbefund, und **sechs Ausgaenge sahen in der Zeile gleich aus** — fuenf davon Ausfaelle. Sie traegt seither `naehen_alle` (jede geprueften Naehe, auch die verworfene), `knappster_verworfener`, `schwelle`, `deckel` und `ausgang`. **Betriebsbeleg vollstaendig** `[gemessen 05.09.2026]`: 1 von 3, dann **3 von 3** und **3 von 3** verstaerkt — und ein Kandidat lag bei **0,5483 gegen die Schwelle 0,55**, also 0,0017 darunter. Die zwei stummen Zeilen davor sind einmalig nachgerechnet: knappste verworfene **0,2935** und **0,4334**, beide unter dem Maximum der Zufallsverteilung (0,5305) — **der Nullbefund war richtig, die Schwelle steht nicht zu hoch**.). Davor 4. September 2026, 13:00 UTC (**Verstaerkung setzt Verwendung voraus — gebaut am 04.09.2026.** `memory/usage_reinforcement.py`, gerufen vom Dispatcher nach `_turn_roh_schreiben`. Schwelle **0,55** aus 75.975 Paaren abgeleitet (P99,9 = 0,5412), Deckel 3. `[gemessen]` gegen den Bestand: **13 von 15 Turns** verstaerken, **24 von 30** nahen Kandidaten genommen, **0 von 45 fremden**. Die Segmentregel ist **vor dem Bau widerlegt** (25 von 25 Antworten ergeben ein Segment, Segmentierung 1,53 s im Turn). `tests/test_usage_reinforcement.py` (16), Suite **2966**. **Betriebsbeleg steht aus.** Der Knoten traegt dafuer `_verwendung_verstaerken`). Davor 4. September 2026 (**der Dispatcher war der vorgesehene Ort der Knoten-Verstaerkung und tat es nicht** — `novaberg-memory-synapsen_k.md` §7.1a: Verstaerkt wird, was Nova in der Antwort hergenommen hat, und zwar hier, weil der Responder formuliert und nicht persistiert. Gemessen: `knoten_verstaerken` **0 Aufrufe**, `lzg_resonanz` **0-mal gelesen**. Beide Eingaenge liegen bereit — `state["antwort_inhalt"]` und `state["lzg_resonanz"]["erinnerungen"]`. Beleg und Kennungen: `novaberg-memory-synapsen_k.md` §7.1a, `KZG-EINTRAG-BLEIBT-NACH-PROMOTION`, `KZG-THEMA-VERSTAERKT-NACHBARN`, `VERSTAERKUNG-OHNE-VERWENDUNG`.). Davor 11. Juli 2026, Chat 104 (Turn-Rohdaten-Schreiber ergänzt)
...
1. **Eingangszeile schreiben** (`schritt='eingang'`, seit 09.09.2026) — Zahl der Writes, ob `response`, `external`, `internal` und das Paar vorliegen. Sie steht **vor** jeder Verzweigung, weil der Knoten zuvor nur bei Erfolg schrieb und *nicht gelaufen* von *erfolglos gelaufen* nicht zu trennen war
2. `pending_writes` aus State lesen — wenn leer, **Durchlauf ohne Verteilung**: Drive-Schnappschuss, Session-Turn und Turn-Rohdaten laufen trotzdem, nur die Verstaerkung faellt aus (sie setzt voraus, dass Nova etwas hergenommen hat). ~~Durchlauf ohne Aktion~~ — der frueher hier stehende Ruecksprung kostete **15 von 227** Turns ihre `turn_roh`-Zeile
3. Nach `ziel` gruppieren (z.B. alle `"kzg"`-Writes zusammen)
4. Pro Ziel: Zuständigen Handler bestimmen
...
5. Handler aufrufen, Rückgabe: Anzahl ausgeführter Operationen pro Handler — **Ausnahme KZG:** `dispatch_kzg()` gibt ein Dict zurück, nicht nur eine Zahl (§3.2)
6. **DelegationsAgent prüfen** (unabhängig von pending_writes, siehe §3.4)
7. `pending_writes` im State leeren
8. **Session-Turn und Turn-Rohdaten schreiben** — jeder Ausstieg von `_turn_roh_schreiben` hinterlaesst seinen Grund als eigene Zeile (`schritt='turn_roh_uebersprungen'`): `paar_unvollstaendig`, `personality_fehlt`, `response_leer` — und `kein_antwortpfad` fuer den HumanGraph, der konstruktiv keine Antwort hat und sonst bei **jedem** Turn einen Ausfall meldete
...
## Seit dem 18.09.2026 — Angebot, Ablehnung, Ausgang

- **`_angebot_merken`** (Scheibe 12 E1/E3): Enthält die fertige Antwort ein Angebot (`find_offers`), liegt es ~~mit den akuten Objekten des Turns~~ → **seit 18.09.2026 mit der Sache, der es gilt** (`utils/offers.py::offer_binding`: die Sache, die der Verfasser anbieten ließ — `state["angebot_kandidat"]`; sonst die einzige akute Sache am Zettel von Timeline oder Notizen; sonst ohne Gegenstand, dann wird ein *»Gerne«* nicht von selbst zum Auftrag; die Dienste einer Sache in der Reihenfolge ihrer Nähe, `_writing_services`, weil der Router dem ersten zustellt), ihren Eigenschaften und Diensten als offener Punkt in Redis (`angebot:{user}:{figur}`, 15 min); sonst wird ein alter Punkt entfernt. Lehnt der Mensch ein offenes Angebot blank ab (*»Nein danke«*), bekommt jede Sache `Speichern = abgelehnt` im Eigenschaftsgedächtnis (`record_declined`).
- **`_ausgang_vermerken`** (Scheibe 12 A, Punkt 2): In beiden Pfaden des Dispatchers wird der Turn des Menschen mit dem Ausgang des Dienstes markiert (`session_turn_mark_action`).
