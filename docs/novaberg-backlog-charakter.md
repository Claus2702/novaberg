...
**Stand:** 15. September 2026 (`SYK-B3-VORZEICHENREGEL` neu bewertet: falsche Erfolgsmeldungen am Bestand gemessen, 24 von 928 Antworten; gerechnet wird der Fall seither in der Auswertung des Tribunals, die Regel ist nicht zurückgeholt). Davor 11. September 2026, 07:00 UTC (**die Warteschlange erreicht die gelesenen Traeger** — die Kandidatensuche sortiert zuerst nach Frische; 7 von 9 neuen Profilen gingen an Traeger der laufenden Reihe, vorher 0 von 20). Davor 7. September 2026, 14:19 UTC (**der Leser ist im Betrieb belegt**). Davor 7. September 2026 (**die Faszination hat einen Leser** — `FASZINATION-PRAEGUNG` nachgezogen; zwei Aussagen des Eintrags waren falsch, und die zweite Kontrolle hat die eigene Eichung des neutralen Punkts widerlegt). Davor 6. September 2026, 21:00 UTC (**die Faszination rechnet zum ersten Mal im Turn** — der Engpass war die Laengenschwelle, die 0 von 95 gelesenen Traegern als Kandidat offen hielt; `FASZINATION-PRAEGUNG` nachgezogen, die Sperre des Lesers ist hinfaellig). Davor 6. September 2026, 19:45 UTC (`ZITAT-ENTWERTEN` **abgeschlossen** — der Betriebsbeleg steht: 4 Belege im Zyklus 19:42 entwertet, im gespeicherten Kern ohne Anfuehrungszeichen). Davor 6. September 2026, 19:05 UTC (`ZITAT-ENTWERTEN` **gebaut und gemessen** — 32 von 282 Zitaten an 20 gepinnten Kern-Laeufen entwertet, danach 0 ohne Fundstelle; der Betriebsbeleg steht aus). Davor 6. September 2026, 18:05 UTC (`ZITAT-ENTWERTEN` neu — der Prompt-Weg ist auch fuer die Zitattreue ausgeschieden, gepinnt 18 % → 16 %; was bleibt, ist der Eingriff in den gespeicherten Text). Davor 6. September 2026, 14:35 UTC (`DECKUNG-BRAUCHT-EINEN-ANDEREN-ORT` **am Tag seiner Anlage geschlossen** — die Pruefung sitzt an der Ausgabe und schlaegt an 3 von 20 echten Profilen an). Davor 6. September 2026, 13:40 UTC (`DECKUNG-BRAUCHT-EINEN-ANDEREN-ORT` neu — der Prompt-Weg gegen die Verallgemeinerung ist gemessen und ausgeschlossen, vier Fassungen bei n = 20 ohne Wirkung ueber die Streuung). Davor 5. September 2026, 20:28 UTC (`EI-KANON-FEHLT` neu bewertet — der Ausloeser *Modellwechsel* ist eingetreten, der geforderte Helfer ist gebaut, und der Rest des Eintrags haengt jetzt an einer fehlenden **Wertemenge** statt an einem fehlenden Riegel). Davor 5. September 2026, 14:53 UTC (**die Reihe des Bestandslaufs hat einen Leser** — `tools/fascination_series.py`; dabei fiel `ohne_strang` als stummes Feld der Protokollzeile auf und ist berichtigt). Davor 5. September 2026, 14:19 UTC (**der Laengenfilter ist gemessen und nicht gebaut** — die Eroeffnungsformel traegt, wo die Themen es nicht tun; offen ist die vorgelagerte Frage, ob vorab gefiltert werden soll). Davor 5. September 2026 (**§10 der Faszination ist vollstaendig gebaut** — Anker, sechs Turn-Modulatoren, Verfall je Dimension, Zusammenfuehrung, dazu **§10.3a neu**: der Strangzug als Lage des Traegers zu einer Praegung. Erzeuger im Turn und Bestandslauf im Tageslauf. **Sie rechnet und wirkt nicht** — kein Leser, und kaum Eingabe: nur 7 von 36 gelesenen Knoten passieren den Laengenfilter). Davor 4. September 2026 (**`bindung_roh` blockiert** — die Verstaerkung haengt am falschen Ereignis, drei Kennungen). Davor 3. September 2026, 21:30 UTC (**Vorbedingung 4/5 zur Haelfte erfuellt** — die Qualitaetsseite der abstrakten Schicht ist gebaut und im Bestand (25 Traeger, 150 Kanten); die Werteseite traegt keine Zeile. Der Hauptbefund der Messung ist negativ: die Dominanz kollabiert auf `komplexitaet`). Davor 3. September 2026, 20:35 UTC (`FASZINATION-PRAEGUNG` nachgezogen — die fehlenden negativen Faeden sind **kein offener Punkt**, sondern der Erwartungswert einer zu kleinen Stichprobe; was hilft, ist Laufzeit). Davor 3. September 2026, 19:30 UTC (**Scheibe 5 und 6**, Praegungszug und Einfaerbung; offen bleiben ihre Verbraucher, und die haengen an der abstrakten Schicht). Davor 3. September 2026, 18:57 UTC (Scheibe 5, der Praegungszug). Davor 2. September 2026, 19:40 UTC (Scheibe 3a/b/c und 4 gebaut und im Betrieb belegt; die Faltung ohne Aufrufer ist behoben).
...
## Block 06.09.2026, abends — aus der Setzung zum Zitat

Ein Eintrag. Er entsteht daraus, dass **der Prompt-Weg auch fuer die Zitattreue gemessen
ausgeschieden ist** — fuenf Fassungen an einem Tag, keine traegt.

| Kennung | Was offen ist | Band |
|---|---|---|
| `ZITAT-ENTWERTEN` | **Ein unbelegtes Zitat im gespeicherten Profil soll seine Anfuehrungszeichen verlieren.** Setzung des Eigentuemers (`F-ZITAT-1`): *Das Modell darf nichts zitieren, was nicht wirklich gesagt wurde.* Die `ZITATREGEL` in den Prompts fuehrt das aus und **wirkt nicht**: gepinnt gemessen **18 % → 16 %** unbelegte Zitate bei n = 20 je Fassung (ungepinnt hatte sie 24 % → 10 % gezeigt — ein Ziel, das der Betrieb nicht benutzt). Nach `F-PROMPT-1` gilt damit: *Wo ein Verhalten verlaesslich sein muss, wird es in der Struktur erzwungen und nicht im Text.* **Der Weg:** `deckung_beanstanden` erkennt die Stelle bereits; sie zu **entwerten** hiesse, die Anfuehrungszeichen fallen zu lassen und den Satz stehen — aus dem angeblichen Zitat wird die Deutung, die es ist. **Zwei Dinge sind vorher zu entscheiden:** ob der erzeugte Text ueberhaupt angefasst wird (ein Eingriff, keine Implementierung), und wie mit der Falsch-Positiv-Rate umgegangen wird — die Normalisierung faengt Formatierung ab, aber eine Paraphrase bleibt ununterscheidbar von einer Erfindung. **Was fertig waere:** Kein gespeichertes Profil traegt Anfuehrungszeichen um Text, der im Material nicht vorkommt — nachgemessen ueber mindestens 20 Laeufe **gegen das gepinnte Ziel**. | [CHA] ✅ **umgesetzt am 06.09.2026 mit benanntem Rest.** `zitate_entwerten` sitzt in `_llm_call` hinter der Meldung und nimmt einem Beleg **ohne Fundstelle** die Anfuehrungszeichen; der Satz bleibt. **Die beiden vorher zu entscheidenden Fragen sind beantwortet:** Der Eigentuemer hat den Eingriff in den erzeugten Text freigegeben, und die Falsch-Positiv-Frage ist durch den Zuschnitt entschaerft — entwertet wird **nur** die Klasse *ohne Fundstelle*, der Einzelbeleg behaelt seine Zeichen, weil sein Wortlaut im Material steht. `[gemessen 06.09.2026]` an **20 Kern-Destillationen gegen das gepinnte Ziel**: **32 von 282 Zitaten entwertet** in 14 von 20 Laeufen, danach **0** ohne Fundstelle; kein gedecktes Zitat verloren (250 = 282 − 32) und 64 Zeichen angefasst, genau zwei je Beleg. 18 Zeugen, Suite **3220**, harte Wand 0. Werkzeuge: `labor/2026-09-06_entwertung_wirkung.py`, `labor/2026-09-06_entwertung_ergebnis.md`. **Das Schliesskriterium ist am 06.09.2026, 19:42 UTC im Betrieb erfuellt.** Ein Messturn setzte `hash_dirty`, der Heartbeat destillierte, und im **gespeicherten** Kern stehen die vier gemeldeten Belege ohne Anfuehrungszeichen, die uebrigen 20 Zitate mit — 4 von 24 ungedeckt, 17 %. Damit ist auch `19_WERKZEUGE/riegel-im-echten-pfad.md` erfuellt. **Und ein zweiter Rest, von der zweiten Kontrolle gefunden:** Die Reichweite der Entwertung ist die von `ZITAT` und endet bei **60 Zeichen** — an den 70 gespeicherten Profiltexten liegen **117 von 125** Anfuehrungspaaren darin, die uebrigen **8 sind ganze zitierte Saetze im Kern** und bleiben unberuehrt. Ob die Grenze dort richtig liegt, ist eine Entscheidung (Fundliste 06.09.). Der Defekt daneben ist `PROFIL-VERALLGEMEINERT-EINZELBELEG` und bleibt offen |

---

## Block 06.09.2026 — aus der Deckungsmessung am Profil-Prompt

Ein Eintrag. Er entsteht daraus, dass **der naheliegende Weg gemessen und ausgeschlossen** ist: Die Verallgemeinerung im Profil laesst sich ueber den Prompt-Wortlaut nicht daempfen.

| Kennung | Was offen ist | Band |
|---|---|---|
| `DECKUNG-BRAUCHT-EINEN-ANDEREN-ORT` | **Die Verallgemeinerung im Charakter-Profil braucht einen Ort ausserhalb des Prompt-Wortlauts, und welcher es ist, ist eine Absichtsfrage.** Der Befund steht als `PROFIL-VERALLGEMEINERT-EINZELBELEG`: Das Beziehungsprofil nennt eine Anrede *durchgehend*, die in **1 von 20** Begegnungen des Prompt-Materials steht (und in 4 von 1662 Wissensdateien). `[gemessen]` 06.09.2026 am Fernmodell, vier Prompt-Fassungen gegen dieselben 20 Begegnungen: das Mass benennen, das Zaehlen verlangen, an den Ausschnitt binden, das Wort *durchgehend* aus dem Satz zur Pronomenwahl nehmen. **Bei n = 20 bewegt keine die Zahl ueber ihre Streuung** — Nulllinie **8 bis 10 von 20 Laeufen** mit Dauerwort, **11 Dauerwoerter** in beiden Runden; die n=5-Vorzeichen waren Rauschen (`labor/2026-09-06_deckung_ergebnis.md`). **Zwei Wege bleiben, und sie schliessen einander nicht aus:** die **Ausgabe pruefen** — ein Dauerwort, dessen Bezugswort im Material selten vorkommt, wird beanstandet, was eine Zaehlung im Erzeuger und eine Entscheidung ueber die Folge braucht (verwerfen, neu anfordern, kennzeichnen) — oder das **Material aendern**, denn 20 Begegnungen sind der Ausschnitt, ueber den verallgemeinert wird. **Was fertig waere:** Der Anteil der Laeufe mit einem ungedeckten Dauerwort liegt belegbar unter der heutigen Nulllinie, gemessen mit **mindestens 20 Laeufen je Fassung**. | [CHA] ✅ **abgeschlossen am 06.09.2026** — **der Eigentuemer hat den Ort entschieden: die Ausgabe wird geprueft.** `deckung_beanstanden` haelt jeden woertlichen Beleg des Profils gegen den Prompt und meldet zwei Klassen — *Beleg ohne Fundstelle* (0 Vorkommen) und *Einzelbeleg als Dauerzug* (1 Vorkommen im selben Satz wie ein Dauerwort). Gemeldet, nicht verworfen. `[gemessen]` 20 echte Beziehungsprofile: **3 Laeufe mit Beanstandung**, 3 von 50 Zitaten, alle aus der zweiten Klasse; ein Vorlauf 4 von 20. 10 Zeugen, Gegenprobe 5 rot, Suite 3191. **Zwei Grenzen stehen am Defekt:** ein Dauerwort ohne woertlichen Beleg ist nicht pruefbar, und der Anschlag im vollen Betriebspfad ist noch nicht belegt. Werkzeug: `labor/2026-09-06_beanstandung_messung.py`. Angelegt und geschlossen am selben Tag |
...
| Kennung | Was offen ist | Band |
|---|---|---|
| `FALTUNG-OHNE-PERIODISCHEN-LAUF` | **`ausschlag_aktuell` altert in der Spalte nicht mit.** Seit dem 01.09.2026 fuehrt `ausschlag_aktuell_nachfuehren` den Wert nach, **wenn eine Beruehrung entsteht** — der Verfall **zwischen** zwei Beruehrungen erreicht die Spalte nicht. Ein Faden, den seit Wochen niemand angesprochen hat, steht darin so hoch wie am Tag seiner letzten Auffrischung. `[gemessen]` 01.09.2026, 15:05 UTC: Faden 353 traegt eine Beruehrung von 14:00 und steht weiter auf `ausschlag_absolut`, weil er nach dem Bau des Aufrufers nicht erneut getroffen wurde; 327 und 354 wurden getroffen und stehen gefaltet. **Zwei Wege, und sie schliessen einander nicht aus:** ein periodischer Lauf ueber den Bestand (Pixie), oder die Faltung beim Lesen — der Leser existiert noch nicht, weil Straenge und Praegungszug nicht gebaut sind. **Kein Datenverlust:** Die Rechnung ist von Grund auf wiederholbar (Wertekonvention Regel 3 und 4), ein Nachlauf holt jeden Wert. **Was fertig waere:** Kein Faden traegt einen `ausschlag_aktuell`, der aelter ist als ein Tag — nachgewiesen an einem Bestand mit Faeden verschiedenen Alters. | [CHA] ✅ **abgeschlossen am 01.09.2026, 15:20 UTC** — `alle_faeden_nachfuehren` faltet den ganzen Bestand und laeuft als **vierter Schritt im Tageslauf** des `SynapsenDecayAgent`; kein neuer Platz im Heartbeat. **Kein `gefaltet_am` und damit kein Schemawechsel:** Die Zusicherung ist die Vollstaendigkeit — der Lauf meldet `gefaltet` **und** `gesamt`, und sind sie gleich, traegt kein Faden einen aelteren Wert als den Lauf. `[gemessen]` 15:19 UTC ueber den echten Bestand: **4 von 4**; Faden 328 mit null Beruehrungen bei Anteil 0,9951, Faden 354 mit drei bei 0,9986. 8 Zeugen, Gegenprobe 2/2. Angelegt am selben Tag, geschlossen am selben Tag |
...
| Kennung | Was offen ist | Band |
|---|---|---|
| `FASZINATION-PRAEGUNG` | **Faszination als Wert [0..1] auf einem Traeger, gebildet aus Bindung x Qualitaet x Praegung x Turn-Modulatoren** — Konzept liegt vor in `novaberg-thinking-faszination_k.md` (v0.7, sieben Fassungen, jede hat eine These der vorigen widerlegt). Der Wert ist **valenzblind**: Kriegsgeschichte und Gartenkraeuter sind derselbe Mechanismus mit unterschiedlicher Haerte. Traeger ist **nicht die Entitaet**, sondern ein Merkmalsprofil auf sechs gesetzten Qualitaetsdimensionen — der Zwilling erbt die Faszination, nicht die Zuwendung. **Mit enthalten ist die Praegungsschicht (Faeden und Straenge)** als dritte Charakterschicht neben dem Zuwendungs-Rad (relational) und den Werte-Knoten (normativ): Sie ist **thematisch** und heute nicht vorhanden. Ein Faden ist ein einschneidendes, embeddingbezogenes Ereignis; wiederkehrende Emotion-Ausgang-Sequenzen verdichten zu Straengen. Die Literatur nennt das Self-Defining Memory (Singer & Salovey 1993) — die Deckung wurde **nach** der Entwicklung gesucht und ist deshalb Konvergenz, kein Beleg. **Acht Vorbedingungen, sechs davon offen** — gemessen am 30.08.2026: (1) MS-Welle Block 2 ff. *in Arbeit* · (2) Synapsen P4, Knoten/Kanten/Spreading *blockiert durch (1)* · (3) `KZG-SALIENZ-NEUBAU` *queued — das Faden-Tor steht darauf, Vorbedingung und nicht Nachbar* · (4) **abstrakte Schicht** mit Qualitaets- und Werte-Knoten samt Typ-Diskriminator *offen; `lzg_knoten_haltung` traegt **null Zeilen**, `praemisse_knoten_id` wartet auf dasselbe Stueck* · (5) ~~`EMGRAV-SCHWELLE-TOT`~~ **erfuellt — behoben am 30.08.2026**, die Gravitationsschwelle lehnt wieder ab (0,71 Aktivierungen je Turn statt 2,00); ohne sie waeren Praegungen unsterblich geworden · (6) `charakter_rad_messung` mit stabiler Reihe *die Reihe laeuft seit dem 12.08.2026, 68 Erhebungen; die **zeitliche** Stabilitaet ist die offene Frage, nicht die Wiederholstreuung aus `RAD-STABILITAET-UNGEMESSEN`* · (7) ~~`PIXIE_AKTIV` steht auf `False`~~ **erfuellt — im laufenden Container steht `PIXIE_AKTIV=true`**; nur der Code-Default in `config.py:360` ist `false` · (8) ~~Haltungsraum bekommt einen Leser *offen*~~ **erfuellt — gemessen am 04.09.2026**: Responder und Verfasser lesen `state["haltung"]` beide, seit dem 12.08.2026 (`HALTUNG-OHNE-LESER` behoben). Der in §11 beschriebene Faszinations-Leser waere damit baubar; er wird **nicht** gebaut, solange die Faszination ueberall 0 ist — er wuerde den Umfangsterm loeschen. **Ohne den Typ-Diskriminator sickert die Valenz von der Werte- auf die Qualitaetsseite** und Kriegsgeschichte truege weniger Faszination als Kraeuter — der Fehler waere fest im Schema verbaut. | [CHA] 🟠 **teilweise gebaut** — angelegt am 30.08.2026, **Scheibe 1 gebaut am 31.08.2026**: Faeden entstehen im Betrieb (`praegung_faden`, Formkurve, Tor als Node). **Scheibe 2 gebaut am 01.09.2026**: scharfes Segment-Embedding, Zuordnung Reaktivierung → Faden ueber Naehe 0,62 (Nulllinie ueber 19.900 Knotenpaare gemessen), Faltung von `ausschlag_aktuell` gegen die gerechnete Tabelle des Konzepts (gebaut und bezeugt, **ohne Aufrufer**). **Nur der thematische Andockweg** — der strukturelle braucht die abstrakte Schicht (Vorbedingung 4). **Im Betrieb belegt am 01.09.2026**: 10 Torpruefungen, 4 Faeden, Ausschlag 0,26 bis 1,00, die Auffrischung laeuft mit je 2 Kandidaten. **Beruehrungen 4** (vierte Reihe, 01.09.2026, 14:00 UTC) — Naehen 0,682 · 0,684 · 0,739 · 0,682; der Reiz war **vorher ausgerechnet** statt geraten (Vorhersage 2/1/2, gemessen 1/1/2). Alle vier nahen Eintraege tragen `beobachter = assistant`. ~~**Die Faltung wirkt trotzdem nicht**~~ → **am 01.09.2026 behoben** (`FALTUNG-OHNE-AUFRUFER`), dazu der periodische Lauf als vierter Schritt des Tageslaufs. Zwei weitere Defekte dabei behoben: `PROMOTION-NUR-EIN-PAAR` und die Beschraenkung der Auffrischung auf LZG. **Scheibe 3 gebaut am 01.09.2026** — a) Zuordnung Faden → Strang ueber ein fortgeschriebenes Zentroid (`PRAEGUNG_STRANG_NAEHE` 0,62), Nachzug als fuenfter Schritt des Tageslaufs; b) das Sektor-Histogramm, weil ein Mittelwert die Ambivalenz loeschen wuerde; c) die Richtung aus vier Regeln, **nicht gespeichert** — ein Strang ist Bestand, das Charakter-Rad ist Zustand. **Scheibe 4 gebaut am 02.09.2026** — die Ladung aus `mittel(salienz)`, `mittel(|valenz|)` und `n/(n+4)`, mal Praesenz; die Formel loest die des Konzepts (Anlaesse, Spitze, Spanne) nach Vorgabe des Eigentuemers ab, und `EMOTION_VALENZ` gibt der Valenz 16 Zwischenstufen statt ±1. **Im Betrieb belegt am 02.09.2026:** zwei Straenge, Ladungen **0,66148** (n=4) und **0,51612** (n=1), beide Richtungen *Annaeherung* ueber verschiedene Regeln; der zweite Strang entstand **ungeplant in einem echten Turn** (Faden 1282, `meister`). Die Beweiskette steht in `novaberg-node-praegung.md` §6a. ~~**Offen bleiben der Praegungszug** (§10.3 — er hat seit heute beide Groessen, es fehlen `PRAEG_HUB` und ein Leser) **und der sektorabhaengige Verfall.**~~ → **Scheibe 5 gebaut am 03.09.2026**: der Praegungszug rechnet je Turn `1,0 + PRAEGUNG_ZUG_HUB · max_j(sim_j · gewicht_j · ladung_j)` und protokolliert als `praegung_zug`. Der Hub ist aus der Spanne **abgeleitet** (`F-NAHT-1`), nicht gesetzt; **Vorgabe des Eigentuemers**: Vermeidung 0, `unbestimmt` 0,5, Annaeherung 1,0. `tests/test_praegung_zug.py` (15 Zeugen). `[gemessen]` ueber alle fuenf Faeden als Reiz: **1,3087 bis 1,3487**, Kreuzprobe **1,0693**. **Scheibe 6 am selben Tag** — die **Einfaerbung**: dieselbe Faltung mit `t x sektor_faktor`, negative Sektoren 1,5, siebter Schritt des Tageslaufs; 11 Zeugen. `[gemessen]` **groesster Abstand 0,000000 bei 5 von 5** — kein Faden des Bestands ist negativ. **Offen bleiben die Verbraucher**: die Faszination selbst (§10.6) mit Merkmalszug, Bindung und den sechs Turn-Modulatoren — keiner davon gebaut, alle hinter dem Traeger und damit hinter der abstrakten Schicht (Vorbedingung 4, `lzg_knoten_haltung` **0 von 3317** Knoten, gezaehlt am 03.09.2026) — dazu Ziele, LZG-Erinnerungen und EI-Calc als Leser der Einfaerbung (§8). **Und eine Frage, die als offen aussah, ist geschlossen** (03.09.2026, 20:35 UTC): Dass die Schicht keine negativen Faeden traegt, ist **kein Befund** — Novas negatives Register existiert (35 von 2825 Knoten im echten Paar, 1,24 %), das Tor gibt es erst seit dem 31.08.2026, und bei 22 Gelegenheiten seither liegt der **Erwartungswert bei 0,27**. Fuer einen einzigen negativen Faden braucht es im Erwartungswert rund **80 Turns**, und er muss zusaetzlich beide Torschwellen reissen. **Was hier hilft, ist Laufzeit und kein Umbau** — vier Bauteile (Einfaerbung, bimodales Histogramm, Regel 4 der Richtung, das `unbestimmt`-Gewicht) warten auf dieselbe Eingabe und sind bis dahin bezeugt und ungemessen. **Und am 03.09.2026, 21:30 UTC kam die abstrakte Schicht dazu — die Vorbedingung, die alles Weitere blockierte, ist zur Haelfte erfuellt.** `abstrakt_knoten` traegt den Typ-Diskriminator `art` und die sechs gesetzten Qualitaeten, `traeger_qualitaet` die **vorzeichenlose** Kante; Erzeuger ist der achte Schritt des Tageslaufs (ein Modellaufruf je Traeger, gedeckelt auf 20), Leser ist `merkmalszug` (§10.1). **Sie liegen in einer eigenen Tabelle statt in `lzg_knoten`** — 24 Lesestellen in 13 Modulen, 22 davon nach Kriterien; der Umzug kostete nichts, weil `lzg_knoten_haltung` 0 Zeilen traegt und `praemisse_knoten_id` nie benutzt wurde. `[gemessen]` 25 von 29 Traegern profiliert, 150 Kanten, 6,5–8,1 s je Traeger. **Der Hauptbefund ist negativ und bleibt offen: 23 von 25 dominant `komplexitaet`**, vier der sechs Dimensionen an keinem Traeger die staerkste — gegen §6.2. Die Textlaenge ist als Ursache **ausgeschlossen** (vier kurze Proben); zwei Erklaerungen stehen noch, und der Zugriff, der sie trennt, ist benannt: dieselben Knoten maschinell **und** von Hand, Zeile fuer Zeile. **Zweite Kontrolle, 21:38 UTC — der Betrieb statt der Zeugen:** `profil_lauf` selbst gegen den Bestand gefahren (bis dahin nur gegen Mocks bezeugt) — **3 versucht, 3 profiliert, 0 gescheitert**, Buchfuehrung geht auf, Bestand **28 Traeger / 168 Kanten**. **Der Betriebsbeleg ueber den Tageslauf steht aus** — der laeuft taeglich gegen 19:58 UTC und lief zuletzt vor dem Bau. **Und `bindung_roh` (§10.2) ist am 04.09.2026 an der Eingabe gescheitert, nicht an der Rechnung.** `wiederkehr` verlangt *verschiedene Tage, an denen der Traeger einen Turn beruehrt hat*. Der Verlauf steht im `pipeline_log` (14.947 `reinforcement`-Zeilen ueber 34 Tage) — **aber er zaehlt Wiederholung statt Wiederkehr**: Ueber verschiedene Turns gezaehlt fallen die Traeger mit Wiederkehr >= 2 von **1.250 auf 106**, das Mittel von 2,64 auf **1,04**. Ursache sind drei Defekte vom selben Tag (`KZG-EINTRAG-BLEIBT-NACH-PROMOTION`, `KZG-THEMA-VERSTAERKT-NACHBARN`, `VERSTAERKUNG-OHNE-VERWENDUNG`) und die neue Regel `novaberg-memory-synapsen_k.md` §7.1a. **`bindung_roh` bleibt blockiert, bis die Verstaerkung an der Verwendung haengt** — und dann braucht es Laufzeit, weil die Zaehlung bei null beginnt. ~~**Was jetzt noch fehlt:** … Von neun Faktoren sind **zwei** gebaut.~~ → **Am 05.09.2026 ist §10 vollstaendig gebaut.** `bindung_roh` (§10.2, Saettigung statt Min-Max, `eigenimpuls` darf fehlen), die sechs Turn-Modulatoren (§10.5, jede Tabelle vollstaendig gegen ihren Kanon), der Verfall je Dimension (§10.4, `ungewissheit` ueber Beruehrungen, die uebrigen ueber die Zeit), die Zusammenfuehrung (§10.6) — und **§10.3a neu: der Strangzug**, die Lage des **Traegers** zu einer Praegung, die es im Konzept nicht gab. Dazu ein **Erzeuger im Turn** und ein **Bestandslauf** als neunter Schritt des Tageslaufs. 60 Zeugen, Suite 3061. **`bindung_roh` war nicht blockiert, sondern falsch gefragt:** Es zaehlt nicht die aufgeblaehte `haeufigkeit`, sondern verschiedene Turns aus der Bruecke — die Zahlen beginnen dort bei null, und was hilft, ist Laufzeit. **Entschieden am 05.09.2026 vom Eigentuemer:** Lesen zaehlt **nicht** als Beruehrung — Faszination ist die Buendelung dessen, was durch Salienz und Valenz schon gepraegt wurde. **Was jetzt fehlt:** die **Werteseite** der abstrakten Schicht (null Zeilen), Entitaeten und Themen als Traeger, der **Laengenfilter** (nur 7 von 36 gelesenen Knoten passieren ihn — der Rest sind Sprechakt-Vermerke; **am 05.09.2026 gemessen und nicht gebaut**: Die Themen trennen nicht — bestes haeufiges Thema 29,2 % bei 106 Vorkommen —, die **Eroeffnungsformel** schon: Frageform 0 von 115 lang, Einsichtsformel 56,7 % von 409. Der Zeichenschnitt kostet **177 Einsichten** unter 400 Zeichen. Dabei die vorgelagerte Frage aufgeworfen, **ob ueberhaupt vorab gefiltert werden soll**: 49 von 50 Traegern schlagen voll aus, **keine gemessene Null im Bestand**, und der Deckel von 20 Profilen je Lauf daempft ohnehin. Drei Wege benannt in `novaberg-memory-qualitaetsprofil.md` §4a/§4b, keiner entschieden) und ein **Leser**: Die Faszination rechnet und protokolliert, sie wirkt nicht. · **06.09.2026: der Erzeuger hat eine Spur, die ihn rufen laesst.** Er stand seit dem 03.09. als achter Schritt im Tageslauf und hat dort **nie einen Traeger profiliert** — zwei Laeufe, beide `0 von 20`, `SpurVerletzungError`, weil die `cpu`-Spur das Sprachmodell verriegelt und ein Profil einen Modellaufruf kostet `[gemessen 06.09.2026]`. Ausgezogen nach `agents/qualitaet_profil/` (taeglich, `llm`-Spur); im Betrieb **20 von 20** gegen vorher 0 von 20, Bestand **72 Traeger / 432 Kanten**, 328 Kandidaten offen. **Damit laeuft die Voraussetzung des Strangs zum ersten Mal von selbst** — ohne Profil kein Merkmalszug, ohne Merkmalszug keine Faszination  **06.09.2026, 21:00 UTC — die Faszination rechnet zum ersten Mal im Turn, und der Satz oben *„er wird nicht gebaut, solange die Faszination ueberall 0 ist“* ist damit ueberholt.** Der Engpass war weder der Leser noch die Auswahl: Von **95** je gelesenen Knoten standen **0** als Kandidat des Profil-Erzeugers offen — 18 passierten beide Filter, und alle 18 trugen laengst ein Profil. Die Warteschlange enthielt keinen Traeger, den der Lesepfad im Turn anbietet. **Die Laenge trennt die Klassen nicht:** Sachaussagen liegen bei **271** Zeichen im Mittel, Sprechakt-Vermerke bei **281**; die Schwelle 400 liess 8 Vermerke durch und 1 Sachaussage. `QUALITAET_LAENGE_MIN` steht seither auf **100**, die inhaltliche Ablehnung liegt beim Modell (Nullprofil wird geschrieben, der Traeger faellt danach aus der Auswahl). Warteschlange 0 → 55 → **35** nach einem Lauf, Bestand **92 Traeger / 552 Kanten** (davor 72/432), **0 von 20** neuen Profilen sind Nullprofile. `[gemessen 06.09.2026]` `werte {"6982": 0.5725}` statt `{}` — der erste Wert, den ein Turn je getragen hat. **Der Leser aus §11 ist damit baubar und seine Sperre hinfaellig**; offen bleiben zwei der drei gelesenen Traeger (zwei Laeufe) und `DESTILLAT-SUBJEKT-SCHABLONE` als unberuehrte Wurzel. · **07.09.2026: der Leser ist gebaut — und zwei Aussagen dieses Eintrags waren falsch.** (1) Der beschriebene Ersatz *„`wissbegier x Bindung` statt `wissbegier`"* zielte auf den **Umfangsterm**; den gibt es seit dem 08.08.2026 nicht mehr, die Zelle ist mit Begruendung gestrichen. `wissbegier` traegt `fragen 0.40` und `draengen 0.20`, und beide werden jetzt moduliert. (2) *„er wuerde den Umfangsterm loeschen"* setzt eine rohe Multiplikation voraus; gebaut ist ein **Abbildungsfaktor um 1,0** (`F-NAHT-1`), bei dem ein leerer Wert neutral wirkt statt zu loeschen. **Dritter Befund, neu:** Der Haltungsraum steht **sieben Knoten vor** dem Praegungsknoten — die volle Turn-Faszination erreicht keinen Leser der Haltung, und vorziehen geht nicht (das Faden-Tor braucht die Salienz). Verwendet wird die **Traegerseite**, Maximum ueber die gelesenen Knoten. **Und die zweite Kontrolle hat die eigene Setzung widerlegt:** neutraler Punkt 0,50, geeicht am Median der *Turn*-Faszination statt der Traegerseite — `[gemessen 07.09.2026]` ueber 25 echte Turns waeren **12 von 15** gedaempft worden. Auf **0,36** korrigiert (Median der richtigen Groesse), danach 6 daempfend / 9 hebend. Suite **3250**. **Im Betrieb belegt 07.09.2026, 14:19:25 UTC**: echter Turn, 3 gelesene Traeger / 1 mit Profil, Faszination 0,5176 → Faktor 1,0985; dasselbe Rad mit und ohne ergibt `fragen` 0,5837 → **0,6207** und **exakt 0** auf `umfang`, `naehe`, `waerme`. Zwei Anlaeufe davor brachen am Anbieter-Rate-Limit ab. **Offen bleibt die Frage, ob sich Novas Antworten dadurch aendern** (Praezedenzfall `MASSBLOCK-IM-BETRIEB-UNGEMESSEN`). · **11.09.2026: die Warteschlange erreicht die gelesenen Traeger.** `[gemessen ueber 363 Turns]` Die Faszination meldete in **74,1 %** der Faelle `werte: {}`; von 410 je gelesenen Traegern hatten **376 kein Profil**. Die Ursache ist dreigeteilt — von 42 Traegern einer Messreihe standen **22 als offene Kandidaten** in der Warteschlange (richtig eingereiht, nicht an der Reihe), 11 unter der Laengenschwelle, 9 unter der Wiederkehr. **Der Tageslauf faellt zuverlaessig** (20 je Tag, lueckenlos) — er reicht nur nicht: 1497 offene Kandidaten sind **75 Tage**. Seither entscheidet die **Frische** (`QUALITAET_FRISCHE_TAGE` = 7) als erster Schluessel vor der Gesamtlesespur. **Betriebsbeleg:** ein angestossener Lauf erzeugte 9 Profile, **7 davon fuer Traeger der laufenden Reihe (78 %)** — vor der Aenderung 0 von 20. **Nicht behoben:** Die Sortierung aendert die Reihenfolge, nicht die Rate; 14 der 60 gelesenen Traeger fallen weiter unter die Laengenschwelle, 14 unter die Wiederkehr. |
...
| Kennung | Was offen ist | Band |
|---|---|---|
| `KERNAUSWAHL-KRITERIUM-OFFEN` | **Welche Turns in die wachsende Auswahl des Kern-Hash kommen, ist nicht entschieden.** Am 25.08.2026 ist entschieden, dass die **Grundlage** fortgeschrieben wird und nicht der Text (`novaberg-pixie-character-hash.md` §3.1a) — der Kern bleibt eine frische Destillation, sein Material wird eine wachsende Auswahl rohen Wortlauts ueber die ganze Historie statt der 40 neuesten Turns. **Das Kriterium dieser Auswahl fehlt, und die drei naheliegenden fallen aus:** `gewicht_absolut` ist eine Eigenschaft der Langzeit-Knoten und existiert fuer Rohturns nicht · `salienz x zeitgewicht` (§4b) ordnet nach **Aktualitaet**, also nach genau der Groesse, die hier das Problem ist · *„die N neuesten"* ist der heutige Zustand. **Zweite offene Haelfte:** waechst die Auswahl unbegrenzt — dann waechst der Prompt mit — oder ist sie gedeckelt, dann entscheidet das Verdraengungskriterium dieselbe Frage noch einmal, nur spaeter und unsichtbarer. **Geschlossen, wenn** das Kriterium samt Deckelung im Konzept steht und begruendet ist, warum es nicht nach Aktualitaet ordnet. | [CHA] ungebaendert — ✅ **abgeschlossen am 26.08.2026** — angelegt am 25.08.2026. **Der Riegel galt einen Tag:** Solange dieser Eintrag offen ist, wird `KERNHASH-TRAEGT-KEINE-PERSON` nicht gebaut — ein Umbau ohne Kriterium ersetzt ein unbegruendetes durch ein anderes und misst hinterher denselben Wert. → ✅ **Geschlossen am 26.08.2026 — das Kriterium steht** (`novaberg-pixie-character-hash.md` §3.1a): **festes Zeichenbudget beim Vierfachen des heutigen, darin zeitlich geschichtet.** Nicht die ganze Historie — sie traegt heute 165 138 Zeichen auf der Seite der Figur und waechst weiter. **Drei Kandidaten sind durch Messung ausgeschieden:** `gewicht_absolut` ueber `verbindung` (erreichbar — 221 von 223 Turns, 159 mit LZG-Knoten — aber Korrelation **−0,716** gegen die Zeit, und im juengsten Zeitblock faellt die Streuung auf **0,27**: die Ankerstaerke ist ueberwiegend Reifung) · die Schichtung als **Haupthebel** (bei gleichem Zeichenbudget nur **+8,0 %** Wortschatz statt der zunaechst gemessenen +73,4 % — der Rest war mehr Text) · und die Kostenannahme gegen ein groesseres Budget (**261 s** fuer eine volle Destillation, gemessen, statt der behaupteten anderthalb Stunden). **Der Riegel auf `KERNHASH-TRAEGT-KEINE-PERSON` ist damit aufgehoben.** |
...
| Kennung | Was offen ist | Band |
|---|---|---|
| `PRUEFFIGUR-DISTANZ-090` | Der Zuwendungs-Riegel (Bauteil D) steht bei 0,25 und trennt im Bestand sauber — **aber der Bestand enthält in der Richtung *sie → Mensch* keine Figur zwischen `distanz` 0,60 und 1,00**, also das Band, in dem der Zug schwach wird. Eine angelegte Prüffigur bei 0,90 entscheidet, ob die Trennung hält. Kosten: eine Kennung, ein Rad, ein Lauf von `haltung_naehe_verteilung`. | [CHA] ungebändert — ⬜ **offen** — nachgesehen am 25.08.2026. Die verlangte Messung ist nicht angesetzt worden. |
...
**Zustand:** offen, unbelegt — nachgesehen am 25.08.2026. Die Beobachtung stammt aus einer Reihe stuendlicher Laeufe und ist seither nicht wiederholt worden.
...
**Zustand:** offen, unbelegt — nachgesehen am 25.08.2026. Ohne Messturn mit einer foermlichen Figur ist nichts zu entscheiden.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Die Beitragstabelle laesst die Kombination unveraendert nicht zu.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Der verlangte Zeuge, der beide Tabellen gegeneinander haelt, ist nicht dazugekommen.
...
**Zustand:** offen, unbelegt — nachgesehen am 25.08.2026. Die Ursache ist nicht bestimmt worden.
...
**Zustand:** offen, unbelegt — nachgesehen am 25.08.2026. Die Frage nach dem Gegenpol ist nicht gemessen worden.
...
**Zustand:** abgeschlossen — der Eintrag traegt seine Messung vom 11.08.2026 selbst, und `F-RAD-2` (drei Laeufe, Median) ist gebaut. Nachgesehen am 25.08.2026: `speichenweise_mediane` steht im Charakter-Agenten. **Achtung: dieselbe Kennung steht ein zweites Mal in der Rangordnung** — zwei Ueberschriften auf einen Schluessel.
...
**Zustand:** offen, am Schema bestaetigt am 25.08.2026. `charakter_anweisungen` traegt `id`, `erstellt_am`, `aktiv`, `geaendert_am`, `user_id`, `anweisung` — **kein Gegenueber**. Unveraendert.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Eine Historie der Profile gibt es nicht; die Raeder sind historisiert, die Profiltexte nicht.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Die Liste ist unveraendert fest verdrahtet.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Zwei Erzeuger derselben Groesse, unveraendert.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Der dreiwertige Zeuge ist nicht gebaut.
...
| EMOTIONS-VECTOR-WERTE-DRIFT | Live-Wert außerhalb des dokumentierten Vertrags (Chat 104). `novaberg-personality.md` §3.2 dokumentiert für `Emotion.emotions_vector` die gültigen Werte `plateau, aufschwung, abschwung, peak, tal, leer`. Der erste `turn_roh`-Eintrag (Chat 104, Live-Abnahme) trägt jedoch `aufbluehen` auf der User-Seite — ein Wert außerhalb dieser Liste. Zu klären: (a) Doku veraltet, die Perzeption-Prompts / `ei/berechnung.py` kennen weitere Werte → Doku nachziehen; oder (b) das Perzeptions-LLM erzeugt freie Werte außerhalb des Vertrags → dann sind die Vektor-Werte unzuverlässig und verunreinigen die dauerhafte Charakter-Quelle (`turn_roh`), auf der die Verhaltensweisen-Destillation aufsetzt. Prüfpfad: `prompts/default/perzeption.task.txt`, `perzeption.assistant_task.txt`, `ei/berechnung.py` (Plausibilitäts-Funktionen). **Auflösung:** kein Code-Problem — das Feld ist deterministisch erzeugt (`_emotions_vektor_bestimmen`, geschlossener Wertebereich), die Doku war falsch (Werte UND Quellen-Zuordnung), korrigiert in docs(personality). | [CHA] ✅ Chat 105 gelöst |
| EI-KANON-FEHLT | `relationship_dynamic`, `tone`, `intent`, `prompt_topic` gehen ungeprüft aus dem LLM in den State und in die dauerhafte `turn_roh`-Quelle. Audit Chat 105: aktuell KEIN Miss — jeder Prompt-Wert ist in jeder Konsumenten-Map gedeckt. Der Vertrag hält aber nur, solange das Modell gehorcht; bei Modellwechsel (zuletzt qwen36) fällt ein abweichender Wert still auf Neutral-Defaults. Fix: Laufzeit-Kanon je Feld analog `EMOTION_KANON`/`_emotion_kanonisieren` (fail loud). **Chat 105: Prio mittel → hoch — live bestätigt:** `stil=gehoben` im Log, kein gültiger `language_style`. Der halluzinierte Wert ist real, kein theoretisches Risiko mehr. | [CHA] ⬜ **offen, neu bewertet am 05.09.2026** — **der Ausloeser ist eingetreten**: Seit 18:04 UTC laeuft die Perzeption ueber ein Fernmodell. **Der geforderte Helfer existiert seither** (`utils/canon.py::to_canonical`, dreistufig: Kanon → aufgeloeste Umlaute → `None`, mit Meldung) und ist an der Naht der Perzeption fuer `emotion` und `modus` angewandt. **Fuer `tone`, `intent`, `sprach_stil` und `prompt_topic` bleibt der Eintrag offen, und der Grund hat sich verschoben:** Es fehlt nicht mehr der Riegel, sondern die **Menge**, gegen die er pruefen koennte — fuer diese Felder gibt es keinen Kanon in `config.py`. Der Bau ist damit klein und die Entscheidung inhaltlich: welche Werte zulaessig sind. Zwei Messungen dazu am 05.09.2026: 20 Perzeptionsaufrufe je Modell ergaben **0 Ausreisser** bei `emotion` und `modus`; die Stil-Vielfalt des neuen Modells ist mit 6 verschiedenen Werten dreimal so breit wie die des alten (2) — die ungepruefte Menge ist also gewachsen. Prio hoch |
| SILENT-SKIP-EI-DEFAULTS | 39 von 41 `state.get("external"/"internal")`-Stellen fallen bei None still auf Neutral-Defaults (Audit Chat 105); nur `ei_calc_persist` (error) und der `turn_roh`-Guard (warning) sind laut. Steht gegen `lesson_l_silent-skip`. `personality.md` §5 empfiehlt das Muster noch. | [CHA] ⬜ Prio mittel |
| TURN-ROH-VOR-KRAFT1-ENTWERTET | Stichtag für die Verhaltensweisen-Destillation, gemessen Chat 108 — Volltext im Abschnitt „Landmine: TURN-ROH-VOR-KRAFT1-ENTWERTET — Sperrvermerk für die Verhaltensweisen-Destillation". | [CHA] [CHA] ⬜ Prio hoch — ⬜ **offen** — nachgesehen am 25.08.2026. Der Sperrvermerk gilt unveraendert. |
| EI-VEKTOR-LOG-GATE | `ei_calc.py` unterdrückt `plateau` beim Loggen (am 25.08.2026 bei `:197`, der Befund nannte `:126`). Der Default-Fallback ist auf der User-Seite unsichtbar — dasselbe Muster, das den Chat-89-Defekt der Nova-Seite mit versteckt hat. | [CHA] ⬜ Prio niedrig |
...
|---|---|---|
| **SYK-B-1-THINKER-WEICHE** | ✅ **umgesetzt am 18.09.2026** (`ee7f606`). Der Thinker schreibt `thinker.schnellcheck` (mit den getroffenen Indikatoren einzeln) und `thinker.ergebnis` (sechs Ausgänge samt Iterationen, Werkzeugaufrufen und Nachfass-Versuchen) ins `pipeline_log`; im Betrieb belegt am selben Tag, Turn `729033c8…`: `reasoning` → `korrektur`. Die Anlaufquote ist damit erhebbar. ~~Der Thinker protokolliert den Ausgang seines Schnell-Checks dauerhaft. Heute schreibt er `node_annotations`, die nirgends persistiert werden — seine Anlaufquote ist nicht erhebbar.~~ | [CHA] keine — ✅ **erledigt** — B1/B5 sind damit von dieser Seite entblockt. |
...
| **SYK-B4-WERT-BENENNEN** | ~~Zwei Felder im Kopfblock~~ — **durch die Messung erledigt, bevor er gebaut wurde.** Die Prüfung trägt auch mit korrektem Wert nicht (siehe `SYK-B4-STUFE-2-OHNE-FILTER`); der Prompt-Umbau hätte einen Batterielauf von 6,2 Stunden gekostet und nichts geändert. ⚠ **Stufe 1 ist an diesem Korpus zu 85 % blind — gemessen am 05.08.2026, unmittelbar nach dem Bau.** 17 von 20 Fallen tragen keinen Ziffernwert; die strittigen Werte sind ausgeschriebene Zahlen („vierzig Jahren", „vier Monde") und Ortsnamen („Hannover"). Der Leser ist nicht defekt, die Verengung „Wert = Zahl" ist es. **Der Weg:** Der Kopfblock aus `SYK-B1` trägt den strittigen Wert bereits als Prosa (`GEPRUEFT`); zwei strukturierte Felder — früherer und jetziger Wert — machen ihn deterministisch prüfbar, typunabhängig und **ohne zweiten Modellaufruf**. Berührt den Prompt, braucht also ein eigenes Messfenster | [CHA] B4 Stufe 1 ✅ |
| **SYK-B8-SCHREIBPFAD** | Ein widersprochener Wert wird nicht ohne Marke destilliert. **Blocker:** In 583 KZG-Einträgen steht keine `turn_id` — die Klammer für die Vererbung fehlt. | [CHA] B1, Klammer — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. |
| **SYK-B3-VORZEICHENREGEL** | Der Responder entscheidet, wie freundlich ein Nein klingt, nicht ob es eins ist. Dazu eine ausgesetzte Regel zurück: das Verbot falscher Erfolgsmeldungen. | [CHA] B4 — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. **Neu bewertet am 15.09.2026, zur zweiten Hälfte:** Falsche Erfolgsmeldungen sind am Bestand gemessen — ab 14.08.2026 (Dienst-Ausgänge protokolliert) **24 von 928** Antworten mit Speicherbehauptung ohne abgeschlossenen Dienst, 3 davon Wiederholungen einer echten Schreibung. Seit dem 15.09.2026 **rechnet** die Auswertung des Tribunals den Fall und schickt die Antwort in die Korrektur (Scheibe 12 A, `novaberg-node-tribunal.md`); die Regel im Prompt ist **nicht** zurückgeholt, und ob sie die Behauptungen verhindert hätte, ist nicht gemessen. Die Aussetzung steht im Code als Probe (`graph/nodes/responder.py`, Kommentar vom 31.07.2026) und wurde seither nicht ausgewertet. |
| **SYK-B9-REGISTER** | Nova bekommt einen gelebten Zustand für „hier stimmt etwas nicht". Das Schema gibt ihn her; der Klassifikator wählt ihn nie. Entwurfsfrage vorgeschaltet. | [CHA] Entscheidung — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. |
| **SYK-B2-IMPULS-GRENZE** | Der GV-Impuls darf die Richtung bestimmen, nicht die Faktenauswahl. | [CHA] keine — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. |
| **SYK-B5-ESKALATION** | Der Thinker sieht das Verfasser-Urteil und bekommt Widerspruchs-Indikatoren im Schnell-Check. | [CHA] B-1, B1 — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. |
| **SYK-B6-MOTIVENTLASTUNG** | Der Drang, die Beziehung zu schonen, wird entlastet statt verboten — gekoppelt an den gemessenen Zuwendungswert. | [CHA] allein einzuführen — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. |
| **SYK-B7-ZWEITABLEITUNG** | Eine zweite Ableitung ohne Beziehungsdruck; die Differenz ist die Sykophanz. Erste Probe: 3 von 5 gegen 1 von 5, trägt noch nicht. | [CHA] nur falls B-1…B6 nicht reichen — ⬜ **offen** — nachgesehen am 25.08.2026. Geplante Arbeit, nicht begonnen; der Eintrag beschreibt ein Ziel und keinen Defekt. |
...
| ID | Frage |
|---|---|
| **SYK-M-TRIBUNAL** | [CHA] Erkennt das Tribunal die fünf Fälle? Seine Zuständigkeit ist aus den Prompts gelesen, nicht gefahren. Billigste der drei; hebt eine Zeile im Konzept von „gelesen" auf „gemessen". — ⬜ **offen** — nachgesehen am 25.08.2026. Die verlangte Messung ist nicht angesetzt worden. |
| **SYK-M-VERFASSER** | [CHA] Dämpft der Verfasser? Ein Zufallsvergleich zeigte ohne ihn 1355 Zeichen bei Arousal 0.8, mit ihm 340 bei 0.5 — ein Fall, kein Beleg. — ⬜ **offen** — nachgesehen am 25.08.2026. Die verlangte Messung ist nicht angesetzt worden. |
| **SYK-M-RAHMUNG** | [CHA] Trägt die Szenario-Rahmung? Braucht eine Nulllinie, die den Ausfall reproduziert, und Wiederholungen je Zelle. — ⬜ **offen** — nachgesehen am 25.08.2026. Die verlangte Messung ist nicht angesetzt worden. |
...
**Zustand:** offen — nachgesehen am 25.08.2026. **Zweite Zeile derselben Kennung.**

**Zustand:** offen — nachgesehen am 25.08.2026. **Dieselbe Kennung steht zweimal in diesem Register.**
...
| CRUD-DESTILL-SUBTRAKT | ⚠️ | [CHA] Subtraktive Änderungen als Anweisung gespeichert |
| CRUD-REACTIVATE-STAMP | ⚠️ | [CHA] Reactivate setzt deaktiviert_am nicht auf NULL |
| SPRACH-STIL-DEFENSIV-STUMM | Niedrig | [CHA] `_sprach_stil_erkennen` fällt ohne Warning auf "neutral" bei leerem charakter_hash (Verstoß gegen "fail loud", Chat 89/90) — ⬜ **offen** — nachgesehen am 25.08.2026. **Zweites Vorkommen derselben Kennung in diesem Register.** Der Befund steht beim ersten; die Dopplung selbst ist der groessere Mangel. |
| EI-CALC-ROLLE-DEFAULT-ASYMMETRIE | Niedrig | [CHA] Inkonsistente Defaults beim Rollen-Dispatch in Enricher/EI-Calc/Salience (Chat 90) — ⬜ **offen** — nachgesehen am 25.08.2026. **Zweites Vorkommen derselben Kennung in diesem Register.** Der Befund steht beim ersten; die Dopplung selbst ist der groessere Mangel. |
...
**Zustand:** offen — nachgesehen am 25.08.2026. Verwandt mit `PERSPEKTIVE-OHNE-DATIV`, das am 22.08.2026 behoben wurde; ob dieser Eintrag damit mit erledigt ist, ist nicht nachgeprueft.
...
**Zustand:** abgeschlossen — am Bestand geprueft am 25.08.2026. Alle Schluessel tragen die Form `hash_dirty:<mensch>:<figur>`; ein einteiliger Schluessel liegt nicht mehr vor. **Der Rest steht als `HASH-DIRTY-ALTLASTEN`** — 13 liegengebliebene Schluessel, ein anderer Befund.
...
**Zustand:** offen — nachgesehen am 25.08.2026. **Zweite Ueberschrift derselben Kennung.** Der Sperrvermerk gilt unveraendert; die Dopplung ist der eigene Mangel.
...
**Zustand:** offen — nachgesehen am 25.08.2026. Verwandt mit `PERSPEKTIVE-OHNE-DATIV` (behoben 22.08.2026); ob dieser Eintrag mit erledigt ist, ist nicht nachgeprueft.
