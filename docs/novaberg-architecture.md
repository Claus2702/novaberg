...
| `OpenRouterProvider` | Spricht das OpenAI-Chat-Protokoll über `httpx` — kein Anbieter-SDK, ein Zugang für viele Modelle. **Die Modell-ID wählt den Anbieter nicht** — sie ist eine Ausschreibung unter vielen (29 am 05.09.2026); der `provider`-Block der Nutzlast benennt ihn, mit abgeschaltetem Rückfall. Setzt `expect_json` als `response_format` **durch** statt es nur zu melden, bildet `think` auf den `reasoning`-Schalter ab und liest den Trace aus `message.reasoning`. Token-Logging mit Kosten und Fenster-Auslastung. |
