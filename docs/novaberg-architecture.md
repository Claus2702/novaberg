...
**Stand:** 16. September 2026, 22:31 UTC (JSON-Bereinigung: **gültiges JSON wird nicht repariert** — die Wiederholungskappung schnitt ausgerichtete Einrückungen ab). Davor 15. September 2026 (`utils/storage_claims.py` im Verzeichnisbaum — die Speicherbehauptung der Antwort, gerechnet in der Auswertung des Tribunals). Davor 14. September 2026, 19:52 UTC (was der Feldtest mit `gemma4-a4b-gpu` am Empfang und im Thinker zeigte — gemessen, §2 bei den Worker-Backends). Davor 13. September 2026, 15:11 UTC (**das Eigenschaftsgedaechtnis der Sachlage** — drei Tabellen `sachlage_objekt`, `sachlage_objekt_turn`, `sachlage_eigenschaft`, die Module `graph/nodes/sachlage_form.py`, `graph/nodes/sachlage_memory.py` und `memory/sachlage_properties.py`). Davor 12. September 2026, 16:20 UTC (**ein zweiter Redis-Client** — `redis_client_bytes` ohne Dekodierung, fuer Abfragen, die einen Vektor zurueckholen; mit dem dekodierenden bricht die Suche ab und liefert null Treffer bei vollem Speicher). Davor 11. September 2026, 16:00 UTC (**vier Konstanten ergänzt** — die beiden Teilschalter für Agenten und Zustellung, `QUALITAET_FRISCHE_TAGE` und `VERFASSER_IMPULS_NAHE`; dazu die berichtigte Zahl offener Profil-Kandidaten: 1511 statt 350, also 75 Tage statt 18). Davor 6. September 2026, 11:31 UTC (`agents/qualitaet_profil/` im Baum — die Qualitaetsprofile sind aus dem Tageslauf ausgezogen, weil ein Modellaufruf nicht in die `cpu`-Spur gehoert; der Tageslauf traegt seither **neun** Schritte). Davor 6. September 2026, 10:20 UTC (der Tageslauf `synapsen_decay` traegt **zehn** Schritte — der Preiswaechter kam heute dazu, er hatte seit seinem Bau keinen Aufrufer; dazu `services/model_costs.py` im Baum: die Kostenrechnung sitzt im Provider, weil die Trennung von Ein- und Ausgabe auf dem Rueckweg verlorengeht). Davor 5. September 2026, 20:33 UTC (die Parallelitaets-Auskunft in §2.2 — der Riegel ist fuer den Fernpfad gegenstandslos, die Queue-Tiefe liegt in 231 von 256 Einreihungen bei 1; dazu die Modellgroesse. **Beides stand nur im Gespraech.**) Davor 5. September 2026, 20:31 UTC (die **zwölf OpenRouter-Konstanten** in der Konstantentabelle und der Env-Uebersicht — gefunden vom Bezeichner-Diff, nachdem der dateibasierte Nachzug vollstaendig war; die Env-Zeile nannte ausserdem noch `ollama_gpu` als Zustand). Davor 5. September 2026, 20:16 UTC (die Prompt-Modellebene macht einen Modellnamen mit Schraegstrich **flach** — ein Fernmodell nennt den Anbieter mit, und ein verschachteltes Verzeichnis waere eine Ebene, die niemand nachschlaegt; dazu `prompts/deepseek_deepseek-v4-flash-0731/` im Baum). Davor 5. September 2026, 18:51 UTC (`utils/canon.py` im Baum — der Kanon-Zug an der Naht der Perzeption; `graph/einwand.py` holt seine Umlaut-Abbildung seither dort). Davor 5. September 2026, 18:12 UTC (§2.2 um das Abschalten des Reasonings ergaenzt — der Trace zaehlt gegen `max_tokens`; **im Betrieb**: alle drei Worker laufen ueber den Fernzugang). Davor 5. September 2026, 17:41 UTC (§2.2 um die Anbieterwahl ergaenzt — eine Modell-ID ist eine Ausschreibung unter 29, und der `provider`-Block benennt den Gewinner; dazu `tools/openrouter_price_watch.py` im Baum). Davor 5. September 2026, 17:06 UTC (`OpenRouterProvider` als vierter Rueckhalt in §2.2 und §2.3 — ein Zugang, viele Modelle; dazu die Aussage, dass **alle drei Graphen** ueber die Worker laufen (35 `chat.submit`, 16 `background.submit` ueber 63 Dateien gezaehlt), und die **Prompt-Modellebene folgt jetzt dem Backend** statt unbedingt `OLLAMA_MODEL`). Davor 5. September 2026, 10:20 UTC (der Tageslauf `synapsen_decay` traegt **neun** Schritte — die Traegerseite der Faszination kam heute dazu; `memory/fascination_store.py` im Baum, und `ei/fascination.py` traegt nicht mehr nur den Merkmalszug, sondern die ganze Rechnung aus §10). Davor 3. September 2026, 21:30 UTC (der Tageslauf `synapsen_decay` traegt **acht** Schritte — die Qualitaetsprofile kamen heute dazu; `memory/quality_profile.py`, `ei/fascination.py` und `memory/repositories/quality_profile_repository.py` im Baum, `abstrakt_knoten` und `traeger_qualitaet` in der Tabellenliste — die abstrakte Schicht). Davor 3. September 2026, 19:30 UTC (der Tageslauf `synapsen_decay` traegt **sieben** Schritte — die Einfaerbungen kamen heute dazu). Davor 3. September 2026, 18:57 UTC (`praegung_faden` **5** und `praegung_strang` **2** Zeilen nachgezählt; die Aussage *„Richtung und Staerke nicht gebaut"* ist widerlegt — beide sind gebaut und werden seit heute vom Praegungszug gelesen). Davor 1. September 2026, 19:45 UTC (`praegung_strang` in der Tabellenliste und der Tageslauf `synapsen_decay` traegt **sechs** Schritte — der Strang-Nachzug kam heute dazu; `praegung_beruehrung` stand seit dem Vormittag falsch als leer). Davor 1. September 2026, 15:30 UTC (der Tageslauf `synapsen_decay` traegt vier Schritte, nicht zwei — Queue-Verfall stand seit dem 15.08.2026 nicht im Baum, die Faltung kam heute dazu). Davor 1. September 2026, 14:30 UTC (`graph/nodes/verfasser.py` und `thinker_cache.py` im Baum, Zahl der Knoten-Dateien von 20 auf **25** nachgezaehlt — beide fehlten, gefunden von `C18`). Davor 1. September 2026 (`memory/praegung.py` im Baum, `praegung_faden` und `praegung_beruehrung` in der Tabellenliste — die Praegungsschicht; zwei Lessons im Register). Davor 30. August 2026 (`memory/repositories/node_stance_repository.py` und die Tabelle `lzg_knoten_haltung` im Baum — die Haltungsschicht). Davor 29. August 2026 (`graph/nodes/sachlage_plausibility.py` und `sachlage_research.py` im Baum — Scheiben 7 und 8). Davor 28. August 2026 (`memory/kurzziel.py` im Baum — Scheibe 2; `memory/sachlage_history.py` im Baum, `sachlage_verlauf` und `shadow_auftrag.ausloeser_turn_id` in der Tabellenliste — Scheibe 4 des Lage-Konzepts). Davor: 25. August 2026, 20:10 UTC (das Backlog ist nach Gegenstand geteilt — sechs Kategoriedateien, die Rangordnung eigens, das Findemittel darueber; alle im Dokumenten-Index §7). Davor, 12:00 UTC: die Chronik ist nach Monaten geteilt — `novaberg-roadmap.md` traegt den laufenden Zeitraum, vier `novaberg-roadmap-2026-*.md` die abgeschlossenen, und `novaberg-roadmap-index.md` spannt als erzeugtes Findemittel darueber; alle im Dokumenten-Index §7). Davor: das Bugregister ist geteilt — `novaberg-bugs-archiv.md` neu im Dokumenten-Index §7, `novaberg-bugs.md` traegt nur noch die nicht abgeschlossenen Eintraege). Davor: 24. August 2026 (Telegram-Kanal abgeschaltet — Dienstetabelle §1.2, Compose-Auszug §8.1, Startreihenfolge §8.3). Davor: 23. August 2026 (`shadow_auftrag.grund` in der Tabellenliste; davor am selben Tag: Prompt-Segregation dreistufig — `default` → `{modell}` → `{connector}`; `utils/etikett.py` und der `tools/dateien/`-Baum berichtigt; Connector-Tabelle um `qwen36` ergaenzt; *0 hardcoded Prompts* gegen 33 gemessene Literale widerlegt). Davor: 19. August 2026 (`agents/wissen/` — die Bibliothek als **bestellbarer** Dienst: Aushang in `plugins/wissen_manager/`, vier Ausgaenge, und die Abfrage liegt geteilt im Repository, weil Quelle und Zettel denselben Bestand lesen. **Der Manager fehlt weiterhin in der Plugin-Tabelle unten** — der Bestandsbefund von §5 ist damit aelter geworden, nicht erledigt; davor 18. August 2026 (`agents/wissen_rueckweg/` — der Rueckweg vom Gespraech in die Wissensdatei, ausgeloest von der Promotion; dazu `agents/dateien/` **vollstaendig** — Suche, Zoom und der Aufrufer (Aushang in `plugins/dateien_manager/`, Klassifikation, Dispatch, Auskunft); davor am selben Tag ohne Aufrufer; dazu `agents/dateien_index/aufzeichnungen.py`; die Zaehlaussage „sechste Kontextquelle“ bei `autonomous_wissen` um den Dateien-Index ergaenzt — eine weitere Quelle des Enrichers, die ausdruecklich **kein** Plugin ist); davor 16. August 2026 (gegen den Code geprüft: die Modelle kommen aus der **Connector-Tabelle**, nicht aus `OLLAMA_GPU_MODEL`/`OLLAMA_CPU_MODEL`; der Baum unter `memory/` nannte `embedding.py` und `lzg.py`, die es beide nicht gibt — §2.5 und §4 nachgezogen); davor 15. August 2026 (`services/pixie/riegel.py` und `memory/haltung.py` im Baum, `shadow_auftrag.arousal` in der Tabellenliste); davor 8. August 2026 (`novaberg-graph-rechenkette.md` im Dokumenten-Index Tiefe 1 ergänzt — das Register der 34 Rechensysteme des Charakter-Pfads, zerlegt entlang der Rechnungen statt entlang der Knoten. Zuvor: 31. Juli 2026, abends — `graph/nodes/haltung.py` im Verzeichnisbaum ergänzt, Knotenzahlen des CharacterGraph und des Node-Verzeichnisses neu gezählt — beide standen älter als der Verfasser da. Zuvor: `ei/haltung.py` im Verzeichnisbaum ergänzt und der Eintrag zum Haltungsraum im Dokumenten-Index auf das Beitragsmodell umgestellt — die Rechnung ist gebaut, der Knoten fehlt. Zuvor: `novaberg-node-verfasser_k.md` im Dokumenten-Index ergänzt — Konzept, nicht gebaut. Zuvor: `ruff-hart.toml` im Verzeichnisbaum ergänzt, §3 — die harte Teilmenge ohne geduldeten Bestand. Zuvor: `ruff.toml` im Verzeichnisbaum ergänzt, §3. Zuvor: Verzeichnisbaum nachgezogen — `ei/` vollständig, vier fehlende Agenten ergänzt. Kern: Chat 94, Microservice-Welle Block 2+3; Embedding-Modellwechsel Chat 107, §2.4)
...
> **Beim Start heißt: nur beim Start.** Eine geänderte `.txt`-Vorlage wirkt erst nach einem Neuladen des Servers. Der Entwicklungsserver läuft mit `uvicorn --reload`, und dessen Beobachter reagiert in der Vorgabe auf Python-Dateien — **nicht einzeln gemessen**: Am 14.09.2026 fiel jede Vorlagenänderung mit einer Python-Änderung zusammen, und das Neuladen im Log 1,6 s danach ist beiden zuzurechnen. Eine Messung, die eine geänderte Vorlage im Betrieb sehen will, beginnt deshalb mit einem Neustart; ein Laborprozess, der `config` frisch importiert, sieht sie sofort — und misst damit etwas anderes als der laufende Server.
...
Producers ──submit / submit_sync──▶  model_service.{ embed | chat | background }
       ...
       ┌──────────────────────┬─────────────────┴──────────┬────────────────────┐
       ▼                      ▼                            ▼                    ▼
ollama_gpu_client      ollama_cpu_client            AnthropicProvider   OpenRouterProvider
(OLLAMA_MODEL)  (PIXIE_ANALYSE_MODEL / SHADOW_MODEL) (ANTHROPIC_MODEL)  (OPENROUTER_MODEL)
```
...
kennen kein Modell, sondern nur die Rolle. Gemessen am 05.09.2026: 35 Aufrufe
von `model_service.chat.submit` und 16 von `background.submit` über 63 Dateien;
die einzigen verbleibenden Direktzugriffe auf einen Modell-Client sind die
Statusabfrage (`api/health.py`, `list()`/`pull()`) und die Embedding-Pfade.
**Ein Wechsel des Backends ist damit ein Wechsel des Modells für das ganze
System, und er ist eine Umgebungsvariable.**

**Was der Wechsel an der Nebenläufigkeit ändert — gemessen am 05.09.2026, auf die
Frage, ob der Riegel entfallen kann.** Drei Ebenen, und nur eine ist der Riegel:

- **Der Ollama-Riegel ist für den Fernpfad gegenstandslos.** Er schützt die GPU als
  *Ressource* (`F-RIEGEL-1`); der `OpenRouterProvider` fasst ihn nicht an, er hält seinen
  eigenen HTTP-Client. Es ist nichts abzuschalten. Für das Embedding, das lokal bleibt,
  ist er weiterhin nötig.
- **Die Worker-Queue serialisiert weiter** — eine `asyncio.Queue`, ein Task, FIFO. Sie ist
  die Fahrgemeinschaft für *ein* Auto; beim Fernmodell gibt es kein Auto.
- **Seriell bleiben muss dreierlei, und keines davon hängt am Modell:** `graph_run_lock`
  (er schützt den Gesprächszustand, nicht die Ressource), der Pixie-Lock, und die
  Knotenfolge im Graphen — die ist Datenfluss, kein Riegel.

**Was ein Umbau brächte:** Queue-Tiefe **1 in 231 von 256** Einreihungen — in 90 % wartet
niemand; Tiefe 2 in 13, Tiefe 3 in 12 Fällen. Der Gewinn läge bei gleichzeitigen Turns
mehrerer Paare, nicht innerhalb eines Turns, dessen elf Aufrufe datenabhängig sind. **Eine
Parallelisierung gäbe die Reihenfolgegarantie der Queue auf, die heute niemand ausdrücklich
prüft** — deshalb ist sie ein eigener Auftrag mit eigener Messung und nicht nebenbei zu
haben.

**Eine Modell-ID ist keine Modellwahl.** Am 05.09.2026 gemessen
(`GET /models/{id}/endpoints`): 29 Anbieter liefern dasselbe Modell, mit Faktor
8,8 im Eingangspreis, Faktor 13,2 im Ausgangspreis, vier Quantisierungen
(fp4/fp8/bf16/unbekannt), Kontextfenstern von 262.144 bis 1.310.720 — und
**vier davon führen `response_format` nicht**, womit `expect_json` dort wieder
eine Bitte wäre. Ohne Festlegung wählt der Zugang pro Aufruf; eine Messreihe
würde den Anbieterwechsel mitmessen. Der `provider`-Block der Nutzlast nennt
deshalb `only` (nicht `order` — das ist eine Reihenfolge und erlaubt
Weitergehen), `quantizations` und `allow_fallbacks: false`.

**Ein Reasoning-Modell denkt ohne Aufforderung, und der Trace zählt gegen
`max_tokens`.** `think=False` schaltet es deshalb ausdrücklich ab
(`reasoning_effort="none"`); ohne diese Zeile bekäme `query_rewrite` bei 64
Token eine **leere** Antwort, gemessen am 05.09.2026 (0 Zeichen Inhalt gegen 238
Zeichen Denken). Kein Schalter ist bei einem solchen Modell kein neutraler
Zustand.

Was der gewählte Anbieter an Sampling-Schrauben führt, steht als Kanon in
`OPENROUTER_GEFUEHRTE_PARAMETER`; ein Aufrufer, der etwas anderes setzt,
bekommt eine Warnung — der Wert geht trotzdem mit, entfernt wird er nicht.
`services/price_watch.py` hält Preis, Rabatt, Fenster, Quantisierung
und diesen Kanon gegen den lebenden Endpunkt und braucht dafür keinen
Schlüssel.

**Die Prompt-Modellebene folgt dem Backend, nicht dem Connector** (seit
05.09.2026). **Ein Modellname mit Schrägstrich wird dabei flach gemacht** —
`deepseek/deepseek-v4-flash-0731` sucht unter
`prompts/deepseek_deepseek-v4-flash-0731/`. Unverändert eingesetzt ergäbe der
Name ein verschachteltes Verzeichnis, dessen Zwischenglied eine Ebene wäre,
die niemand nachschlägt; `test_prompt_loader_ebenen` meldet so etwas als
totes Material. `prompts/{modell}/` wird nach dem Modell geschlüsselt, das
tatsächlich antwortet — `config.antwortendes_chat_modell()` löst es aus
`MODEL_WORKER_BACKENDS["chat"]` über `MODELL_NACH_BACKEND` auf. Vorher stand
dort unbedingt `OLLAMA_MODEL`; solange der Chat-Worker auf `ollama_gpu` lief,
war das deckungsgleich. Bei einem fremden Backend hätte es die sieben Blöcke
unter `prompts/gemma4-gpu/` (Perzeption, Router, Salienz, Tribunal) einem
Modell untergeschoben, das nie Gemma war.
...
| `OpenRouterProvider` | Spricht das OpenAI-Chat-Protokoll über `httpx` — kein Anbieter-SDK, ein Zugang für viele Modelle. **Die Modell-ID wählt den Anbieter nicht** — sie ist eine Ausschreibung unter vielen (29 am 05.09.2026); der `provider`-Block der Nutzlast benennt ihn, mit abgeschaltetem Rückfall. Das seit 05.09.2026 eingesetzte Modell ist ein spärlich besetztes Mixture-of-Experts: **284 Mrd. Parameter gesamt, 13 Mrd. aktiv** — daher **70,7 Ausgabe-Token je Sekunde** gegen 49,4 lokal, plus 43 % `[gemessen]`. Setzt `expect_json` als `response_format` **durch** statt es nur zu melden, bildet `think` auf den `reasoning`-Schalter ab und liest den Trace aus `message.reasoning`. Token-Logging mit Kosten und Fenster-Auslastung. |
...
>
> **Der Feldtest läuft weiter — Entscheidung des Eigentümers am 13.09.2026**, auf das Angebot, zum Vorgängermodell zurückzukehren: *„Ja, machen wir mit dem Feldtest weiter"* und *„Wir müssen aber den TimelineAgenten unter Gemma 4 korrekt betreiben"*. Der Router wird an das Modell angepasst, nicht das Modell an den Router.
>
> **Was der Feldtest mit `gemma4-a4b-gpu` am Empfang zeigt (13./14.09.2026, gemessen):** Dieselben vier Router-Prompts mit Terminaussagen ohne Kommando-Verb stellte `gemma4-a4b-gpu` in **0 von 40** Aufrufen zu, `gemma4-gpu` in **9 von 9** — die Modelle lösen den Widerspruch zwischen Dispatch-Guard und Aushang verschieden auf. Unter der seither geltenden Absicht (eingetragen wird nur auf Auftrag) schreibt die Kette unter `gemma4-a4b-gpu` Aufträge 12/12 und Aussagen 0/21 (`novaberg-node-router.md`, letzter Abschnitt). Im selben Feldtest lieferte der Thinker am 14.09.2026 in vier Nachfass-Iterationen hintereinander leeren `content` bei gefülltem `thinking` (`novaberg-node-thinker.md` §3.5).
...
| `OPENROUTER_API_KEY` | `""` | Schlüssel für den Fernzugang (aus `.env`) |
| `OPENROUTER_MODEL` | `"deepseek/deepseek-v4-flash-0731"` | Modell-ID beim Zugang (Backend `openrouter`) |
| `OPENROUTER_URL` | `…/api/v1/chat/completions` | Ziel des Aufrufs |
| `OPENROUTER_PROVIDER` | `"baidu"` | **Welcher Anbieter die ID bedienen darf.** Hinter ihr standen am 05.09.2026 **29**; ohne diese Zeile wählt der Zugang pro Aufruf, und Preis, Quantisierung und Kontextfenster sind nicht die konfigurierten · **Am 06.09.2026 gegen `Relace/fp4` geprüft und beim Bestand geblieben:** Relace ist ~10 % billiger **und dauerhaft** (kein Rabatt, der enden kann), führt alle drei Penalties und bietet das siebenfache Ausgabefenster — **aber kein `response_format` und keine `structured_outputs`**. Neun Produktivaufrufe setzen `expect_json=True`; sie brächen laut (die Worker propagieren `JSONDecodeError`), nicht still. Dazu FP4 statt FP8. **Der Tausch wäre Wiederholungsbremse gegen JSON-Fessel** — vor einem Wechsel sind die neun Aufrufe zu messen |
| `OPENROUTER_QUANTISIERUNG` | `"fp8"` | Zweiter Riegel — bei einem Anbieter mit mehreren Endpunkten der Unterschied zwischen zwei Modellen |
| `OPENROUTER_FALLBACKS` | `false` | Rückfall aus: lieber ein Fehler als eine unbemerkt andere Rechnung |
| `OPENROUTER_DENKEN_AUS` | `"none"` | `reasoning_effort` bei `think=False`. **Kein Schalter ist kein neutraler Zustand:** Das Modell denkt von sich aus, und der Trace zählt gegen `max_tokens` — mit 64 Token Grenze kam **null Zeichen Inhalt** gegen 238 Zeichen Denken `[gemessen 05.09.2026]` |
| `OPENROUTER_NUM_CTX` | `1048576` | Kontextfenster **beim gewählten Anbieter** — geht in keine Nutzlast, nur in die Warnschwellen der Token-Buchführung |
| `OPENROUTER_TIMEOUT_S` | `180.0` | Frist eines einzelnen Aufrufs |
| `OPENROUTER_PRICE_INPUT_PER_M` | `0.04998` | USD je Million Eingangs-Token, vom **Endpunkt** gelesen (nicht aus der aggregierten Modellliste — die nannte einen anderen Anbieter). Trägt 64,3 % Rabatt **ohne genannte Frist**; `tools/openrouter_price_watch.py` hält beides gegen den lebenden Endpunkt |
| `OPENROUTER_PRICE_OUTPUT_PER_M` | `0.09996` | dasselbe für Ausgangs-Token |
| `PREIS_BEFUND_KEY` | `preis_befund` | Wo ein Befund des Preiswächters liegt, bis ihn eine Antwort mitnimmt. **Redis und nicht der direkte Zuruf:** Der Tageslauf hat keinen Event-Loop, und `broadcast_threadsafe` braucht einen |
| `PREIS_BEFUND_TTL_S` | `172800` | Zwei Tage — länger als der Abstand zweier Tagesläufe, damit ein Befund nicht verfällt, bevor ihn jemand gesehen hat; kurz genug, dass ein behobener nicht nachhallt |
| `PIXIE_QUALITAET_INTERVALL_SEKUNDEN` | `86400` | Takt des Profil-Agenten. **Setzung des Eigentümers, 06.09.2026: einmal am Tag.** ~~Gemessen: 350 Kandidaten offen, 20 je Lauf — rund 18 Tage bis zum aufgeholten Bestand~~ → **am 11.09.2026 nachgemessen: 1511 offene Kandidaten, also rund 75 Tage.** Der Bestand wächst schneller, als der Lauf ihn abarbeitet. Der Takt ist deshalb unverändert geblieben und stattdessen die **Auswahl** geändert: Die Kandidatensuche sortiert seit dem 11.09.2026 zuerst nach `QUALITAET_FRISCHE_TAGE` (7) — die zuletzt gelesenen Träger kommen zuerst, statt auf ihre Stelle in einer 75 Tage langen Schlange zu warten |
| `PIXIE_QUALITAET_PRIORITAET` | wie `PIXIE_DECAY_PRIORITAET` (0.2) | Ein Wartungslauf drängt nicht, er soll nur zuverlässig drankommen |
| `QUALITAET_FRISCHE_TAGE` | `7` | **Neu am 11.09.2026.** Das Fenster, in dem ein Lesevorgang als *frisch* zählt; die Kandidatensuche sortiert zuerst danach. Die Gesamtlesezahl wählte die historisch häufigen, und ein heute erstmals gelesener Knoten stand hinter allen mit zwanzig — obwohl die Faszination genau ihn jetzt braucht. Sieben Tage decken den Gesprächsverlauf ab, nicht den Bestand |
| `PIXIE_PAUSE_SCHLUESSEL` | `pixie:paused` | **Als Konstante benannt am 11.09.2026.** Der Schlüssel stand viermal als Zeichenkette im Code, und ein Leser fehlte: Die Pause hielt den Scheduler an, während die Zustellschleife weiter auslieferte |
| `PIXIE_AGENTEN_PAUSE_SCHLUESSEL` | `pixie:agenten_paused` | **Neu am 11.09.2026.** Hält nur die Hintergrundagenten an; die Zustellung läuft weiter. Für eine Reihe, die zugestellte Impulse prüft, ohne dass ein frischer Fund dazwischenfällt |
| `PIXIE_DELIVERY_PAUSE_SCHLUESSEL` | `pixie:delivery_paused` | **Neu am 11.09.2026.** Hält nur die Zustellung an; die Agenten arbeiten. Für eine Messung am Hintergrund, deren Ergebnisse nicht mitten in die Reihe fallen sollen |
| `VERFASSER_IMPULS_NAHE` | `0.55` | **Neu am 11.09.2026.** Ab dieser thematischen Nähe kommt ein zugestellter Fund ohne Übergangsblock aus. Die Zustellschwelle liegt bei 0,30 — ein Eintrag mit 0,37 sprang gemessen ohne ein Wort des Übergangs vom Gesprächsthema weg. Gesetzt, nicht abgeleitet: Eine Spanne gemessener Nähen mit dem Urteil *brauchte eine Brücke* daneben gibt es noch nicht |
| `OPENROUTER_GEFUEHRTE_PARAMETER` | elf Werte | Was der gewählte Anbieter an Sampling-Schrauben führt. **`presence_penalty` und `repetition_penalty` sind nicht dabei**, obwohl Responder und Verfasser sie setzen — der Provider meldet das und entfernt sie nicht |
...
Workaround: `think=False` senden, `format="json"` NICHT senden. JSON-Einhaltung erfolgt über Prompt-Overrides (Gemma4-spezifische `[REGELN]`) + die Cleanup-Pipeline in `services/postprocess.py` (`clean_json_response` → `deduplicate_repetition` → `repair_truncated_json`, gebündelt in `parse_json_strict`). **Seit dem 16.09.2026 in dieser Reihenfolge nur noch für ungültigen Text:** Nach `clean_json_response` wird zuerst geparst; nur wenn das scheitert, greifen Kappung und Klammerreparatur, und das steht als `JSON repariert` im Log. Gemessen: Die Kappung hielt eine Einrückung von 25 Leerzeichen für eine Endlosschleife und schnitt eine gültige Sachlage ab. Die Pipeline läuft seit der MS-Welle im Worker (`expect_json`-Pfad), nicht mehr im Provider.
...
│   │   └── nodes/                       #   25 Node-Dateien, von den drei Graphen geteilt (gezählt 01.09.2026; 20 am 31.07.2026)
...
│   │       ├── sachlage_form.py         #     → novaberg-thinking-lage_k.md §3 Festlegung 1 (die Form jedes Objektfelds beim Parse, beim Laden der vorigen Blase und im Kontext-Tab; seit 13.09.2026)
│   │       ├── sachlage_memory.py       #     → novaberg-thinking-lage_k.md §4 Scheibe 11 (die Objekte eines Turns ablegen, an Entitäten und Zeitanker binden; seit 13.09.2026)
...
│   │       ├── praegung.py              #     → novaberg-node-praegung.md (Faden-Tor, Praegungszug, Faszination je Turn)
...
│   │       ├── verfasser.py             #     → novaberg-node-verfasser_k.md (zwischen GV und Responder: bestimmt WAS gesagt wird, der Responder gibt die Form)
│   │       ├── thinker_cache.py         #     Per-Turn-Tool-Cache des Thinkers, zwei Stufen (THINK-MEM-LOOP); strikt lokal je think()-Aufruf
│   │       └── gespraechsvektor.py      #     → novaberg-node-gv_k.md
│   │
│   ├── agents/                          # Agent-System (Epic 11 + Epic 5) — 18 Agenten
...
│   │   ├── synapsen_decay/              #   Der Tageslauf (Pixie), neun Schritte: Knoten-Decay · pipeline_log-Retention · Queue-Verfall · Faltung der Prägungsfäden · Nachzug der Stränge · Strang-Richtungen · Einfärbungen · Trägerseite der Faszination · Preiswächter — die Qualitätsprofile sind am 06.09.2026 nach agents/qualitaet_profil/ ausgezogen (Modellaufruf gehört nicht in die cpu-Spur) (→ novaberg-memory-qualitaetsprofil.md, novaberg-thinking-faszination_k.md §10.6)
│   │   ├── qualitaet_profil/            #   Die sechs Qualitäten je Träger, täglich, `llm`-Spur — am 06.09.2026 aus dem Tageslauf ausgezogen: dort scheiterte jeder Träger am Spur-Riegel (0 von 20 über zwei Läufe), weil ein Profil einen Modellaufruf kostet
...
│   │   ├── fascination_store.py         #   Die Traegerseite der Faszination: Anker-Zaehler, verfallenes Profil, Strangnaehe, Bestandslauf (→ novaberg-thinking-faszination_k.md §10.2, §10.3a)
│   │   ├── praegung.py                  #   Praegungsfaeden: Tor, Formkurve, Auffrischung, Faltung, Straenge, Zug (→ novaberg-node-praegung.md)
│   │   ├── quality_profile.py           #   Der Erzeuger der Qualitätsprofile: Prompt, Annahme der Modellantwort, gedeckelter Lauf (→ novaberg-memory-qualitaetsprofil.md)
...
│   │   │   ├── node_stance_repository.py #     Die Ladung eines Knotens (→ novaberg-thinking-opinion_k.md §5a)
│   │   │   └── quality_profile_repository.py # Qualitätsprofil eines Trägers, vorzeichenlos (→ novaberg-memory-qualitaetsprofil.md)
│   │   └── services/
...
│   │   ├── storage_claims.py            #   Speicherbehauptung der Antwort gegen die Dienste (15.09.2026)
│   │   ├── etikett.py                   #   was ein Pfad ueber die Geltung sagt (23.08.2026)
│   │   └── canon.py                     #   Modellwert auf kanonische Form ziehen (05.09.2026)
│   │
...
│       ├── model_costs.py               #   Kosten je Aufruf + Turn-Kennung — die Rechnung sitzt im Provider, weil ChatResponse nur die Summe traegt
...
│           ├── dispatch.py              #     Agent-Ausfuehrung; `fehler` ins Audit nur, wenn der Dienst schweigt (entkommene Ausnahme, fehlender Agent) — jeder Dienst schreibt sein Audit selbst (NMCP §8.4, seit 18.09.2026)
...
| `praegung_faden` | im Betrieb belegt, **5 Zeilen** (03.09.2026 gezählt), alle einem Strang zugeordnet | novaberg-node-praegung.md |
| `abstrakt_knoten` | die geschlossenen Sätze der abstrakten Schicht; **6 Zeilen** `art = 'qualitaet'` (gesetzt im Schema), **0** `art = 'wert'` (03.09.2026) | novaberg-memory-qualitaetsprofil.md |
| `traeger_qualitaet` | die **vorzeichenlose** Kante Träger → Qualität; **168 Zeilen an 28 Trägern** (03.09.2026, 21:38 UTC gemessen) | novaberg-memory-qualitaetsprofil.md |
| `praegung_beruehrung` | im Betrieb belegt 01.09.2026 — 7 Zeilen | novaberg-node-praegung.md |
| `praegung_strang` | im Betrieb belegt, **2 Zeilen** (03.09.2026 gezählt) ueber 4 und 1 Faden; ~~**Richtung und Staerke nicht gebaut**~~ → **beide gebaut** (02.09.2026) und seit dem 03.09.2026 vom Praegungszug gelesen, keine davon gespeichert | novaberg-node-praegung.md §6b |
...
### Tiefe 2 — Gedaechtnis-Module (5)

| Dokument | Beschreibung |
|----------|-------------|
| novaberg-memory-qualitaetsprofil.md | **Die abstrakte Schicht: Qualitätsprofile** — `abstrakt_knoten` (Typ-Diskriminator, sechs gesetzte Dimensionen) und `traeger_qualitaet` (**vorzeichenlose** Kante). Der Träger einer Faszination ist nicht die Entität, sondern ein Merkmalsprofil; der `merkmalszug` verrechnet die sechs zu einem weichen ODER. Erzeuger ist der achte Schritt des Tageslaufs, ein Modellaufruf je Träger (🟠 gebaut und im Bestand — 25 Träger, 150 Kanten —, **aber die Dominanz kollabiert auf `komplexitaet`, 23 von 25**) |
...
| novaberg-lesson_l_zweite-fundstelle-nach-behebung.md | Eine behobene Fehlerklasse steht an ihrer zweiten Stelle weiter (Grep auf die Quelle des falschen Werts) |
...
> **Zwei Redis-Clients, ein Dienst** (12.09.2026). `config.py` hält neben
> `redis_client` (`decode_responses=True`) einen zweiten: `redis_client_bytes`
> (`decode_responses=False`). Er ist für Abfragen da, die einen **Vektor**
> zurückholen — der Lückenpfad holt seit dem 12.09.2026 das `embedding` der
> KZG-Treffer mit, um die Kern-Nähe je Kandidat zu rechnen.
>
> **Der Grund ist kein Stil, sondern ein gemessener Ausfall:** Mit
> `decode_responses=True` liest redis-py die ganze Antwort als UTF-8, ein
> float32-Blob bricht das ab, die Ausnahme wird gefangen — und die Suche liefert
> **null Treffer bei 3544 Schlüsseln im Speicher**. Ein gebrochener Zugriff sieht
> dann aus wie ein leeres Kurzzeitgedächtnis. Die Lebenszeichen-Prüfung oben
> bleibt bei einem Client: Sie prüft den **Dienst**, und den gibt es einmal.
...
| Pixie | `PIXIE_INTERVALL_MIN` (10), `PIXIE_PROMOTION_PRIORITAET` (0.9), `PIXIE_PROMOTION_INTERVALL_SEKUNDEN` (300), `PIXIE_QUALITAET_INTERVALL_SEKUNDEN` (86400), `PIXIE_QUALITAET_PRIORITAET` (0.2), `PROMOTION_THRESHOLD` (0.8, in `memory/kzg.py` — Salienz-Schwelle, nicht in config.py) |
...
| LLM-Backends | `WORKER_BACKEND_CHAT`, `WORKER_BACKEND_BG_ANALYSE`, `WORKER_BACKEND_BG_SPRACHE` — **alle drei seit 05.09.2026 auf `openrouter`**, gesetzt in der `.env` neben der Compose-Datei; die Vorgabewerte im Code bleiben lokal und sind der Rückweg. Dazu `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`, `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, `OPENROUTER_PROVIDER`, `OPENROUTER_QUANTISIERUNG`, `OPENROUTER_FALLBACKS`, `LLM_PROFILE` (lokal — nur noch ThinkingNormalizer-Schalter, siehe §2.5/§2.7) |
...
| `sachlage_objekt` | `db/init.sql` | **Das Eigenschaftsgedaechtnis der Sachlage, Objekte** (13.09.2026) — eine Zeile je Paar und Objektname (`name_schluessel` eindeutig je Paar), `klasse`, `entitaet_id` (→ `entitaeten`, **ON DELETE SET NULL**, gesetzt erst, wenn die Magnete eines Turns das Objekt nennen) und `entitaet_bindung` (wie gebunden wurde, heute `turn_magnet`). Kein Verfall. Konzept `novaberg-thinking-lage_k.md` §4, Scheibe 11 |
| `sachlage_objekt_turn` | `db/init.sql` | **Die Objekte je Turn** (13.09.2026) — `verlauf_id` (→ `sachlage_verlauf`), `objekt_id` (→ `sachlage_objekt`), beide **ON DELETE RESTRICT**, `turn_id`, `akut`; eindeutig je Verlaufszeile und Objekt. Ueber `turn_id` erreicht die Bindung `verbindung`, KZG und LZG |
| `sachlage_eigenschaft` | `db/init.sql` | **Die Werte mit Historie** (13.09.2026) — `objekt_id` (**RESTRICT**), `eigenschaft` samt Schluessel, `wert`, `sprecher`, `quelle` (JSONB), `turn_id`, `bestaetigt_turn_id`, `timeline_id` (nackt im Kern; Fremdschluessel **SET NULL** in `server/agents/timeline/init.sql`), `aktiv`, `t_valid`, `t_invalid`, `abgeloest_durch` (selbstbezogen, **SET NULL**). **Ein neuer Wert ueberschreibt nicht** — der alte wird inaktiv; hoechstens ein aktiver Wert je Objekt und Eigenschaft, erzwungen per eindeutigem Teilindex. Kein Verfall (`F-VERFALL-1`) |
