...
**Seit dem 16.09.2026 davor der Schatten** (Scheibe 12 C2, `agents/object_nearness.py::shadow_nearness`): je akutem Objekt der Sachlage der Kosinus des Objekttexts zu jedem Objekt-Merkmal; Empfänger ist jeder Dienst mit Nähe ≥ `OBJEKT_NAEHE_UNTERGRENZE` = 0,40 (`zugeordnet`, `mehrere`, `still_untergrenze`; ~~und Abstand ≥ 0,04~~ seit der Entscheidung vom selben Abend nicht mehr); ein Eintrag je Durchlauf im Pipeline-Log (Quelle `objekt_naehe`), kein Feld im Zustand.
