...
| 2 | S14a Sachlage — Wiederaufnahme, Verstehen, Verlauf, kurzfristiges Ziel, Brücke, Frame-Auflöser (Scheibe 6, 28.08.2026), Plausibilität (Scheibe 7, 29.08.2026), Wissensträger und Recherche (Scheibe 8, 29.08.2026), Sprecher der gedeckten Eigenschaften (Scheibe 9, 29.08.2026), Formprüfung jedes Objektfelds und Eigenschaftsgedächtnis (Scheibe 11, 13.09.2026) | `graph/nodes/sachlage.py`, `graph/nodes/sachlage_form.py`, `graph/nodes/sachlage_resolver.py`, `graph/nodes/sachlage_plausibility.py`, `graph/nodes/sachlage_research.py`, `graph/nodes/sachlage_memory.py`, `memory/sachlage_history.py`, `memory/sachlage_properties.py`, `memory/kurzziel.py` | unrein (Modell, Redis, PostgreSQL) |
| 2 | S15 Ressourcen-Routing | `graph/nodes/router.py` | unrein (Modell, Redis, Embed-Worker, Pipeline-Log) |
...
**Rechnung:** Je Ziel `similarity × motivation`; über `GRAVITATIONS_SCHWELLE` gilt es als aktiviert — **ein `kurzfristig`-Ziel (S14a) ist seit dem 28.08.2026 per Bauart aktiviert, solange es lebt, und steht vorn:** sein Zielsatz liegt zur Nutzeräußerung bei Kosinus 0,13–0,41, Stärke 0,09–0,29, die Schwelle hätte es nie passiert; sein Tor ist der Verfall (3 h Halbwertszeit) — und der wird beim Laden gerechnet, nicht aus dem Tagesfeld gelesen (`[gemessen]` 28.08.2026: mit dem Tagesfeld allein hätte es ~25 h gelebt). Der `gravitationsterm` ist die Summe der Aktivierungsstärken, skaliert mit `GRAVITATIONS_SALIENZ_FAKTOR` und **seit dem 09.09.2026 auf [0, 1] normiert** (`sin^0.5`, `GRAVITATIONSTERM_CAP` = 6,0, aus der gemessenen Spanne abgeleitet: max 5,9955, P99 5,8834). Roh war die Summe unbeschränkt und brach an beiden Lesern die Skala — `GRAVITATIONSTERM-OHNE-OBERGRENZE`.
...
**Rechnung:** Acht Stufen, seit dem 13.09.2026 neun. **(0) Wiederaufnahme** (Scheibe 5, seit 28.08.2026 abends) — auf dem rechnenden Weg die nächste `sachlage_verlauf`-Zeile des Paares zum Prompt-Embedding des Enrichers (`state["prompt_embedding"]`), unter Ausschluss des aktuellen Themas, über `SACHLAGE_WIEDERAUFNAHME_MIN_KOSINUS` = 0,35; ein Treffer geht als *frühere Sachlage* in den Call und als `wiederaufnahme` ins Artefakt. **(1) Verstehen** — ein Chat-Call (T 0,1, erzwungenes JSON) schreibt die Sachlage fort: `thema`, `gegenstand`, `nutzerziel`, `ausdrucksweise`, Referenzobjekte mit gedeckten und offenen Eigenschaften; latente Objekte verlieren ihre offenen Eigenschaften in der Prüfung, nicht im Prompt; **seit dem 13.09.2026 hält die Prüfung jedes Objektfeld** (`graph/nodes/sachlage_form.py::normalize_object_form`: `gedeckt` als Zuordnung mit Wert — ohne Wert, als Kanonwort oder als Vorlage `nutzer|nova` ist eine Eigenschaft offen, eine Angabe im Namen wird geborgen —, `offen` ohne Gedecktes, `akut` als Wahrheitswert, `klasse` im Kanon, die Felder des Servers aus dem Parse entfernt; **seit dem 16.09.2026 der Name ohne Auszeichnung**, `plain_object_name`), führt gleichnamige Objekte zusammen (`merge_same_name_objects`) und wendet die Smalltalk-Schranke erst danach an; dieselbe Prüfung läuft beim Laden der vorigen Blase (`sachlage_load`, ohne lesbare Objektliste laut frisch) und im Lesepfad des Kontext-Tabs; `thema` und `gegenstand` benennen die Sache, nie den Wechsel (Prompt-Regel seit 28.08.2026 abends, 5/5 vorher »Themenwechsel«, 5/5 nachher die Sache). Jeder Rückkehrpfad trägt `herkunft` (frisch · fortgeschrieben · verfallen_neu · impuls_uebernommen · ausfall_uebernommen). **(2) Verlauf** — jedes gerechnete Artefakt wird als Faktum in `sachlage_verlauf` abgelegt (Vektor über den Gegenstand-Satz, kein Verfall). **(3) Kurzfristiges Ziel** — je akutem Objekt zählt eine Strecke in Redis die aufeinanderfolgenden gerechneten Lagen; bei zwei entsteht ein `ziel_typ='kurzfristig'` in `ziele` (Anker `KURZZIEL_MOTIVATION` = 0,7), kein zweites, solange das Objekt steht; eine neue Blase setzt zurück. **(4) Brücke** — nur auf dem Impuls-Turn: die Verlaufszeile des Auslösers über die harte `turn_id`, sonst die ähnlichste Zeile des Paares über das nachgerechnete Impuls-Embedding (Schwelle `SACHLAGE_BRUECKE_MIN_KOSINUS` = 0,35), als Rückfall markiert. **(5) Frame-Auflöser** (Scheibe 6, seit 28.08.2026 spät, `graph/nodes/sachlage_resolver.py`) — auf dem rechnenden Weg ein nummeriertes Angebot aus dem Gedächtnis-Pool des Turns (Kalender zu akuten Objekten der vorigen Blase, `memory_entries` der Quellen kzg/lzg/plugin_wissen nach Gewicht, Aufzeichnungen; gekappt auf `SACHLAGE_BESTAND_MAX_EINTRAEGE` = 8), dann ein eigener Call (`sachlage_aufloeser`, T 0) nur mit den offenen Eigenschaften der akuten Objekte und dem Angebot; nur Treffer auf angebotene Einträge und offene Eigenschaften decken, mit Quelle in `quellen`, geerbt über die Fortschreibung; **seit dem 13.09.2026 stehen vor dem Pool die aktiven gespeicherten Werte der akuten Objekte mit offenen Eigenschaften** (`property_hits`, Quelle `eigenschaft`, nie eine schon gedeckte, gekappt auf `SACHLAGE_EIGENSCHAFT_ANGEBOT_MAX` = 4, `combine_offer` nummeriert neu), und eine Deckung aus ihnen übernimmt den gespeicherten Wert wörtlich — **seit der zweiten Kontrolle (13.09.2026, 15:29 UTC) nur an seiner eigenen Eigenschaft** (`MemoryHit.subject`; vorher deckte *»Alter: 970 Jahre«* die Masse), jeder Anspruch läuft durch denselben Wertprüfer wie die Form (`covered_value`, ohne Wert verworfen), und Namen werden mit dem Schlüssel der Datenbank verglichen (`casefold`). **(6) Plausibilität** (Scheibe 7, seit 29.08.2026, `graph/nodes/sachlage_plausibility.py`) — bei akutem Objekt ein Call (`sachlage_plausibilitaet`, T 0) über die Äußerung gegen Weltwissen; nur `frage_wert`, `konflikt`, `unmoeglich` mit Behauptung bleiben (`plausibilitaet` je akutem Objekt, höchstens drei), Objektzuordnung wörtlich, per Enthaltensein, sonst dem einzigen akuten Objekt. **(7) Wissensträger und Recherche** (Scheibe 8, seit 29.08.2026, `graph/nodes/sachlage_research.py`) — der Sachlage-Call gibt jeder offenen Eigenschaft ihren Träger (`nutzer` / `welt` / `nachschlagen`, gegen den Kanon gehalten, fehlend = `nutzer`); `question_target` nimmt nur `nutzer`, `answer_targets` die anderen; für die erste `nachschlagen`-Eigenschaft eine Websuche je Turn (`SACHLAGE_RECHERCHE_MAX_TREFFER` = 3), Treffer in `recherche`. **Seit dem 29.08.2026 mittags im selben Call der Sprecher je gedeckter Eigenschaft** (Scheibe 9: `nutzer` / `nova`, `_normalize_speakers` gegen den Kanon, `carry_speakers` erbt aus der vorigen Blase, `speaker_lines` im Block) — rein, keine Suche. **(8) Eigenschaftsgedächtnis** (Scheibe 11, seit 13.09.2026, `graph/nodes/sachlage_memory.py`, `memory/sachlage_properties.py`) — nach der Verlaufszeile (2), nur auf den gerechneten Wegen: je Objekt eine Zeile in `sachlage_objekt`, je Turn `sachlage_objekt_turn`, je gedeckter Eigenschaft in `sachlage_eigenschaft` neu, bestätigt (derselbe Wert) oder abgelöst (anderer Wert — der alte `aktiv = FALSE`, `t_invalid`, `abgeloest_durch`), in einer Transaktion; eine verschwundene Eigenschaft bleibt aktiv. Ungebundene Objekte werden gegen die Entitäten der Magnete ihrer jüngsten `SACHLAGE_BINDUNG_TURN_FENSTER` = 10 Turns gehalten (KZG-Hash, sonst LZG-Knoten; gleich oder Enthaltensein ab vier Zeichen, Satzzeichen als Leerraum, nie bei zwei Kandidaten); ein Zeitanker nur, wenn die Magnete dieses Turns genau einen Timeline-Eintrag nennen und der Wert einen Tag oder eine Uhrzeit trägt. Ein Ausfall ist laut und kostet den Turn nicht.
...
**Reinheit:** unrein — Modell, Redis, PostgreSQL, Embed-Worker. Rein sind `_validate_artifact`, `normalize_object_form`, `merge_same_name_objects`, `match_entity`, `combine_offer`, `sachlage_block`, `sachlage_bridge_block`, `normalize_object_name`, `build_short_goal_sentence`.
**Prüfstand:** `test_sachlage.py`, `test_sachlage_bruecke.py`, `test_sachlage_verlauf_schema.py` (live), `test_sachlage_kette.py`, `test_kurzziel.py`, `test_sachlage_resolver.py` (Scheibe 6), `test_sachlage_plausibility.py` (Scheibe 7), `test_sachlage_traeger.py` (Scheibe 8), `test_sachlage_form.py` (Formprüfung, 13.09.2026), `test_sachlage_properties_schema.py` (live), `test_sachlage_memory.py` und `test_sachlage_recall.py` (Scheibe 11).
...
| `ohne_profil_ids` | **welche** das waren — seit dem 06.09.2026. Die Zahl allein sagt, dass die Faszination leer blieb, nicht woran; damit lässt sich später nicht nachsehen, ob die Lücke geschlossen ist. Der Profil-Erzeuger holt täglich 20 Träger auf, und ob er die **gelesenen** trifft, ist genau diese Frage |
...
**Rechnung:** Der Router entscheidet über Gedächtnis-, Web- und Timeline-Bedarf und erkennt Management-Intents; ein wartender Agent überspringt den Modellaufruf. Der Planner findet den zuständigen Manager über ~~vier Prioritätsstufen (Timeline-Flag, Intent, Zielname, Auffangbecken)~~ → **seit dem 17.09.2026 eine Reihenfolge**: die Dienste, deren Merkmal ein akutes Objekt erreicht (nach Nähe), dann Intent, Zielname, Auffang; ein Treffer ohne Merkmal zuerst; nach einer Ablehnung der nächste (Scheibe 12 D1) — und baut aus den Agent-Ergebnissen einen fertigen `[AUFGABE]`-Block. Die Priorität ist geschlossen: Rückfrage vor Fehler vor Ablehnung vor Erfolg vor Alt-Management — **seit dem 17.09.2026 mit einer Ausnahme:** Folgt einer Ablehnung ein Erfolg, Fehler oder Verwerfen eines anderen Dienstes, steht dessen Ausgang im Block.
...
**Seit dem 16.09.2026 davor der Schatten** (Scheibe 12 C2, `agents/object_nearness.py::shadow_nearness`): je akutem Objekt der Sachlage der Kosinus des Objekttexts zu jedem Objekt-Merkmal; Empfänger ist jeder Dienst mit Nähe ≥ `OBJEKT_NAEHE_UNTERGRENZE` = 0,40 (`zugeordnet`, `mehrere`, `still_untergrenze`; ~~und Abstand ≥ 0,04~~ seit der Entscheidung vom selben Abend nicht mehr); ein Eintrag je Durchlauf im Pipeline-Log (Quelle `objekt_naehe`), ~~kein Feld im Zustand~~ → seit dem 17.09.2026 `objekt_urteil`, das der Planner liest; **dazu der `[LAGE]`-Block im Router-Prompt** (akute Objekte, Bekanntes, Dienste — nicht auf Impuls-Turns, nicht nach Sachlage-Ausfall) und **`objekt_bezug`** für den gefragten Dienst (D2).

**Reinheit:** Router unrein; rein sind `_build_task_block` und die fünf `_build_task_*`, dazu `build_object_text`, `cosine` und `judge`.
...
**Rechnung:** Von einer Grundlänge 1,0 ausgehend addieren oder subtrahieren Emotion samt Arousal, Beziehungsdynamik, Modus (`GV_LAENGE_MODUS_DELTA`, alle zehn Modi) und Sprachstil. Das Ergebnis wird auf `[0, 3]` beschränkt. Eine Krise setzt sofort 0 — nur Empathie, keine Antizipation. ~~**Die obere Schranke ist `[0, 3]` nur nominal** — in den drei Modi mit Zuschlag −0,3 ist die beste Summe exakt 2,5, und `round` rundet zur geraden Zahl.~~ → **Am 12.09.2026 behoben:** Die Rundung geht zur nächsten Zahl (`math.floor(laenge + 0.5)`), die Schranke `[0, 3]` gilt wieder für alle zehn Modi. Nachgemessen über 1434 Rohturns: 141 statt 116 erreichen 3, 80 statt 123 bleiben bei 0, die Tor-Quote unverändert 36,6 %.
...
**Rechnung:** ~~Kandidaten mit Similarity über 0,20 aus beiden Speichern.~~ → **Seit dem 12.09.2026 Themen statt Sätzen:** die zehn nächsten Einträge je Speicher mit `beobachter = 'assistant'` (Similarity über 0,20), ihre Themen einzeln, ohne jene, die der Nutzer im Bestand des Paares berührt hat oder die im Gespräch samt laufendem Reiz gefallen sind; je Thema ein Vektor (Stapel, zwischengehalten), daraus Nähe zum Turn und Resonanz zum Kern. Bereits Erwähntes fällt über einen Token-Overlap von mehr als 40 % gegen die letzten acht Turns heraus, zu Ähnliches über eine Obergrenze. Die Relevanz ist ein Produkt aus sechs Systemen: `similarity × gewicht_rang × quellen_faktor × (1 + neugier_boost) × aufnahmebereitschaft × register`. **`gewicht_rang` seit dem 12.09.2026:** der Rang des Gewichts in der eigenen Quelle (`ei/source_weights.py`), weil LZG-Gewicht (3–10) und KZG-Salienz (0–1) roh auf verschiedenen Skalen ins selbe Produkt gingen und das LZG allein dadurch 77 von 83 Plätzen besetzte. Am Ende Deduplizierung über Token-Overlap und Kappung auf `GV_LUECKEN_MAX`.
**Was *berührt* heißt** (`ei/gap_topics.py::touched_by_user`): Der Themenbestand des Nutzers sind die Themen aller Einträge des Paares mit `beobachter = 'user'`, aus dem LZG (`lzg_knoten.themen`) und dem KZG (Suche auf `@beobachter:{user}`), zwischengehalten für `GV_GEWICHT_VERTEILUNG_TTL_S`. Ein Thema gilt als berührt, wenn es wörtlich (ohne Groß-/Kleinschreibung) darin steht **oder** mindestens `TOUCHED_SHARE` = 2/3 seiner Wortstämme im Wortschatz des Nutzers vorkommen; Wortstamm heißt: Wörter über drei Zeichen, klein, auf `STEM_LENGTH` = 7 Zeichen gekürzt. Ein Thema ohne solche Wörter gilt nur bei wörtlichem Treffer als berührt. Die Themen eines Eintrags trennt `split_topics` am Komma — auch innerhalb einer Klammer (Fundliste 12.09.2026).
...
**Rechnung:** Der Thinker prüft über einen Indikator-Schnellcheck, ob überhaupt nachgedacht wird, und läuft dann als ReAct-Schleife mit fünf Werkzeugen und höchstens fünf Iterationen; ein zweistufiger Cache verhindert Werkzeugschleifen. Das Tribunal lässt drei Agenten unabhängig bewerten und übersetzt ihren Score über konfigurierte Schwellen in ein Votum; der Jurist trägt zwei Scores, und der strengere gewinnt. Zwei Ablehnungen ergeben `ablehnen`, zwei Warnungen oder mehr `warnung`, sonst `ok`. **Danach heben drei gerechnete Prüfungen das Urteil auf mindestens `warnung`** — kein Modellurteil, sondern Python: ein Wochentag, der nicht zu seinem Datum passt; ein Datum, das kein Dienst dieses Turns gemeldet hat (beide `utils/datum_pruefung.py`); und **seit dem 15.09.2026 eine Speicherbehauptung ohne abgeschlossenen Dienst** (`utils/storage_claims.py`, Scheibe 12 A des Lage-Konzepts). Höchstens zwei Korrekturrunden.
**Reinheit:** unrein; rein sind `_score_to_vote`, `_score_to_vote_direktive`, `find_storage_claims`, `uncovered_claims`, `_extract_corrected_response`, `_extract_issues`, `_format_faktencheck_treffer`, `_build_verarbeitungs_block`. ~~`evaluate`~~ → **seit dem 15.09.2026 nicht mehr rein:** Die Speicherprüfung schreibt je Durchlauf einen dauerhaften Eintrag (`pipeline_log`, Quelle `speicherbehauptung`), auch im stillen Fall.
