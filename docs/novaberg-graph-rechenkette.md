...
**Rechnung:** Von einer Grundlänge 1,0 ausgehend addieren oder subtrahieren Emotion samt Arousal, Beziehungsdynamik, Modus (`GV_LAENGE_MODUS_DELTA`, alle zehn Modi) und Sprachstil. Das Ergebnis wird auf `[0, 3]` beschränkt. Eine Krise setzt sofort 0 — nur Empathie, keine Antizipation. **Die obere Schranke ist `[0, 3]` nur nominal** (12.09.2026): In den drei Modi mit Zuschlag −0,3 (`fachgespraech`, `lernmodus`, `philosophischer_austausch`) ist die beste erreichbare Summe exakt 2,5, und `round` rundet zur geraden Zahl — dort ist die Decke **2**. `[gemessen über 1434 Rohturns]` 116 erreichen 3, keine davon in diesen Modi, in denen 794 der Turns liegen. Geführt als `GV-LAENGE-RUNDUNG-ZUR-GERADEN`.
