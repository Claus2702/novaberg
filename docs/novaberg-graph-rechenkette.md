...
**Seit dem 16.09.2026 davor der Schatten** (Scheibe 12 C2, `agents/object_nearness.py::shadow_nearness`): je akutem Objekt der Sachlage der Kosinus des Objekttexts zu jedem Objekt-Merkmal; `zugeordnet` bei größter Nähe ≥ `OBJEKT_NAEHE_UNTERGRENZE` = 0,40 und Abstand zur zweitgrößten ≥ `OBJEKT_NAEHE_ABSTAND` = 0,04; ein Eintrag je Durchlauf im Pipeline-Log (Quelle `objekt_naehe`), kein Feld im Zustand.
