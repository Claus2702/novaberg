...
> **Die Sektorfrage löst Synonyme auf — seit dem 06.09.2026.** `EMOTION_SEKTOR_MAP` trägt nur die 16 Kanonwerte. Ein gültiges Synonym fand dort nichts und lief als *unbekannter Wert* in die Warnung, obwohl sein Sektor feststand: `sektor_faktor("mitgefuehl")` gab `(1.0, None)`, **nachdem** die Synonymkarte den Wert bereits kannte `[gemessen 06.09.2026]`. Dieselbe Lücke saß im Strang-Histogramm, wo ein Synonym in `unbekannt` fiel statt mitzufärben, und die Valenzrechnung es übersprang. Beide Nähte lösen jetzt auf — so wie `ei/berechnung.py::emotion_kanonisieren` und `lzg_knoten.py:865` an ihren eigenen schon taten.
>
> **Die Trennung von Regelfall und Befund hält:** `neutral` ist kanonisch und sektorlos und meldet nicht; ein Wert außerhalb von Kanon **und** Synonymkarte meldet weiter.
...
**Seit dem 06.09.2026 nennt sie auch die Träger *ohne* Profil einzeln** (`ohne_profil_ids`), nicht
nur ihre Zahl. Der Anlass war eine Frage, die das Protokoll nicht beantworten konnte: Der
Profil-Erzeuger holt täglich 20 Träger auf — trifft er die **gelesenen**? `ohne_profil: 3` sagt, dass
die Faszination leer blieb, aber nicht, woran; ohne die Kennungen lässt sich später nicht nachsehen,
ob die Lücke geschlossen ist. **Dieselbe Begründung wie bei den Modulatoren daneben:** Ohne die
Eingangsgröße ist das Ergebnis nicht nachrechenbar (`18_NACHVOLLZIEHBARKEIT`).

`[gemessen]` 06.09.2026 zum Stand: **72 von 3430** aktiven Knoten tragen ein Profil (2,1 %), Zuwachs
22 je Tag. Im **Bestand** rechnet die Faszination Werte von 0,0 bis 0,7182; im letzten **Turn**
(10:34 UTC) waren 3 von 3 gelesenen Trägern ohne Profil, die Werte blieben leer. **Bestand und Turn
sind zwei Zahlen, und nur die zweite entscheidet über die Sperre des Lesers** aus §11 des Konzepts.
