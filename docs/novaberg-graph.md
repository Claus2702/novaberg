...
| 4a | Sachlage (`sachlage_node`) | GPU | Seit 28.08.2026. Schreibt das fortgeschriebene Verstehen des Gespraechs nach `state["sachlage"]` (Gegenstand, vermutetes Nutzerziel, Referenzobjekte mit offenen Eigenschaften; je Paar in Redis, 4 h Verfall), legt es als Faktum in `sachlage_verlauf` ab, zaehlt je akutem Objekt die Strecke zum kurzfristigen Ziel und baut auf Impuls-Turns die Bruecke zum Ausloeser nach `state["sachlage_bruecke"]`. **Seit 13.09.2026** haelt eine Formpruefung jedes Objektfeld (`sachlage_form.py`, auch die vorige Blase beim Laden), und die Objekte jedes gerechneten Turns stehen mit ihren Werten dauerhaft in `sachlage_objekt` / `sachlage_objekt_turn` / `sachlage_eigenschaft` — ein neuer Wert loest ab, die Entitaet wird ueber die Magnete des Turns gebunden, der Frame-Aufloeser bekommt die gespeicherten Werte angeboten (Scheibe 11, `sachlage_memory.py`). Vor dem Router, damit beide Pfade dasselbe Verstehen sehen. `novaberg-thinking-lage_k.md`, Rechenkette S14a. |
...
| `angebot_kandidat` | `dict` | Verfasser (`_offer_block`, Scheibe 12 E2) | Dispatcher (`_angebot_merken` → `offer_binding`). Die Sache, die der Verfasser in diesem Turn anbieten ließ (`name`, `dienst`); sonst leer. Das erkannte Angebot wird daran gebunden, nicht an alle akuten Objekte. Seit 18.09.2026. |
...
| `gravitationsterm` | `float` | Salienz-Boost aus Ziel-Gravitation, **[0, 1]** seit dem 09.09.2026 (`GRAVITATIONSTERM-OHNE-OBERGRENZE`). |
...
| `zielsog_roh` | `float` | Die staerkste **ungetorte** Zielstaerke des Turns (`similarity x motivation`, ohne `GRAVITATIONS_SCHWELLE`). Sie zieht die Salienz von Novas Aeusserung auf die Luecke nach oben. **Getrennt vom Term, weil eine Zahl sonst zwei Wirkungen haette:** Der Term entsteht aus den *aktivierten* Zielen, und dieselben Ziele stehen im `[GEDANKEN]`-Block des GV-Prompts — wer die Schwelle fuer die Salienz senkte, aenderte zugleich, worueber Nova nachdenkt. Seit 01.09.2026 |
