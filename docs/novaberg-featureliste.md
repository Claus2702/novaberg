...
**Verlauf:** [Verlauf des Standes](#verlauf-des-standes) — 58 Eintraege, juengster zuerst
...
| **Stapel + Zustellung** (Shadow Delivery) | ⚫ | `services/shadow_delivery.py` `[Code]` | **⚫ seit 23.08.2026** (zuvor 🔴): **der Weg ist geschlossen, nicht nur mangelhaft.** Letzter Impuls-Turn am **15.08.2026**, bei 489 Stapeleinträgen — neun Tage. Zwei Sperren hintereinander gemessen: `last_activity` hat 2 h TTL und wird nur vom Nutzer-Turn gesetzt, also gibt es ohne Gespräch **keinen Auslöser**; nach einem echten Turn feuerte er sofort wieder, und dann sperrte **Riegel 2** jeden Zyklus (`[wollen+0.55 frequenz- ruhe+]`), weil der Haltungsstand kein Führungsmaß trägt. Beides in `novaberg-fundliste.md`. Dazu unverändert `SHADOW-STACK-THEMA-LEER`, `ZUSTELLUNG-ABBRUCH-UNGEZAEHLT`, `PROMPTAENDERUNG-OHNE-STAPELWIRKUNG` offen · `LAGEBILD-IMPULS-ALS-NUTZEREINGABE` **behoben**, geprueft 20.08.2026 |
