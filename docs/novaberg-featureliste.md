...
| **Bibliothek als bestellbarer Dienst** (`wissen`, NMCP-Zettel + Dienst) | 🟡 | **Der Dienst läuft, die Trefferqualität trägt ihn nicht** (19.08.2026, siehe Zeile darunter: 20 % auf Rang 1) — `plugins/wissen_manager/manager.py` (Zettel), `agents/wissen/` (Dienst), geteilte Abfrage in `AutonomousWissenRepository.suchen`, **26 Zeugen** · **zwei echte Turns 19.08. 12:41 und 12:43 UTC**: beide über den Empfang geroutet (`Match via target 'wissen' → wissen (exakt)`), einer in den **vierten Ausgang** (0 Treffer, Bestand 242, nächste Nähe 0,3084), einer **abgeschlossen** mit 5 Treffern `[gemessen]` | die **Schwelle trennt nicht** (siehe Zeile darunter); der Wortlaut der Datei fehlt — Stufe 2 ist nicht gebaut, und der Dienst sagt es in seiner `grenze` |
| **Retrieval der Bibliothek** (Ziel des Vergleichs, nicht die Schwelle) | 🔴 | **Die Ampel stand bis zum 19.08.2026 an der Schwelle und damit am falschen Gegenstand.** 40 Fragen gegen 249 Einträge mit bekannter richtiger Antwort: **8 auf Rang 1 — 20 %**; der beste Fehltreffer liegt im Median näher als der richtige (0,4781 gegen 0,4045). Bei 0,40 werden **gleichzeitig** 50 % der richtigen verworfen und 80 % der Fehltreffer durchgelassen, und keine andere Zahl schafft beides — beide Verteilungen überlappen vollständig. **Eine Schwelle entscheidet, wie viel einer Rangfolge durchkommt; herstellen kann sie sie nicht.** Ursache ist das Embedding-Ziel: `themen_embedding` trägt das **Destillat** (Ø 552 Zeichen) statt des Themas (Ø 110). Gegen ein Thema-Embedding: **39 von 40**. Grenze der Messung mitgeschrieben — die Frage trägt das Thema wörtlich, die 20 % sind deshalb eine **Untergrenze** `[gemessen]` | die Schwelle ist **übernommen, nicht gemessen** (`config.py`, Backlog `WIS-SCHWELLE-MESSEN`) — sie sortiert an diesem Korpus genau falsch herum |
