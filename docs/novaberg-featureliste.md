...
| **Multi-Channel — Matrix + WireGuard** | 🟢 | `matrix_bot/` (Connector, 21 eigene Zeugen) · Synapse als Compose-Dienst **auf Postgres** · `novaberg-matrix-kanal_k.md` · **im Betrieb 23.08.2026**: Raumverlauf mit zwei Absendern und ohne `[Du]`-Praefix, Client verbunden, Turns aus der App in der Ereignistabelle | **7 von 8 Arbeitspaketen.** TLS (4) ist **zurueckgestellt, nicht erledigt** — ohne ihn gehen im WLAN Passwort und Text im Klartext. **Keine Zeugen in der Server-Suite**: Der Connector laeuft als eigener Dienst und wird von `discover` nicht erfasst (Fundliste 23.08.2026) |
