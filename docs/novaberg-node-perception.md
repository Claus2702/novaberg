...
> ⚠ **Befund 14.09.2026: Jeder Beitrag erreicht die Perzeption nur in seinen ersten 100 Zeichen** (`max_chars = 100`, von hier nicht überschrieben). Novas Antworten beginnen häufig mit einer Regieanweisung — `[gemessen 14.09.2026]` 157 bis 208 Zeichen in 4 von 4 Antworten; die Perzeption sah von ihnen nur deren Anfang. Wirkung auf die Wahrnehmung nicht gemessen. Fundliste 14.09.2026, Abhilfe als Teil B von Scheibe 12 (`novaberg-thinking-lage_k.md` §4). → **Behoben am 16.09.2026:** Die Perzeption bekommt jeden Beitrag des Fensters vollständig; eine Grenze greift nur noch über das ganze Fenster und wirft älteste Gruppen ganz weg (`novaberg-mem-session.md` §3.7). `[gemessen]` Über 1439 Fenster des Bestands fällt die Zahl mitten abgeschnittener Beiträge von 7213 auf **0**. **Die Wirkung auf die Wahrnehmung ist weiterhin nicht gemessen** — der Satz oben gilt unverändert, nur mit umgekehrtem Vorzeichen: Was die Perzeption aus dem ganzen Beitrag anders liest als aus seinen ersten 100 Zeichen, hat niemand erhoben.
