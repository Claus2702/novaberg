...
**Pfad:** novaberg/docs/nova-node-router.md
...
→ Perzeption (liefert Kontext): nova-node-perception.md
→ Enricher (nutzt Flags): nova-node-enricher.md
→ Planner (nutzt Management-Felder): nova-node-planner.md
→ Plugin-System (erweitert Prompt): nova-architecture.md
→ Shadow Delivery (nutzt Momentum): nova-pixie.md
→ NotizenAgent (Resume-Flow, gewichtete Suche): nova-agent-notes.md
...
→ Lesson: "NIEMALS" ist kein Proxy: nova-node-router_l.md
