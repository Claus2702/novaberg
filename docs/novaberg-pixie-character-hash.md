...
>
> **Seit dem 06.09.2026 führen alle fünf Prompts die `ZITATREGEL`** — Setzung des Eigentümers: *Das Modell darf nichts zitieren, was nicht wirklich gesagt wurde* (`F-ZITAT-1`). Sie führt positiv: Anführungszeichen sind für den Wortlaut reserviert, eigene Deutungen stehen daneben. `[gemessen]` mit **n = 20 je Fassung in zwei Runden**: **24 % → 10 %** und **21 % → 9 %** unbelegte Zitate. **Die erste Prompt-Änderung dieses Tages, die über die Streuung hinaus wirkt** — drei andere hatten es nicht getan. **Sie erfüllt die Setzung trotzdem nicht:** rund ein Zehntel bleibt, und der strukturelle Schritt (ein unbelegtes Zitat im gespeicherten Profil entwerten) ist ein Eingriff in den Text und eine eigene Entscheidung.
>
