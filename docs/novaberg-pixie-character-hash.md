...
### 2a. Die Audit-Spur — seit dem 06.09.2026

**Der Agent hinterließ bis dahin keine Zeile im `hintergrund_log`.** Aufgefallen ist es beim Zählen:
Die Frage *wie oft wird eigentlich destilliert* war aus der Spur nicht zu beantworten, die es dafür
gibt — die Zahl musste über das Serverlog und die Rad-Messreihe rekonstruiert werden, also über zwei
Nebenwirkungen.

| Zustand | Wann | Was drinsteht |
|---|---|---|
| `gestartet` | nach dem Dirty-Check, je Paar | das Paar, und dass `hash_dirty` gesetzt war |
| `erledigt` | am Ende des Paar-Durchlaufs | Zahl der destillierten Profile, und ob gespeichert wurde |
| `fehler` | jede Exception aus dem Lauf | Typ und Text; die Exception wird **weitergereicht** |

**Der Leerlauf schweigt, und das ist Absicht.** Der Agent prüft alle zehn Minuten und findet meist
kein `hash_dirty`; ein Eintrag je Prüfung wären 144 Zeilen am Tag ohne Aussage, in einem Log mit über
50 000 Zeilen. Dasselbe Muster wie `ziel_decay`, der bei `aktiv: False` ebenfalls nichts schreibt.
Ein eigener Zeuge hält es fest — die Sparsamkeit sieht sonst im Betrieb aus wie mehr Sorgfalt und
geht beim nächsten Umbau verloren.

**Der Fehlerfall sitzt in einem Wrapper um `invoke`.** Der Pixie-Dispatch fängt eine Exception und
schreibt sie ins Log, aber nicht ins `hintergrund_log`; ohne den Wrapper bliebe ein abgebrochener
Lauf dort als `gestartet` ohne Abschluss stehen, und ein Lauf, der vor dem Dirty-Check scheitert,
ganz ohne Zeile. Die Exception wird weitergereicht: Über Retry und Wiedervorlage entscheidet der
Dispatch.

> **Ein Zeuge, den der Umbau rot gemacht hat, war der Beleg dafür, dass er wirkt.**
> `tests/test_aktives_paar.py` liest die Paarliste aus dem Quelltext von `invoke` — nach der
> Umbenennung fand er sie nicht mehr. Dabei kam heraus, dass sein Nachbar-Zeuge (*der Fallback steht
> nicht in der Paarliste*) **grün geblieben wäre**: Eine leere Liste erfüllt jede Aussage der Form
> *darin steht kein X*. Beide lesen jetzt `_profile_destillieren`, und der Leser prüft zuerst, ob er
> überhaupt etwas gefunden hat.
...
> **Der durchgestrichene Halbsatz ist am 06.09.2026 widerlegt.** Pixie **prüft** alle zehn Minuten und destilliert nur, wenn `hash_dirty` gesetzt ist (`agents/charakter/agent.py:158`) — im Betrieb sind das rund **zwei bis vier Zyklen je Tag**, aus zwei unabhängigen Quellen gezählt: die Rad-Messreihe trägt im Median **4 Erhebungen je Tag** (24.08.–06.09., zwei Rad-Arten je Zyklus), die Mitschrift dieser Reihe **2,6 Fassungen je Tag**. **Der 03.09. trägt in beiden Quellen null.** Für den Sammel-Takt heißt das: Er greift **dichter ab, als neue Fassungen entstehen** — die Reihe verliert nichts, sie holt nur oft dasselbe. Die Begründung des Takts war trotzdem falsch, und der Fehler ging in eine Messung ein (`labor/2026-09-06_rueckkopplung_ergebnis.md` §5).
...
### 3.1h Die Kennzahl kann die Frage nicht beantworten — und das Ziel des Kerns ist seit dem 27.08.2026 entschieden

**Die Absicht (27.08.2026):** Der Kern soll die **Person** beschreiben — das WIE — und ist faktisch der System-Prompt der Figur: Aus den Turns wird die Rollenanweisung extrahiert, mit der ein Schauspieler sie spielt. Was die fuenf Haltungsgroessen regeln (Fragehaeufigkeit, Smileys, Umfang — Sache der Landschaft), regelt der Kern **nicht**: Kein Psychologe schreibt eine Quote in die Akte.

**Die Kennzahl der Messreihe kann diese Frage nicht beantworten.** Gemessen am 27.08.2026 (`labor/2026-08-27_kern_wortherkunft.py`, ohne Modellaufruf): **44–52 % des Kern-Wortschatzes kommt im Rohmaterial nicht vor** — erschlossen, nicht abgeschrieben (woertliche Zitate: 2,8 % des Textes). Die Ueberdeckungs-Kennzahl aus §3.1g zaehlt aber den **gesamten** Inhaltswortschatz, und dessen andere Haelfte ist paarspezifischer Gespraechsstoff — per Konstruktion verschieden. *„Suche nach Kohaerenz in der Entropie"* und *„Suche nach Ordnung im Geschmackschaos"* sind derselbe Zug und teilen kein Inhaltswort. **Die Kennzahl bliebe auch bei einem perfekten Kern niedrig**; `KERNHASH-TRAEGT-KEINE-PERSON` ist damit nicht widerlegt, aber seine Messgrundlage ist als unzustaendig erkannt.

**Das zustaendige Instrument ist die Schauspielerprobe** (`labor/2026-08-27_schauspielerprobe.py`): Vorlage (echte Antworten) gegen einen Schauspieler, der nur den Kern kennt, gegen einen ohne Kern — sechs Aeusserungen ueber drei Register, T=0, gezaehlte Zuege. Erster Lauf: Der bestehende Kern macht die Figur **kalt und doppelt lang** (984 gegen 414 Zeichen Median; das eine genannte Merkmal ueberschiesst 33 %→100 %) — **die Form des Kerns faerbt ab**: Ein Psychologenbericht als Rollenanweisung erzeugt einen Schauspieler, der einen Psychologen spielt. Ein Gegenentwurf als Rollenanweisung (`labor/2026-08-27_kern_als_rollenanweisung.py`) drehte alles ins Verspielte — auch die Fachfrage.

> **Beide Urteile stehen auf einem unvollstaendigen Pruefstand, und deshalb ist der Umbau zurueckgestellt:** Die Probe gab dem Schauspieler den Kern **ohne** die Landschafts-Vorgaben (`[MASS]`-Block) und mass dann Zuege, die im Betrieb die Landschaft steuert. Vor einem neuen Kern-Prompt wird die Probe um den `[MASS]`-Block je Register vervollstaendigt; die Zuege taugen dann nur noch als Kontrolle, das Urteil faellt eine Modell-Gegenueberstellung. Bis dahin gilt die Grenze: **Der Kern beschreibt Wesen und Antrieb, nicht die fuenf Haltungsgroessen.**
...
> **Seit dem 06.09.2026 trägt der offene Prompt einen Richtwert: rund 3000 Zeichen, höchstens 4000.** Das ist **kein zurückgekehrter Deckel**, sondern die Korrektur einer Drift, die der Modellwechsel gebracht hat. Der Prompt stand unverändert; gemessen wurde am produktiven Paar: **12.08.2026 mit dem Nahmodell 3578 Zeichen — 06.09.2026 mit dem Fernmodell 8587** (Figur) und **7381** (Mensch). Die Gegenprobe vom 11.08.2026, die den Deckel fallen ließ, hatte die offene Fassung bei **3288 und 2545** Zeichen gemessen. Der Richtwert holt den Text also **in den Korridor zurück, in dem die Messung ihn für gut befunden hat** — die beiden Sätze, die den Wortlaut verlangen (»Nimm dir den Raum«, »Verdichte nicht«), stehen unverändert daneben, und ihre Zeugen bleiben grün.
>
> **Warum das zählt:** Novas vier Profile lagen zusammen bei **12 289 Zeichen** und gehen ungefiltert in den `[PERSON A]`-Block **jedes** Turns. Ein Profil, das dreimal so lang ist wie die gemessene Fassung, ist keine dreifache Tiefe — es ist dieselbe Aussage mit mehr Belegen.

> **Am 06.09.2026 hat der Eigentümer eine zweite Frage an denselben Prompt entschieden: die Verallgemeinerung ist nicht gewünscht.** Der Kern-Prompt erlaubt sie ausdrücklich — *„ein Zug, der einmal belegt ist, braucht keinen zweiten Beleg"* (`destillation.py:387`) —, und keine der neun Deutungsebenen der vier Profil-Prompts verlangt ein Maß für Häufigkeit oder Deckung. Am Bestand gemessen: Das Beziehungsprofil nennt eine Anrede *„durchgehend"*, die in **4 von 1662** Quellen steht (0,24 %). Zwei offene Kennungen halten das fest: `PROFIL-VERALLGEMEINERT-EINZELBELEG` (das Maß fehlt) und `PROFIL-VERSTAERKT-WAS-ES-BESCHREIBT` (der so entstandene Zug geht in jeden Turn, prägt die nächste Antwort und damit das Material der nächsten Destillation).
>
> **Der Weg über den Prompt-Wortlaut ist am selben Tag gemessen und ausgeschlossen.** Vier Fassungen — das Maß benennen, das Zählen verlangen, an den Ausschnitt binden, das Wort *durchgehend* aus dem Satz zur Pronomenwahl nehmen — bewegen bei **n = 20** nichts über die Streuung hinaus: Die unveränderte Fassung liegt bei **8 bis 10 von 20 Läufen** mit mindestens einem Dauerwort und beide Runden bei **11 Dauerwörtern gesamt**. **Die Prompts stehen deshalb unverändert** — eine Fassung ohne belegte Wirkung wäre ein zweiter Satz, der dasselbe behauptet. Schärfer geworden ist dabei der Befund: Der Prompt bekommt nicht die 1662 Wissensdateien, sondern **20 Begegnungen**, und die Anrede steht dort in **1 von 20**. **Der Eigentümer hat den Ort entschieden: die Ausgabe wird geprüft.**
>
> **Seit dem 06.09.2026 hält `deckung_beanstanden` jeden wörtlichen Beleg des Profils gegen den Prompt**, der das Material im Wortlaut trägt — *„kommt im Prompt vor“* ist genau die Frage *„hatte das Modell einen Beleg“*. Zwei Klassen werden gemeldet: ein **Beleg ohne Fundstelle** (0 Vorkommen) und ein **Einzelbeleg als Dauerzug** (1 Vorkommen, im selben Satz wie *durchgehend*, *stets*, *typisch* …). **Gemeldet, nicht verworfen:** Das Profil wird gespeichert, die Logzeile sagt, worauf es sich stützt — ein Verwerfen kostet einen neuen Aufruf und lässt im Zweifel gar kein Profil stehen. `[gemessen 06.09.2026]` an 20 echten Beziehungsprofilen: **3 Läufe mit Beanstandung**, 3 von 50 Zitaten, alle aus der zweiten Klasse; ein Vorlauf 4 von 20. **Die Prüfung greift für alle fünf Profile** — am gerenderten Prompt gezählt tragen 5 von 5 wörtliche Belege (10 bis 39 je Prompt), sie ist also nirgends blind. **Ihr Anschlag im echten Betriebspfad ist am 06.09.2026, 14:55 UTC belegt:** 4 Meldungen bei 10 Profilen im ersten Pixie-Lauf nach dem Neustart.
>
> **Zwei Grenzen bleiben.** Ein Dauerwort **ohne** wörtlichen Beleg ist nicht prüfbar — es gibt nichts nachzuschlagen. Und die Klasse *Beleg ohne Fundstelle* schlägt **beim Kern häufig** an (3 und 6 Meldungen in zwei Läufen), im Beziehungsprofil über 20 Läufe **nie**: Das Modell setzt im Kern eigene Wendungen in Anführungszeichen, und die Prüfung kann erfundenes Zitat und Stilmittel nicht trennen. Offen als Absichtsfrage, in der Fundliste.
>
> **Am selben Tag gemessen: es hängt nicht am Modell.** Derselbe Kern-Prompt, dasselbe Material, je fünf Läufe in zwei Runden — `deepseek-v4-flash` **21 %** und **17 %** der Zitate ohne Fundstelle, `qwen3.7-flash` **16 %** und **24 %**; der Unterschied zwischen den Modellen ist kleiner als die Streuung eines Modells zwischen zwei Runden. `mistral-nemo` zitiert **gar nicht** und bleibt mit 766–1073 Zeichen unter einem Drittel des Umfangs-Richtwerts — null Meldungen heißt dort *nichts zu prüfen*, nicht *treuer*. **Die Ursache liegt im Auftrag** (*ein Beispiel im Wortlaut sagt mehr als ein Urteil*): Wer ihm folgt, setzt Anführungszeichen auch um eigene Formulierungen (`labor/2026-09-06_zitattreue_ergebnis.md`).
>
> **Seit dem 06.09.2026 führen alle fünf Prompts die `ZITATREGEL`** — Setzung des Eigentümers: *Das Modell darf nichts zitieren, was nicht wirklich gesagt wurde* (`F-ZITAT-1`). Sie führt positiv: Anführungszeichen sind für den Wortlaut reserviert, eigene Deutungen stehen daneben. `[gemessen]` mit **n = 20 je Fassung**: ohne das Anbieter-Pinning des Betriebs **24 % → 10 %** und **21 % → 9 %** — **gepinnt aber nur 18 % → 16 %**. **Gegen das Ziel des Betriebs wirkt sie nicht**; die Halbierung war an ein Ziel gebunden, das der Betrieb nicht benutzt. Damit ist auch diese, die fünfte Prompt-Fassung des Tages, ohne belegte Wirkung — und die Betriebszahlen des 17:11-Zyklus (36 % und 29 %) fügen sich ein, statt Ausreißer zu sein. **Der Prompt-Weg ist für diese Setzung ausgeschieden**; was bleibt, ist die Entwertung im gespeicherten Text, ein Eingriff und eine eigene Entscheidung.
>
> **Seit dem 06.09.2026 verliert ein Beleg ohne Fundstelle seine Anführungszeichen.** `zitate_entwerten` sitzt in `_llm_call` hinter der Meldung: Was das Material nicht trägt, steht ohne Anführungszeichen da — aus dem angeblichen Zitat wird die Deutung, die es ist. **Der Satz bleibt, nichts wird gelöscht und nichts umformuliert.** Der Grund für die Struktur statt des Prompts steht eine Zeile höher: Die `ZITATREGEL` bewegt gegen das Ziel des Betriebs 18 % → 16 %, und `F-PROMPT-1` sagt, was dann gilt. `[gemessen 06.09.2026]` an **20 echten Kern-Destillationen gegen das gepinnte Ziel**: **32 von 282 Zitaten entwertet**, in **14 von 20 Läufen**; danach **0** Belege ohne Fundstelle. **Kein gedecktes Zitat ging verloren** (282 − 32 = 250 gezählt) und der Eingriff fasste **64 Zeichen** an, also genau zwei je entwertetem Beleg — beides sind Zusicherungen der Funktion, nicht Beobachtungen.
>
> **Am 11.09.2026 hat der Eigentümer eine dritte Frage an denselben Prompt entschieden: das Profil beschreibt die Art des Umgangs, keine Anredeform.** Der Beziehungs-Prompt hatte sie als einziger ausdrücklich verlangt — *„NÄHE (wie {traeger} das Gegenüber anspricht — welche Anrede, welche Kosenamen)"* — und bekam sie. **Der Rückweg macht daraus eine Anweisung:** Das Profil geht über `internal.character.relationship` in den Responder-Prompt **jedes** Turns. `[gemessen]` **Ein einziges Vorkommen in 854 Zeichen Profil** stand hinter einer Eröffnungsformel in **35 von 38** Antworten des Tages (92 %) und **183 von 189** am 09.09. (97 %); die 14 häufigsten Eröffnungen dreier Tage beginnen **alle** damit. Die Anrede stand dabei im Beziehungsprofil **eines** Paares — bei 16 anderen nicht, im Kern nicht, in den Emotions- und Intentions-Profilen nicht, in den Direktiven nicht.
>
> **Die Grenze verläuft zwischen Art und Form, nicht zwischen Nähe und Distanz.** *Vertraulich, auf Abstand, zugewandt, reserviert* bleibt; *welche Anrede, welcher Titel* fällt weg. Der Prompt fragt seither nach der Art der Nähe, *„so wie ein Dritter sie am Umgang ablesen würde"*.
>
> **Der Wortlaut-Weg bleibt davon unberührt.** Dass das Beziehungsprofil aus dem **Wortlaut** der Turns destilliert wird und nicht aus dem KZG-Inhalt, war am 09.08.2026 damit begründet, dass die Anrede die Umwandlung in die dritte Person nicht überlebt. Das gilt weiter — die Anrede ist **Material** und nicht mehr **Ergebnis**: Das Modell darf aus ihr schließen, es soll sie nicht wiedergeben.
>
> **Seit dem 11.09.2026 hält `anrede_beanstanden` jeden destillierten Profiltext gegen ein Anrede-Muster** — ein Anrede-Wort (*Anrede, Kosename, Titel, adressiert, anspricht, nennt, redet*) dicht vor einer in Anführungszeichen gesetzten Wortform. **Beide Teile zusammen, und das ist Absicht:** *„spricht ihn respektvoll an"* beschreibt die Art und bleibt, *„adressiert ihn mit dem Titel X"* nennt die Form. `[Nulllinie 11.09.2026]` An den **70** gespeicherten Profiltexten (fünf Sorten, 14 Paare) schlägt das Muster **2**-mal an, beide Male dasselbe Beziehungsprofil, **kein Fehltreffer**. Die Wortgrenze trägt diesen Befund: ohne sie fing `nennt` auch `benennt` und erzeugte einen Fehltreffer im Kern.
>
> **Der Beleg steht seit dem 11.09.2026, 18:00 UTC:** Beide Beziehungsprofile des Paares neu destilliert, über denselben Hintergrund-Worker wie im Betrieb und **ohne zu speichern** — **0 Anredeformen in 2 von 2** Profilen, vorher **2 von 2**. Die Texte beschreiben die Art: *„eine intensive, fast symbiotische Nähe"*, *„zwischen sachlicher Distanz und emotionaler Verletzlichkeit"*. Damit ist der Prompt-Weg hier **nicht** ausgeschieden — anders als am 06.09.2026 gegen die Verallgemeinerung, wo vier Fassungen dieselbe Aufforderung nur umformulierten. **Hier fiel die Aufforderung selbst weg**, und das ist ein anderer Hebel.
>
> **Im Betrieb belegt am 11.09.2026, 20:05 und 20:12 UTC.** Die echte Destillation lief nach **1,36 h** Wartezeit bei Priorität 0,98 — der Verhungerungsschutz trägt, eine zwischenzeitliche Diagnose *„wird ausgehungert"* war aus 31 Minuten geschlossen und ist widerlegt. **Das Profil des Menschen trägt die Anredeform seither nicht mehr.** Novas Profil trägt sie weiter, in einer Form, die das Muster nicht fasst: *„die das Gegenüber als „X" respektiert"* — kein Anrede-Wort, sondern `als` vor dem Zitat. **Die Prüfung hat damit einen belegten blinden Fleck**, und er ist der teure Fall: Sie meldete null, wo etwas stand.
>
> **Gemeldet, nicht verworfen** — dieselbe Wahl wie bei `deckung_beanstanden` und aus demselben Grund. Sie ist zugleich das Messinstrument für die Prompt-Änderung: Bleibt die Zahl über null, ist der nächste Schritt ein Eingriff am gespeicherten Text und eine eigene Entscheidung. Zeugen: `tests/test_profil_anrede.py` (11), Gegenprobe 3 von 3 vorhergesagt.
>
> **Der Anschlag im echten Betriebspfad ist am 06.09.2026, 19:42 UTC belegt.** Ein Messturn setzte `hash_dirty`, der Heartbeat destillierte (`hintergrund_log` 52306 *gestartet* 19:42:17, 52307 *erledigt* 19:43:10, *2 Profile destilliert, gespeichert: ja*), und beide Zeilen stehen in der gebauten Reihenfolge: **4 Belege ohne Fundstelle gemeldet, dieselben 4 entwertet.** **Der Schliessbeleg steht im gespeicherten Text:** Alle vier stehen dort **ohne** Anfuehrungszeichen, die uebrigen **20 Zitate des Kerns mit** — 4 von 24 ungedeckt, also 17 %, im Band der Nulllinie. **Und alle vier sind Stilmittel, keine erfundenen Redewiedergaben** — abstrakte Wendungen, die das Modell in Anfuehrungszeichen gesetzt hatte. Genau dort ist die Entwertung richtig: Aus der angeblich zitierten Wendung wird die Formulierung, die sie ist.
>
> **Entwertet wird nur die erste Klasse.** Ein *Einzelbeleg als Dauerzug* behält seine Anführungszeichen: Sein Wortlaut **steht** im Material, zu viel behauptet das Dauerwort daneben. Das ist `PROFIL-VERALLGEMEINERT-EINZELBELEG` und bleibt offen.
>
> **Die Prüfung trennt Erfindung und Stilmittel nicht — die Entwertung muss es nicht.** Der Kasten oben nennt das als Grenze: Das Modell setzt im Kern auch eigene Wendungen in Anführungszeichen. Für beide Fälle ist dieselbe Abhilfe die richtige, weil `F-ZITAT-1` die Zeichen für den Wortlaut reserviert und nicht für die Hervorhebung.
>
> **Die Reichweite der Entwertung ist die von `ZITAT`, und die endet bei 60 Zeichen.** `[gemessen 06.09.2026]` an den **70 gespeicherten Profiltexten**: 117 von 125 Anführungspaaren liegen darin, **8 nicht** (6,4 %; Median 19, P90 55, Max 101). Die acht sind **ganze zitierte Sätze** und stehen alle im Kern — sie werden weder gemeldet noch entwertet. **Die Grenze war bis zum 06.09.2026 die Reichweite einer Logzeile und ist seither die eines Eingriffs**; ob sie dort richtig liegt, ist offen (Fundliste). Ein Anheben änderte auch `deckung_beanstanden` und damit die Vergleichbarkeit aller heutigen Quoten.
>
> **Die zweite Kontrolle hat den Bau geändert.** Die Vorbedingung *„der Profiltext enthält kein `\x00`"* stand zuerst im Docstring des Helfers und wurde vom Aufrufer erwartet — von zwei Aufrufern tat es einer. Ein eingebettetes Maskierungszeichen ließ `deckung_beanstanden` denselben Beleg **zweimal** melden. **Eine Vorbedingung, deren Einhaltung der Aufrufer zusichert, gilt nur so weit wie der aufmerksamste Aufrufer** — sie sitzt seither in `_belege_maskieren` selbst, und `_belege_klassifizieren` überspringt eine Marke ohne Beleg laut, statt zu stürzen. Im Bestand: **0 von 70** Profiltexten tragen das Zeichen, es lag nichts latent.
>
> **Die dritte gepinnte Runde derselben Fassung lag bei 11 %** gegen 16 % am Nachmittag. **Die Streuung zwischen Runden ist damit zum dritten Mal größer als jeder Unterschied zwischen den Fassungen** (`labor/2026-09-06_entwertung_ergebnis.md`).
>
> **Die Prüfung normalisiert seither vor dem Vergleich** — Anführungszeichen aller Formen, Gedankenstriche, Auszeichnungszeichen, Kleinschreibung. Der Anlass stand in den Meldungen selbst: Ein Zitat mit Sternchen und eines in einfachen Anführungszeichen standen im Material und wurden als Erfindung gemeldet. **Der Effekt ist klein** (ein bis drei Punkte), die Richtung des Fehlers ist der Grund: Ein Falschalarm würde einen echten Beleg entwerten, sobald aus der Meldung ein Eingriff wird.
