...
**Pfad:** novaberg/docs/nova-node-thinker.md
...
**Wichtig:** Der Suchbegriff wird aus der FRAGE DES NUTZERS abgeleitet, nicht aus der Antwort des Assistenten. → Lesson nova-node-thinker_l.md
...
→ Responder (liefert Antwort): nova-node-responder.md
→ Tribunal (bewertet danach): nova-node-tribunal.md
→ Timeline-Repository: nova-agent-timeline.md
→ Lesson Timezone: nova-tool-timeparser_l_timezone.md
→ Lesson Suchbegriff-Verzerrung: nova-node-thinker_l.md
→ Web-Infrastruktur: `tools/web/` (Chat 35)
→ Prompt-Schema: nova-pattern-prompt-schema.md
