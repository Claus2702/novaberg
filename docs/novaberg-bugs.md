...
**Nicht behoben.** Ein fremder Zeuge wird gemeldet und nicht im Zug eines anderen Auftrags repariert.

**Betroffen.** Diese eine Stelle ist belegt. **Nicht nachgemessen** ist, ob weitere Zeugen ihren Erwartungswert aus `time.time()` zweimal rechnen — das gehört zur Behebung, nicht zum Befund.

**Geschlossen, wenn.** Der Schlüssel wird einmal gerechnet und in einer Variablen gehalten, ein Kriterium über den Testbaum hat die übrigen Stellen derselben Bauart gezählt, und zehn vollständige Läufe hintereinander sind grün.
