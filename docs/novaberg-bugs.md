...
**Geschlossen, wenn** `expect_json` bis zum Anbieter durchschlägt — beim Ollama-Weg als `format`, beim Anthropic-Weg über das entsprechende Mittel — und ein Zeuge prüft, dass die Nutzlast es trägt. **Vor dem Bau zu klären:** `types.py` hält fest, dass `think=True` und `expect_json=True` einander ausschließen; ob das eine Eigenschaft des Anbieters oder unserer Verarbeitung ist, entscheidet, ob die Fessel überall gesetzt werden darf.
