...
**Für das Kurzzeitgedächtnis ist der Schaden seit dem 21.08.2026 begrenzt** — seine Abrufschwelle steht auf einem gemessenen Wert und lässt eine Frage ohne Gegenstand nicht mehr durch. **Das ist seit dem 21.08.2026, 23:43 UTC nicht mehr nur die Aussage des Lesepfads, sondern am echten Turn belegt** — und zwar auf dem Weg, der durch diesen Knoten führt: Eine anaphorische Rückfrage ohne aufgelösten Gegenstand liefert **0** Einträge, eine Frage zu einem nie besprochenen Gegenstand **0**, eine einschlägige **10**; dieselben drei gegen einen Server mit der alten Schwelle liefern **10 / 10 / 10** `[gemessen]`. Der Rewriter erzeugt der Frage ohne Gegenstand also keinen, der die Schwelle überspränge. Für den NMCP-Dienst `dateien`, der gar keine Schwelle trägt, gilt es unverändert.
...
- Perzeption/Router: Gekürzter Text + Emotion als Annotation (via `format_session_turns_numbered()`) — **gekürzt heißt: 100 Zeichen je Beitrag**, bei Antworten mit Regieanweisung nur deren Anfang (Befund 14.09.2026, Fundliste; `novaberg-mem-session.md` §3.7)
