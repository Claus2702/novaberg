...
| `services/event_consumer.py` | `character` | `continue` | Thinker-Selbsttrigger bei Doppel-Fehlschlag. **Erbt** die `turn_id` — es ist derselbe Gedanke, nochmal versucht. **Live belegt 14.09.2026:** Unter einer `turn_id` stehen danach zwei `turn_roh`-Zeilen, und die zweite Antwort beginnt mit dem vollständigen Text der ersten (`novaberg-node-thinker.md` §3.5). |
...
**Und seit dem 12.09.2026 in die Ausfallmeldung.** `turn_gescheitert` traegt dieselben `nachrichten_ids`; der Client nimmt sie aus seiner offenen Menge. Eine Antwort, die nur Kennungen nennt, die der Client nie bestaetigt bekam, gilt dort als Antwort an einen anderen Absender (`unbeobachtet`) — der Server stellt jede Antwort allen Clients des Nutzers zu.
...
**Zwei Erzeuger, nicht einer.** Die Zustellung legt den Wert ins Ereignis, und der **Thinker-Wiederholungsversuch** baut das Payload des Folgelaufs neu und muss ihn mitnehmen. Er liest ihn dabei ueber denselben Zugang, ueber den der Folgelauf ihn liest — eine zweite Leseart waere die Stelle, an der beide auseinanderlaufen. Laesst er ihn weg, faellt der zweite Versuch auf Novas gespeicherten Stand zurueck, **und der Ausfall ist still**: Der Zugriffsknoten meldet dann ordnungsgemaess `kein_level`, die Meldung ist richtig und ihre Ursache nicht. Von einem Stapel-Eintrag ohne Stand ist der Fall nicht zu unterscheiden.
