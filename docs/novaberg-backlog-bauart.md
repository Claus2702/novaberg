...
## Block 19.09.2026 — die NMCP-Konvention gegen die Protokollrevision `2026-07-28`

**Der Anlass ist eine Nachprüfung an der Primärquelle, nicht ein Defekt im Betrieb.** Die NMCP-Konvention (`novaberg-convention-nmcp.md`, Stand v0.5) stützt sechs Aussagen auf das Model Context Protocol. Am 19.08.2026 wurden sie gegen die aktuelle Revision `2026-07-28` gehalten; der Draft-Changelog ist leer, es gibt keine neuere. **Vier Aussagen stehen unverändert** — die Auswahl ist kein Protokollgegenstand (das Ablaufdiagramm trägt *Tool Selection* weiterhin ohne Nachricht), Werkzeuge sind *model-controlled*, `instructions` in `server/discover` ist je Server und optional abzurufen, die Verhaltenshinweise sind unverbindlich. **Zwei sind überholt, und vier Ergänzungen fehlen.** Die Rollenmatrix aus §9 ist ein eigener Eintrag (`ROLLENMATRIX-OHNE-PRUEFUNG`, Block 19.08.2026) und hier nicht wiederholt.

| Kennung | Was offen ist | Band |
|---|---|---|
| `NMCP-KONVENTION-MCP-2026-07-28` | **Die Konvention an die Protokollrevision `2026-07-28` anpassen — sechs Stellen, zwei davon Faktenkorrekturen.** ⬜ **offen — entworfen am 19.08.2026, nicht umgesetzt; der Befund ist am 19.09.2026 unverändert (Konvention weiterhin v0.5).** (1) **§3.3 ist unvollständig:** *„alle drei fehlen im etablierten Protokoll“* stimmt für `instructions` weiterhin wörtlich, beschreibt aber das Ökosystem nicht mehr — die Skills-Erweiterung (SEP-2640, `io.modelcontextprotocol/skills`) trägt über die Agent-Skills-Spezifikation eine Beschreibung **je Skill**, lädt sie **beim Start für alle**, und verlangt *„what the skill does and when to use it“*; deren Gegensatzpaar (*Helps with PDFs.* gegen *Use when the user mentions PDFs, forms, or document extraction.*) ist die Gattungsregel aus §3.1. Offen bleiben dort Pflicht zum Lesen, Negativfälle und Quote. (2) **§5.5 ist überholt:** Das Rückfrage-Tor ist im Protokoll gebaut (*Multi Round-Trip Requests*: `resultType: "input_required"`, `inputRequests`, `requestState`). Die Warnung wird zum Verweis auf ein funktionierendes Beispiel — **und die eigene Forderung bleibt sichtbar: MRTR nennt keine Obergrenze für die Zahl der Rückfragen.** (3) **§5.2 fehlt die Vertrauensgrenze:** Für den Handle eines fremden Dienstes nennt der nicht-normative Abschnitt *Stateful Tools* drei Regeln, die die vier Teile der Zusage nicht abdecken — *„a handle is a name, not a capability“* (Berechtigung bei jedem Aufruf prüfen), Opazität, und ein abgelaufener Handle muss das **sagen**. (4) **§8.5 bekommt einen Präzedenzfall:** `isError: true` fasst *API failures* und *business logic errors* zusammen — Störung und Urteil unter einer Flagge, als Form des Protokolls; §6a.2 gilt damit für `tools/call`, nicht nur für den Denkknoten. Die Absicht (*actionable feedback … self-correct*) zielt in die Richtung des vierten Ausgangs; falsch ist die Zusammenlegung. (5) **§3.4 braucht eine Abgrenzung:** Das Skills-Feld `compatibility` nennt **Umgebung** (Pakete, Netzzugang), nicht **Zustand** — wer es auf den Bedarf abbildet, hält die Vorbedingung für gelöst. (6) **Die deterministische Reihenfolge des Bretts ist gebaut und nirgends begründet:** `aushaenge_sammeln` sortiert beide Schleifen. Das Protokoll verlangt dasselbe seit `2026-07-28` für den Prompt-Cache; **der schwerere Grund ist hier ein anderer — ein nach Relevanz oder Quote sortiertes Brett wäre eine Rangfolge (§8.7a)**, und ein Riegel ohne Begründung wird beim nächsten Umbau wegoptimiert. **Was fertig wäre:** Konvention v0.6 mit den sechs Stellen, jede mit Revisionsangabe; der Zustandskasten auf den dann gemessenen Stand. **Nicht mitgemacht:** jede Änderung am Code. | [BAU] ungebändigt |
...
| DIRECTIVE-DATACLASS | `InternalPersonality.directives` ist `list[dict]` mit implizitem Schema `{anweisung, kontext}`. Drei Fremd-Leser lesen die Keys von Hand (`corrector.py:49`, `tribunal.py:147`, `responder.py:465`). Kandidat für eine `Directive`-dataclass — Gegenargument: nur zwei Felder, ein Loader (§11-Faustregel). | [BAU] ⬜ Prio niedrig |
...
- **THINKER-DOPPELFEHLSCHLAG-LIVE-VERIFIKATION (offen, abwartend):** Self-Trigger-Notnagel (Block 3 Teil D+E+F) ist gebaut und logisch belegt, aber der Doppel-Fehlschlag (beide Nachfass-Iterationen liefern leeren `content`) ist im Live-Betrieb noch nie gefeuert. Abwarten — tritt im Normalbetrieb auf, wenn `gemma4` zweimal hintereinander ins `thinking`-Feld driftet. Verifikations-Log: „Doppel-Fehlschlag — Self-Trigger gesetzt" → „continue erzeugt" → „Unsicherheits-Retry erkannt". Erwartetes User-Erlebnis: erste Antwort + „Hmm... ich muss das nochmal durchgehen.", dann zweite Nachricht mit Klärung. Kein eigener Bau — nur Beobachtung. Löst den Diagnose-Logging-Ausbau aus. **→ Live gefeuert am 14.09.2026, 18:52–18:57 UTC, unter `gemma4-a4b-gpu` — die Verifikationskette lief vollständig, das Erlebnis trat nicht ein.** „Doppel-Fehlschlag — Self-Trigger gesetzt" (18:54:10) → „continue erzeugt" → „Unsicherheits-Retry erkannt" (18:55:20) → im Retry erneut zweimal leerer `content` bei gefülltem `thinking` → „Nachfass-Limit erreicht im Unsicherheits-Retry". **Die zweite Nachricht beginnt mit dem vollständigen Text der ersten** und hängt einen neuen Absatz an, dieselbe `turn_id` — keine Klärung, sondern eine Wiederholung. Fundliste 14.09.2026. Damit ist der Pfad belegt und die Beobachtung erledigt; offen ist der Defekt der Wiederholung, und der Diagnose-Logging-Ausbau ist entblockt.
