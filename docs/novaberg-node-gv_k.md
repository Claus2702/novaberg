...
**Absicht:** Nova erkennt über mehrere Turns, wohin ein Gespräch geht, und denkt voraus — in eine Richtung, die sie aus ihrem Charakter wählt, und so weit, wie Vertrautheit, Arousal und Gesprächstyp es tragen, höchstens drei Schritte: Sie antwortet auf das Gemeinte statt nur auf das Gesagte und führt das Gespräch mit, statt es nur zu spiegeln.
**Stand:** 12. September 2026 (am 19.09.2026 in fünf Teile aufgeteilt, ohne inhaltliche Änderung)
**Umsetzung:** Featureliste *`gespraechsvektor` — Landschaft, Strategie, Lücken* 🟢 · *GV1 + GV2* (Farbmischung, Landschaft) 🔴 · *GV4 — Wissenslücken im Prompt* 🔴 · *Wissenslücken-Speicher* 🔴 · *GV-Panel* 🟠 — der Zustand steht dort, nicht hier
**Teile:** [`novaberg-node-gv_t.md`](novaberg-node-gv_t.md) · [`novaberg-node-gv_b.md`](novaberg-node-gv_b.md) · [`novaberg-node-gv_e.md`](novaberg-node-gv_e.md) · [`novaberg-node-gv_m.md`](novaberg-node-gv_m.md)
**Entschieden:** 3 · **Offen beim Meister:** 2 (Liste in [`novaberg-node-gv_e.md`](novaberg-node-gv_e.md))

**§ → Datei.** Die Abschnittsnummern sind die des ungeteilten Konzepts; ein Verweis der Form `novaberg-node-gv_k.md §8.1` findet seinen Abschnitt über diese Tabelle. `_k` ist diese Datei, `_t` ist [`novaberg-node-gv_t.md`](novaberg-node-gv_t.md), `_b` ist [`novaberg-node-gv_b.md`](novaberg-node-gv_b.md), `_e` ist [`novaberg-node-gv_e.md`](novaberg-node-gv_e.md), `_m` ist [`novaberg-node-gv_m.md`](novaberg-node-gv_m.md).
...
| 4 · 4.1 · 4.2 · 4.3 | `_k` |
| 5 | `_k` |
| 6 · 6.1 · 6.2 · 6.3 · 6.4 · 6.5 | `_k` |
| 7 · 7.1 · 7.2 · 7.3 · 7.4 | `_k` |
| 8 · 8.0a · 8.1 · 8.2 · 8.3 · 8.4 · 8.5 | `_t`; aus §8.1 der Unterabschnitt *Ergebnis des Vollaudits* in `_m` |
| 9 · 9.1 · 9.2 · 9.3 · 9.4 | `_k` |
| 10 | `_b` (Einleitung und Tabelle GV1–GV6); der Unterabschnitt *Die Decke ist nicht 3, sie ist modusabhängig …* in `_m`; der Absatz *Architektur-Entscheidung* in `_t`, am Schluss von §10.1 |
| 10.1 · 10.1a | `_t` |
| 11 · 12 | `_e` (Abschnitt D) |
| 13 | `_k` |
| Anhänge *GV3 — Invertierte Perzeption*, *GV4 — Wissenslücken*, *GV-Panel*, *Dreischicht-Architektur* | `_t`; aus *GV4* die zwei `[gemessen 10.09.2026]`-Absätze *Was die erste Messung ergab* und *Die Aufnahmebereitschaft ist dabei nie die Hürde* in `_m` |
| Verweise am Schluss (→ …) | `_k` |
| bisheriger Kopf (Projekt, Dokument, Stand, Nachträge, Pfad, Quellen) | `_m` |
...
> **§8, §10 bis §12 und die Anhänge stehen nicht in dieser Datei.** Die Ausarbeitung (§8, §10.1, §10.1a und die Anhänge zu GV3, GV4, GV-Panel und Dreischicht) steht in [`novaberg-node-gv_t.md`](novaberg-node-gv_t.md), die Implementierungsreihenfolge (§10) in [`novaberg-node-gv_b.md`](novaberg-node-gv_b.md), die offenen Fragen und die Architektur-Zitate (§11, §12) in [`novaberg-node-gv_e.md`](novaberg-node-gv_e.md), die Messungen (der bisherige Kopf, das Vollaudit aus §8.1, die Decke der Vektorlänge aus §10, die erste Messung am Längen-Tor aus dem Anhang GV4) in [`novaberg-node-gv_m.md`](novaberg-node-gv_m.md). Welche Datei welchen Abschnitt trägt, sagt die Tabelle oben.
